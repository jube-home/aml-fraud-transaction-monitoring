...
namespace Jube.Service.Repository.CaseWorkflowFilter
{
    public sealed class CaseWorkflowFilterService
    {
        ...
        [Description("Lists the fields a query builder JSON filter over Case Workflow Filters " +
                     "may use, with each field's type, the operators allowed for it and what it " +
                     "means. Use them as rule ids in CaseWorkflowFilterFilter and " +
                     "CaseWorkflowFilterCount.")]
        [ServiceOperation("CaseWorkflowFilterFilterFields", OperationKind.Read, Idempotent = true)]
        public async Task<List<FilterFieldDto>> FilterFieldsAsync(
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowFilter", "FilterFields", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowFilter.FilterFields: entry user={userName}");
            }

            try
            {
                EnsurePermitted("CaseWorkflowFilter.FilterFields", activeOnlyPermissions);
                await Task.CompletedTask.ConfigureAwait(false);
                var result = DtoFilter.Fields<CaseWorkflowFilterDto>();
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowFilter.FilterFields: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Returns the Case Workflow Filters in the caller's tenant matching query " +
                     "builder JSON (the same format as the rule builder, over the fields from " +
                     "CaseWorkflowFilterFilterFields), ordered by id and capped at 'take' rows " +
        ...
        [ServiceOperation("CaseWorkflowFilterFilter", OperationKind.Read, Idempotent = true)]
        public async Task<FilterResultDto<CaseWorkflowFilterDto>> FilterAsync(
            [Description(
                "Query builder JSON selecting the Case Workflow Filters, using the fields from CaseWorkflowFilterFilterFields; empty selects all.")]
            ...
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowFilter", "Filter", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowFilter.Filter: entry take={take} afterId={afterId} user={userName}");
            }

            try
            {
                EnsurePermitted("CaseWorkflowFilter.Filter", activeOnlyPermissions);
                var rows = CaseWorkflowFilterMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowFilter.Filter: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Counts the Case Workflow Filters in the caller's tenant matching query " +
                     "builder JSON (over the fields from CaseWorkflowFilterFilterFields; empty " +
                     ...
                     "its JSON path.")]
        [ServiceOperation("CaseWorkflowFilterCount", OperationKind.Read, Idempotent = true)]
        public async Task<FilterCountResultDto> CountAsync(
            [Description(
                "Query builder JSON selecting the Case Workflow Filters, using the fields from CaseWorkflowFilterFilterFields; empty selects all.")]
            string? builderJson = null,
            [Description(
                "A field from CaseWorkflowFilterFilterFields to count the matching rows by, e.g. Active; empty for a single total.")]
            string? groupBy = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowFilter", "Count", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowFilter.Count: entry groupBy={groupBy} user={userName}");
            }

            try
            {
                EnsurePermitted("CaseWorkflowFilter.Count", activeOnlyPermissions);
                var rows = CaseWorkflowFilterMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowFilter.Count: unexpected failure user={userName}", ex);
                throw;
            }
        }
        ...
        [Description("Validates a Case Workflow Filter without saving it, running every check a create (Id 0) " +
                     "or an update (any other Id) would run, and returns each failure. Nothing is stored or " +
                     "changed.")]
        [ServiceOperation("CaseWorkflowFilterValidate", OperationKind.Read, Idempotent = true)]
        public async Task<ValidationResultDto> ValidateAsync(
            [Description("The Case Workflow Filter to validate.")]
            CaseWorkflowFilterDto? model,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowFilter", "Validate", userName, tenantRegistryId,
                auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowFilter.Validate: entry id={model?.Id} user={userName}");
            }

            try
            {
                ArgumentNullException.ThrowIfNull(model);
                EnsurePermitted("CaseWorkflowFilter.Validate", permissions);
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowFilter.Validate: unexpected failure user={userName}", ex);
                throw;
            }
        }

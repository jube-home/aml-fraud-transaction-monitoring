...
namespace Jube.Service.Repository.VisualisationRegistryDatasource
{
    public sealed class VisualisationRegistryDatasourceService
    {
        ...
        [Description("Lists the fields a query builder JSON filter over Visualisation Registry " +
                     "Datasources may use, with each field's type, the operators allowed for it " +
                     "and what it means. Use them as rule ids in " +
                     "VisualisationRegistryDatasourceFilter and " +
                     "VisualisationRegistryDatasourceCount.")]
        [ServiceOperation("VisualisationRegistryDatasourceFilterFields", OperationKind.Read, Idempotent = true)]
        public async Task<List<FilterFieldDto>> FilterFieldsAsync(
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistryDatasource", "FilterFields", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"VisualisationRegistryDatasource.FilterFields: entry user={userName}");
            }

            try
            {
                EnsurePermitted(listPermissions, "VisualisationRegistryDatasource.FilterFields");
                await Task.CompletedTask.ConfigureAwait(false);
                var result = DtoFilter.Fields<VisualisationRegistryDatasourceDto>();
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistryDatasource.FilterFields: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Returns the Visualisation Registry Datasources in the caller's tenant " +
                     "matching query builder JSON (the same format as the rule builder, over " +
                     "the fields from VisualisationRegistryDatasourceFilterFields), ordered by " +
                     ...
                     "path.")]
        [ServiceOperation("VisualisationRegistryDatasourceFilter", OperationKind.Read, Idempotent = true)]
        public async Task<FilterResultDto<VisualisationRegistryDatasourceDto>> FilterAsync(
            [Description(
                "Query builder JSON selecting the Visualisation Registry Datasources, using the fields from VisualisationRegistryDatasourceFilterFields; empty selects all.")]
            ...
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistryDatasource", "Filter", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug(
                    $"VisualisationRegistryDatasource.Filter: entry take={take} afterId={afterId} user={userName}");
            }

            try
            {
                EnsurePermitted(listPermissions, "VisualisationRegistryDatasource.Filter");
                var rows = VisualisationRegistryDatasourceMapper.ToDto(await repository.GetAsync(token)
                    .ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistryDatasource.Filter: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Counts the Visualisation Registry Datasources in the caller's tenant " +
                     "matching query builder JSON (over the fields from " +
                     "VisualisationRegistryDatasourceFilterFields; empty counts all), " +
        ...
        [ServiceOperation("VisualisationRegistryDatasourceCount", OperationKind.Read, Idempotent = true)]
        public async Task<FilterCountResultDto> CountAsync(
            [Description(
                "Query builder JSON selecting the Visualisation Registry Datasources, using the fields from VisualisationRegistryDatasourceFilterFields; empty selects all.")]
            string? builderJson = null,
            [Description(
                "A field from VisualisationRegistryDatasourceFilterFields to count the matching rows by, e.g. Active; empty for a single total.")]
            string? groupBy = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistryDatasource", "Count", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"VisualisationRegistryDatasource.Count: entry groupBy={groupBy} user={userName}");
            }

            try
            {
                EnsurePermitted(listPermissions, "VisualisationRegistryDatasource.Count");
                var rows = VisualisationRegistryDatasourceMapper.ToDto(await repository.GetAsync(token)
                    .ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistryDatasource.Count: unexpected failure user={userName}", ex);
                throw;
            }
        }
        ...
        [Description("Validates a Visualisation Registry Datasource without saving it, running every check a " +
                     "create (Id 0) or an update (any other Id) would run, and returns each failure. Nothing " +
                     "is stored or changed.")]
        [ServiceOperation("VisualisationRegistryDatasourceValidate", OperationKind.Read, Idempotent = true)]
        public async Task<ValidationResultDto> ValidateAsync(
            [Description("The Visualisation Registry Datasource to validate.")]
            VisualisationRegistryDatasourceDto? model,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistryDatasource", "Validate", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"VisualisationRegistryDatasource.Validate: entry id={model?.Id} user={userName}");
            }

            try
            {
                ArgumentNullException.ThrowIfNull(model);
                EnsurePermitted(writePermissions, "VisualisationRegistryDatasource.Validate");
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistryDatasource.Validate: unexpected failure user={userName}", ex);
                throw;
            }
        }

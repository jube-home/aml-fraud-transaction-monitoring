...
namespace Jube.Service.Repository.VisualisationRegistry
{
    public sealed class VisualisationRegistryService
    {
        ...
        [Description("Lists the fields a query builder JSON filter over Visualisation Registrys " +
                     "may use, with each field's type, the operators allowed for it and what it " +
                     "means. Use them as rule ids in VisualisationRegistryFilter and " +
                     "VisualisationRegistryCount.")]
        [ServiceOperation("VisualisationRegistryFilterFields", OperationKind.Read, Idempotent = true)]
        public async Task<List<FilterFieldDto>> FilterFieldsAsync(
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistry", "FilterFields", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"VisualisationRegistry.FilterFields: entry user={userName}");
            }

            try
            {
                EnsurePermitted("VisualisationRegistry.FilterFields", listPermissions);
                await Task.CompletedTask.ConfigureAwait(false);
                var result = DtoFilter.Fields<VisualisationRegistryDto>();
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistry.FilterFields: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Returns the Visualisation Registrys in the caller's tenant matching query " +
                     "builder JSON (the same format as the rule builder, over the fields from " +
                     "VisualisationRegistryFilterFields), ordered by id and capped at 'take' " +
        ...
        [ServiceOperation("VisualisationRegistryFilter", OperationKind.Read, Idempotent = true)]
        public async Task<FilterResultDto<VisualisationRegistryDto>> FilterAsync(
            [Description(
                "Query builder JSON selecting the Visualisation Registrys, using the fields from VisualisationRegistryFilterFields; empty selects all.")]
            ...
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistry", "Filter", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"VisualisationRegistry.Filter: entry take={take} afterId={afterId} user={userName}");
            }

            try
            {
                EnsurePermitted("VisualisationRegistry.Filter", listPermissions);
                var rows = VisualisationRegistryMapper.ToDto(await repository.GetOrderByIdAsync(token)
                    .ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistry.Filter: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Counts the Visualisation Registrys in the caller's tenant matching query " +
                     "builder JSON (over the fields from VisualisationRegistryFilterFields; " +
        ...
        [ServiceOperation("VisualisationRegistryCount", OperationKind.Read, Idempotent = true)]
        public async Task<FilterCountResultDto> CountAsync(
            [Description(
                "Query builder JSON selecting the Visualisation Registrys, using the fields from VisualisationRegistryFilterFields; empty selects all.")]
            string? builderJson = null,
            [Description(
                "A field from VisualisationRegistryFilterFields to count the matching rows by, e.g. Active; empty for a single total.")]
            string? groupBy = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistry", "Count", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"VisualisationRegistry.Count: entry groupBy={groupBy} user={userName}");
            }

            try
            {
                EnsurePermitted("VisualisationRegistry.Count", listPermissions);
                var rows = VisualisationRegistryMapper.ToDto(await repository.GetOrderByIdAsync(token)
                    .ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistry.Count: unexpected failure user={userName}", ex);
                throw;
            }
        }
        ...
        [Description("Validates a Visualisation Registry without saving it, running every check a create (Id " +
                     "0) or an update (any other Id) would run, and returns each failure. Nothing is stored or " +
                     "changed.")]
        [ServiceOperation("VisualisationRegistryValidate", OperationKind.Read, Idempotent = true)]
        public async Task<ValidationResultDto> ValidateAsync(
            [Description("The Visualisation Registry to validate.")]
            VisualisationRegistryDto? model,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("VisualisationRegistry", "Validate", userName, tenantRegistryId,
                auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"VisualisationRegistry.Validate: entry id={model?.Id} user={userName}");
            }

            try
            {
                ArgumentNullException.ThrowIfNull(model);
                EnsurePermitted("VisualisationRegistry.Validate", writePermissions);
            ...
            {
                op.Error(ex);
                log.Error($"VisualisationRegistry.Validate: unexpected failure user={userName}", ex);
                throw;
            }
        }

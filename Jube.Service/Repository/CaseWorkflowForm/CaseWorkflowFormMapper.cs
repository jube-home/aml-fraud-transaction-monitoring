...
using RulePoco = Jube.Data.Poco.CaseWorkflowForm;

namespace Jube.Service.Repository.CaseWorkflowForm
{
    internal static class CaseWorkflowFormMapper
    {
        [return: NotNullIfNotNull(nameof(rulePoco))]
        public static CaseWorkflowFormDto? ToDto(RulePoco? rulePoco) => rulePoco is null
            ? null
            : new CaseWorkflowFormDto
            {
                Id = rulePoco.Id,
                Name = rulePoco.Name,
        ...
        public static List<CaseWorkflowFormDto> ToDto(IEnumerable<RulePoco> source) =>
            source.Select(rulePoco => ToDto(rulePoco)).ToList();

        public static RulePoco ToPoco(CaseWorkflowFormDto dto) => new()
        {
            Id = dto.Id,
            Name = dto.Name,

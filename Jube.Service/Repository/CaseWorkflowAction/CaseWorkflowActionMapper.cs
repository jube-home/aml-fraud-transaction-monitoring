...
using RulePoco = Jube.Data.Poco.CaseWorkflowAction;

namespace Jube.Service.Repository.CaseWorkflowAction
{
    internal static class CaseWorkflowActionMapper
    {
        [return: NotNullIfNotNull(nameof(rulePoco))]
        public static CaseWorkflowActionDto? ToDto(RulePoco? rulePoco) => rulePoco is null
            ? null
            : new CaseWorkflowActionDto
            {
                Id = rulePoco.Id,
                Name = rulePoco.Name,
        ...
        public static List<CaseWorkflowActionDto> ToDto(IEnumerable<RulePoco> source) =>
            source.Select(rulePoco => ToDto(rulePoco)).ToList();

        public static RulePoco ToPoco(CaseWorkflowActionDto dto) => new()
        {
            Id = dto.Id,
            Name = dto.Name,

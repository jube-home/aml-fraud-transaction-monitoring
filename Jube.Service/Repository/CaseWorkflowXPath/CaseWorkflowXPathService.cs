...
namespace Jube.Service.Repository.CaseWorkflowXPath
{
    public sealed class CaseWorkflowXPathService
    {
        ...
        [Description("Lists the fields a query builder JSON filter over Case WorkflowXPaths may " +
                     "use, with each field's type, the operators allowed for it and what it " +
                     "means. Use them as rule ids in CaseWorkflowXPathFilter and " +
                     "CaseWorkflowXPathCount.")]
        [ServiceOperation("CaseWorkflowXPathFilterFields", OperationKind.Read, Idempotent = true)]
        public async Task<List<FilterFieldDto>> FilterFieldsAsync(
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowXPath", "FilterFields", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowXPath.FilterFields: entry user={userName}");
            }

            try
            {
                EnsurePermitted("CaseWorkflowXPath.FilterFields", permissions);
                await Task.CompletedTask.ConfigureAwait(false);
                var result = DtoFilter.Fields<CaseWorkflowXPathDto>();
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowXPath.FilterFields: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Returns the Case WorkflowXPaths in the caller's tenant matching query " +
                     "builder JSON (the same format as the rule builder, over the fields from " +
                     "CaseWorkflowXPathFilterFields), ordered by id and capped at 'take' rows " +
        ...
        [ServiceOperation("CaseWorkflowXPathFilter", OperationKind.Read, Idempotent = true)]
        public async Task<FilterResultDto<CaseWorkflowXPathDto>> FilterAsync(
            [Description(
                "Query builder JSON selecting the Case WorkflowXPaths, using the fields from CaseWorkflowXPathFilterFields; empty selects all.")]
            ...
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowXPath", "Filter", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowXPath.Filter: entry take={take} afterId={afterId} user={userName}");
            }

            try
            {
                EnsurePermitted("CaseWorkflowXPath.Filter", permissions);
                var rows = CaseWorkflowXPathMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowXPath.Filter: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Counts the Case WorkflowXPaths in the caller's tenant matching query " +
                     "builder JSON (over the fields from CaseWorkflowXPathFilterFields; empty " +
                     ...
                     "its JSON path.")]
        [ServiceOperation("CaseWorkflowXPathCount", OperationKind.Read, Idempotent = true)]
        public async Task<FilterCountResultDto> CountAsync(
            [Description(
                "Query builder JSON selecting the Case WorkflowXPaths, using the fields from CaseWorkflowXPathFilterFields; empty selects all.")]
            string? builderJson = null,
            [Description(
                "A field from CaseWorkflowXPathFilterFields to count the matching rows by, e.g. Active; empty for a single total.")]
            string? groupBy = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowXPath", "Count", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowXPath.Count: entry groupBy={groupBy} user={userName}");
            }

            try
            {
                EnsurePermitted("CaseWorkflowXPath.Count", permissions);
                var rows = CaseWorkflowXPathMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowXPath.Count: unexpected failure user={userName}", ex);
                throw;
            }
        }
        ...
        [Description("Validates a Case WorkflowXPath without saving it, running every check a create (Id 0) or " +
                     "an update (any other Id) would run, and returns each failure. Nothing is stored or " +
                     "changed.")]
        [ServiceOperation("CaseWorkflowXPathValidate", OperationKind.Read, Idempotent = true)]
        public async Task<ValidationResultDto> ValidateAsync(
            [Description("The Case WorkflowXPath to validate.")]
            CaseWorkflowXPathDto? model,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("CaseWorkflowXPath", "Validate", userName, tenantRegistryId,
                auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"CaseWorkflowXPath.Validate: entry id={model?.Id} user={userName}");
            }

            try
            {
                ArgumentNullException.ThrowIfNull(model);
                EnsurePermitted("CaseWorkflowXPath.Validate", permissions);
            ...
            {
                op.Error(ex);
                log.Error($"CaseWorkflowXPath.Validate: unexpected failure user={userName}", ex);
                throw;
            }
        }

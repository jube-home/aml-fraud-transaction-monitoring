...
using RulePoco = Jube.Data.Poco.CaseWorkflowMacro;

namespace Jube.Service.Repository.CaseWorkflowMacro
{
    internal static class CaseWorkflowMacroMapper
    {
        [return: NotNullIfNotNull(nameof(rulePoco))]
        public static CaseWorkflowMacroDto? ToDto(RulePoco? rulePoco) => rulePoco is null
            ? null
            : new CaseWorkflowMacroDto
            {
                Id = rulePoco.Id,
                Name = rulePoco.Name,
                Active = rulePoco.Active == 1,
                Locked = rulePoco.Locked == 1,
                Javascript = rulePoco.Javascript,
                ImageLocation = rulePoco.ImageLocation,
        ...
        public static List<CaseWorkflowMacroDto> ToDto(IEnumerable<RulePoco> source) =>
            source.Select(rulePoco => ToDto(rulePoco)).ToList();

        public static RulePoco ToPoco(CaseWorkflowMacroDto dto) => new()
        {
            Id = dto.Id,
            Name = dto.Name,

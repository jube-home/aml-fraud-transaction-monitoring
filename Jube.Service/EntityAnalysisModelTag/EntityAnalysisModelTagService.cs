...
namespace Jube.Service.EntityAnalysisModelTag
{
    ...
    public sealed class EntityAnalysisModelTagService
    {
        ...
        [Description("Lists the fields a query builder JSON filter over Tags may use, with each " +
                     "field's type, the operators allowed for it and what it means. Use them as " +
                     "rule ids in EntityAnalysisModelTagFilter and EntityAnalysisModelTagCount.")]
        [ServiceOperation("EntityAnalysisModelTagFilterFields", OperationKind.Read, Idempotent = true)]
        public async Task<List<FilterFieldDto>> FilterFieldsAsync(
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelTag", "FilterFields", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelTag.FilterFields: entry user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelTag.FilterFields");
                await Task.CompletedTask.ConfigureAwait(false);
                var result = DtoFilter.Fields<EntityAnalysisModelTagDto>();
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelTag.FilterFields: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Returns the Tags in the caller's tenant matching query builder JSON (the " +
                     "same format as the rule builder, over the fields from " +
                     "EntityAnalysisModelTagFilterFields), ordered by id and capped at 'take' " +
        ...
        [ServiceOperation("EntityAnalysisModelTagFilter", OperationKind.Read, Idempotent = true)]
        public async Task<FilterResultDto<EntityAnalysisModelTagDto>> FilterAsync(
            [Description(
                "Query builder JSON selecting the Tags, using the fields from EntityAnalysisModelTagFilterFields; empty selects all.")]
            ...
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelTag", "Filter", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelTag.Filter: entry take={take} afterId={afterId} user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelTag.Filter");
                var rows = EntityAnalysisModelTagMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelTag.Filter: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Counts the Tags in the caller's tenant matching query builder JSON (over " +
                     "the fields from EntityAnalysisModelTagFilterFields; empty counts all), " +
        ...
        [ServiceOperation("EntityAnalysisModelTagCount", OperationKind.Read, Idempotent = true)]
        public async Task<FilterCountResultDto> CountAsync(
            [Description(
                "Query builder JSON selecting the Tags, using the fields from EntityAnalysisModelTagFilterFields; empty selects all.")]
            string? builderJson = null,
            [Description(
                "A field from EntityAnalysisModelTagFilterFields to count the matching rows by, e.g. Active; empty for a single total.")]
            string? groupBy = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelTag", "Count", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelTag.Count: entry groupBy={groupBy} user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelTag.Count");
                var rows = EntityAnalysisModelTagMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelTag.Count: unexpected failure user={userName}", ex);
                throw;
            }
        }
        ...
        [Description("Validates a Tag without saving it, running every check a create (Id 0) or an update (any " +
                     "other Id) would run, and returns each failure. Nothing is stored or changed.")]
        [ServiceOperation("EntityAnalysisModelTagValidate", OperationKind.Read, Idempotent = true)]
        public async Task<ValidationResultDto> ValidateAsync(
            [Description("The Tag to validate.")] EntityAnalysisModelTagDto? model,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelTag", "Validate", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelTag.Validate: entry id={model?.Id} user={userName}");
            }

            try
            {
                ArgumentNullException.ThrowIfNull(model);
                EnsurePermitted("EntityAnalysisModelTag.Validate");
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelTag.Validate: unexpected failure user={userName}", ex);
                throw;
            }
        }

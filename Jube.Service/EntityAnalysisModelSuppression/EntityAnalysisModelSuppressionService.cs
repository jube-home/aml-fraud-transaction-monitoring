...
namespace Jube.Service.EntityAnalysisModelSuppression
{
    ...
    public sealed class EntityAnalysisModelSuppressionService
    {
        ...
        public async Task<List<EntityAnalysisModelSuppressionDto>> GetAsync(CancellationToken token = default)
        {
            ...
            {
                log.Debug($"EntityAnalysisModelSuppression.List: entry user={userName}");
            }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelSuppression.List: cancelled user={userName}");
                }
            ...
            CancellationToken token = default)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelSuppression.Get: entry id={id} user={userName}");
            }
        ...
        [Description("Lists the fields a query builder JSON filter over Suppressions may use, " +
                     "with each field's type, the operators allowed for it and what it means. " +
                     "Use them as rule ids in EntityAnalysisModelSuppressionFilter and " +
                     "EntityAnalysisModelSuppressionCount.")]
        [ServiceOperation("EntityAnalysisModelSuppressionFilterFields", OperationKind.Read, Idempotent = true)]
        public async Task<List<FilterFieldDto>> FilterFieldsAsync(
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelSuppression", "FilterFields", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelSuppression.FilterFields: entry user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelSuppression.FilterFields");
                await Task.CompletedTask.ConfigureAwait(false);
                var result = DtoFilter.Fields<EntityAnalysisModelSuppressionDto>();
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelSuppression.FilterFields: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Returns the Suppressions in the caller's tenant matching query builder " +
                     "JSON (the same format as the rule builder, over the fields from " +
                     "EntityAnalysisModelSuppressionFilterFields), ordered by id and capped at " +
        ...
        [ServiceOperation("EntityAnalysisModelSuppressionFilter", OperationKind.Read, Idempotent = true)]
        public async Task<FilterResultDto<EntityAnalysisModelSuppressionDto>> FilterAsync(
            [Description(
                "Query builder JSON selecting the Suppressions, using the fields from EntityAnalysisModelSuppressionFilterFields; empty selects all.")]
            ...
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelSuppression", "Filter", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug(
                    $"EntityAnalysisModelSuppression.Filter: entry take={take} afterId={afterId} user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelSuppression.Filter");
                var rows = EntityAnalysisModelSuppressionMapper.ToDto(await repository.GetAsync(token)
                    .ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelSuppression.Filter: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Counts the Suppressions in the caller's tenant matching query builder " +
                     "JSON (over the fields from EntityAnalysisModelSuppressionFilterFields; " +
        ...
        [ServiceOperation("EntityAnalysisModelSuppressionCount", OperationKind.Read, Idempotent = true)]
        public async Task<FilterCountResultDto> CountAsync(
            [Description(
                "Query builder JSON selecting the Suppressions, using the fields from EntityAnalysisModelSuppressionFilterFields; empty selects all.")]
            string? builderJson = null,
            [Description(
                "A field from EntityAnalysisModelSuppressionFilterFields to count the matching rows by, e.g. Active; empty for a single total.")]
            string? groupBy = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelSuppression", "Count", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelSuppression.Count: entry groupBy={groupBy} user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelSuppression.Count");
                var rows = EntityAnalysisModelSuppressionMapper.ToDto(await repository.GetAsync(token)
                    .ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelSuppression.Count: unexpected failure user={userName}", ex);
                throw;
            }
        }
            ...
            CancellationToken token = default)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelSuppression.Create: entry user={userName}");
            }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelSuppression.Create: cancelled user={userName}");
                }
        ...
        [Description("Validates a Suppression without saving it, running every check a create (Id 0) or an " +
                     "update (any other Id) would run, and returns each failure. Nothing is stored or changed.")]
        [ServiceOperation("EntityAnalysisModelSuppressionValidate", OperationKind.Read, Idempotent = true)]
        public async Task<ValidationResultDto> ValidateAsync(
            [Description("The Suppression to validate.")]
            EntityAnalysisModelSuppressionDto? model,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelSuppression", "Validate", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelSuppression.Validate: entry id={model?.Id} user={userName}");
            }

            try
            {
                ArgumentNullException.ThrowIfNull(model);
                EnsurePermitted("EntityAnalysisModelSuppression.Validate");
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelSuppression.Validate: unexpected failure user={userName}", ex);
                throw;
            }
        }
            ...
            CancellationToken token = default)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelSuppression.Delete: entry id={id} user={userName}");
            }

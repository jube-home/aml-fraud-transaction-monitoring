...
namespace Jube.Service.EntityAnalysisModelList
{
    ...
    public sealed class EntityAnalysisModelListService
    {
        ...
        public async Task<List<EntityAnalysisModelListDto>> GetAsync(CancellationToken token = default)
        {
            ...
            {
                log.Debug($"EntityAnalysisModelList.List: entry user={userName}");
            }
            ...
            try
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelList.List: {dtos.Count} rows user={userName}");
                }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelList.List: cancelled user={userName}");
                }
            ...
            CancellationToken token = default)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelList.Get: entry id={id} user={userName}");
            }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelList.Get: cancelled id={id} user={userName}");
                }
            ...
            CancellationToken token = default)
        {
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelList.ListPaged: cancelled user={userName}");
                }
        ...
        [Description("Lists the fields a query builder JSON filter over Lists may use, with " +
                     "each field's type, the operators allowed for it and what it means. Use " +
                     "them as rule ids in EntityAnalysisModelListFilter and " +
                     "EntityAnalysisModelListCount.")]
        [ServiceOperation("EntityAnalysisModelListFilterFields", OperationKind.Read, Idempotent = true)]
        public async Task<List<FilterFieldDto>> FilterFieldsAsync(
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelList", "FilterFields", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelList.FilterFields: entry user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelList.FilterFields");
                await Task.CompletedTask.ConfigureAwait(false);
                var result = DtoFilter.Fields<EntityAnalysisModelListDto>();
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelList.FilterFields: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Returns the Lists in the caller's tenant matching query builder JSON (the " +
                     "same format as the rule builder, over the fields from " +
                     "EntityAnalysisModelListFilterFields), ordered by id and capped at 'take' " +
                     "rows (max 200). If 'more' is true, call again with 'afterId' set to the " +
                     "last returned Id to continue. Invalid JSON is not an error: Valid is " +
                     "false and Errors gives each problem with its JSON path.")]
        [ServiceOperation("EntityAnalysisModelListFilter", OperationKind.Read, Idempotent = true)]
        public async Task<FilterResultDto<EntityAnalysisModelListDto>> FilterAsync(
            [Description(
                "Query builder JSON selecting the Lists, using the fields from EntityAnalysisModelListFilterFields; empty selects all.")]
            ...
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelList", "Filter", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelList.Filter: entry take={take} afterId={afterId} user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelList.Filter");
                var rows = EntityAnalysisModelListMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelList.Filter: unexpected failure user={userName}", ex);
                throw;
            }
        }

        [Description("Counts the Lists in the caller's tenant matching query builder JSON (over " +
                     "the fields from EntityAnalysisModelListFilterFields; empty counts all), " +
        ...
        [ServiceOperation("EntityAnalysisModelListCount", OperationKind.Read, Idempotent = true)]
        public async Task<FilterCountResultDto> CountAsync(
            [Description(
                "Query builder JSON selecting the Lists, using the fields from EntityAnalysisModelListFilterFields; empty selects all.")]
            string? builderJson = null,
            [Description(
                "A field from EntityAnalysisModelListFilterFields to count the matching rows by, e.g. Active; empty for a single total.")]
            string? groupBy = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelList", "Count", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelList.Count: entry groupBy={groupBy} user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelList.Count");
                var rows = EntityAnalysisModelListMapper.ToDto(await repository.GetAsync(token).ConfigureAwait(false));
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelList.Count: unexpected failure user={userName}", ex);
                throw;
            }
        }
            ...
            CancellationToken token = default)
        {
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelList.Create: cancelled user={userName}");
                }
        ...
        [Description("Validates a List without saving it, running every check a create (Id 0) or an update " +
                     "(any other Id) would run, and returns each failure. Nothing is stored or changed.")]
        [ServiceOperation("EntityAnalysisModelListValidate", OperationKind.Read, Idempotent = true)]
        public async Task<ValidationResultDto> ValidateAsync(
            [Description("The List to validate.")] EntityAnalysisModelListDto? model,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("EntityAnalysisModelList", "Validate", userName,
                tenantRegistryId, auditLog, log, serviceChangeBus);
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelList.Validate: entry id={model?.Id} user={userName}");
            }

            try
            {
                ArgumentNullException.ThrowIfNull(model);
                EnsurePermitted("EntityAnalysisModelList.Validate");
            ...
            {
                op.Error(ex);
                log.Error($"EntityAnalysisModelList.Validate: unexpected failure user={userName}", ex);
                throw;
            }
        }
            ...
            CancellationToken token = default)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"EntityAnalysisModelList.Delete: entry id={id} user={userName}");
            }
                ...
                try
                {
                    ...
                    if (existing != null)
                    {
                        var dependents = await deleteValidator
                            .ValidateAsync(
                                new ModelEntityDelete(ModelEntityKind.List, id, null, existing.EntityAnalysisModelGuid),
                                token)
                            .ConfigureAwait(false);

...
using Jube.Dto.EntityAnalysisModelResponseTimePipelineCounter;
using Jube.Dto.Payload;
using Jube.Resources;
using Jube.Service.Agent;
using Jube.Service.Exceptions.EntityAnalysisModelResponseTimePipelineCounter;
...
namespace Jube.Service.EntityAnalysisModelResponseTimePipelineCounter
{
    public sealed class EntityAnalysisModelResponseTimePipelineCounterService
    {
        ...
        private readonly EntityAnalysisModelResponseTimePipelineCounterRepository repository;
        private readonly IServiceChangeBus serviceChangeBus;
        private readonly IStringLocalizer strings;
        private readonly int tenantRegistryId;
        private readonly string userName;

        private EntityAnalysisModelResponseTimePipelineCounterService(DbContext dbContext, string userName,
            ...
            IServiceChangeBus serviceChangeBus, IStringLocalizer strings)
        {
            ...
            repository = new EntityAnalysisModelResponseTimePipelineCounterRepository(dbContext, userName);
        }

        public static Task<EntityAnalysisModelResponseTimePipelineCounterService> CreateAsync(DbContext dbContext,
        ...
        internal static async Task<EntityAnalysisModelResponseTimePipelineCounterService> CreateAsync(
            DbContext dbContext, string? userName, ILog log, IStringLocalizerFactory stringLocalizerFactory,
            IServiceChangeBus serviceChangeBus, ILog auditLog, CancellationToken token = default)
        {
            var strings =
                stringLocalizerFactory.Create(typeof(EntityAnalysisModelResponseTimePipelineCounterResources));

            if (string.IsNullOrWhiteSpace(userName))
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn("EntityAnalysisModelResponseTimePipelineCounter.Create: no authenticated user; refusing.");
                }

                throw new NotAuthenticatedException(
                    strings[EntityAnalysisModelResponseTimePipelineCounterResources.NotAuthenticated]);
            }
            ...
            if (resolvedTenantRegistryId is null)
            {
                ...
                {
                    log.Warn(
                        $"EntityAnalysisModelResponseTimePipelineCounter.Create: user '{userName}' resolves to no tenant; refusing.");
                }
            ...
            return new EntityAnalysisModelResponseTimePipelineCounterService(dbContext, userName,
                resolvedTenantRegistryId.Value, permissionValidation, log, auditLog, serviceChangeBus, strings);
        }

        [Description("Lists the most recent Response Time Pipeline Counter rows across every Model, most " +
                     ...
                     "MinMicroseconds, MaxMicroseconds, TotalAllocatedBytes, MinAllocatedBytes, " +
                     "MaxAllocatedBytes, InvokeCount) over the full filtered set.")]
        [ServiceOperation("EntityAnalysisModelResponseTimePipelineCounterList", OperationKind.Read, Idempotent = true)]
        public async Task<PayloadResult<EntityAnalysisModelResponseTimePipelineCounterDto>> ListAsync(
            [Description(
            ...
            DateTime? to = null,
            [Description(
                "Exact match against StageId (1=Parse, 2=CheckIntegrityAndUpsert, 3=InlineFunctions, 4=InlineScripts, 5=Gateway, 6=CacheDbStorage, 10=JoinReadTasks, 11=AbstractionRulesWithoutSearchKeys, 12=AbstractionCalculations, 13=ExhaustiveAdaptation, 14=HttpAdaptation, 15=Activation, 16=JoinWriteTasks, 17=WriteResponse, 18=BuildArchivePayload); null matches every row.")]
            int? stageId = null,
            [Description(
        ...
        {
            using var op = OperationScope.Start("EntityAnalysisModelResponseTimePipelineCounter", "List", userName,
            ...
            {
                log.Debug(
                    $"EntityAnalysisModelResponseTimePipelineCounter.List: entry take={clampedTake} user={userName}");
            }

            try
            {
                EnsurePermitted("EntityAnalysisModelResponseTimePipelineCounter.List");
                ...
                var dtos = rows.Select(r => new EntityAnalysisModelResponseTimePipelineCounterDto(
                    r.Id, r.EntityAnalysisModelGuid, r.EntityAnalysisModelName, r.StageId.GetValueOrDefault(),
                    ((InvokeStage)r.StageId.GetValueOrDefault()).ToString(),
                    r.SequenceNumber.GetValueOrDefault(), r.TotalMicroseconds.GetValueOrDefault(),
                    r.MinMicroseconds.GetValueOrDefault(), r.MaxMicroseconds.GetValueOrDefault(),
                    r.TotalAllocatedBytes.GetValueOrDefault(), r.MinAllocatedBytes.GetValueOrDefault(),
                    r.MaxAllocatedBytes.GetValueOrDefault(), r.InvokeCount.GetValueOrDefault(),
                ...
                {
                    log.Debug(
                        $"EntityAnalysisModelResponseTimePipelineCounter.List: {dtos.Count} rows user={userName}");
                }

                return new PayloadResult<EntityAnalysisModelResponseTimePipelineCounterDto>(dtos, total, statistics);
            }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"EntityAnalysisModelResponseTimePipelineCounter.List: cancelled user={userName}");
                }
            ...
            catch (Exception ex)
            {
                op.Error(ex);
                log.Error(
                    $"EntityAnalysisModelResponseTimePipelineCounter.List: unexpected failure user={userName}", ex);
                throw;
            }
        }
        ...
        private void EnsurePermitted(string op)
        {
            ...
            throw new ForbiddenException(
                strings[EntityAnalysisModelResponseTimePipelineCounterResources.PermissionDenied], permissions);
        }
    }
}
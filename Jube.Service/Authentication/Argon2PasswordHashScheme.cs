...
{
    public sealed class Argon2PasswordHashScheme : IPasswordHashScheme
    {
        public bool Verify(string? passwordHash, string password, string? key)
        {
            return HashPassword.Verify(passwordHash, password, key);
        }

        public string Hash(string password, string? key)
        {
            return HashPassword.Argon2(password, key);
        }
    }
}
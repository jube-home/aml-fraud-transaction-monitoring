...
namespace Jube.Service.PostgresLogEntry
{
    public sealed class PostgresLogEntryService
    {
        ...
        private readonly PostgresLogEntryRepository repository;
        private readonly IServiceChangeBus serviceChangeBus;
        private readonly IStringLocalizer strings;
        private readonly int tenantRegistryId;
        private readonly string userName;

        private PostgresLogEntryService(DbContext dbContext, string userName, int tenantRegistryId,
            ...
            IStringLocalizer strings)
        {
            ...
            repository = new PostgresLogEntryRepository(dbContext);
        }

        public static Task<PostgresLogEntryService> CreateAsync(DbContext dbContext, string? userName, ILog log,
        ...
        internal static async Task<PostgresLogEntryService> CreateAsync(DbContext dbContext, string? userName,
            ILog log, IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            ILog auditLog, CancellationToken token = default)
        {
            var strings = stringLocalizerFactory.Create(typeof(PostgresLogEntryResources));

            if (string.IsNullOrWhiteSpace(userName))
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn("PostgresLogEntry.Create: no authenticated user; refusing.");
                }

                throw new NotAuthenticatedException(strings[PostgresLogEntryResources.NotAuthenticated]);
            }
            ...
            if (resolvedTenantRegistryId is null)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"PostgresLogEntry.Create: user '{userName}' resolves to no tenant; refusing.");
                }
            ...
            return new PostgresLogEntryService(dbContext, userName, resolvedTenantRegistryId.Value,
                permissionValidation, log, auditLog, serviceChangeBus, strings);
        }

        [Description("Lists Postgres's own server log lines (checkpoints, connection issues, deadlocks, " +
                     "crashes, slow autovacuum, ...), tailed via pg_ls_logdir()/pg_read_file() -- requires " +
                     "logging_collector = on server-side, otherwise this always returns an empty list. Most " +
                     ...
                     "and/or a case-insensitive substring search against Message or Level. Optionally apply " +
                     ...
                     "Statistics -- empty here, since this DTO has no continuous measured columns (Pid is a " +
                     "process identifier, not a measurement, and every other column is a string or " +
                     "timestamp).")]
        [ServiceOperation("PostgresLogEntryList", OperationKind.Read, Idempotent = true)]
        public async Task<PayloadResult<PostgresLogEntryDto>> ListAsync(
            [Description(
            ...
            DateTime? to = null,
            [Description(
                "Case-insensitive substring match against Message or Level; null or empty matches every row.")]
            string? search = null,
            [Description(
            ...
            [Description(
                "Column to sort by (e.g. 'occurredDate', 'level', 'pid'); unrecognised or omitted falls back to most-recent-first.")]
            string? sortField = null,
            [Description("'asc' for ascending; anything else (including omitted) sorts descending.")]
            string? sortDirection = null,
            CancellationToken token = default)
        {
            using var op = OperationScope.Start("PostgresLogEntry", "List", userName, tenantRegistryId, auditLog,
            ...
            {
                log.Debug($"PostgresLogEntry.List: entry take={clampedTake} user={userName}");
            }

            try
            {
                EnsurePermitted("PostgresLogEntry.List");
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"PostgresLogEntry.List: {dtos.Count} rows user={userName}");
                }

                return new PayloadResult<PostgresLogEntryDto>(dtos, total, statistics);
            }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (log.IsDebugEnabled)
                {
                    log.Debug($"PostgresLogEntry.List: cancelled user={userName}");
                }
            ...
            {
                op.Error(ex);
                log.Error($"PostgresLogEntry.List: unexpected failure user={userName}", ex);
                throw;
            }
        }

...
namespace Jube.ResilientRedisConnection
{
    public interface IHybridResilientRedisDatabase
    {
        ...
        void HashSet(RedisKey key, RedisValue field, RedisValue value, When when = When.Always,
            CommandFlags flags = CommandFlags.None);
        ...
        Task<long> HashIncrementAsync(RedisKey key, RedisValue field, long value,
            CommandFlags flags = CommandFlags.None);

        Task<double> HashIncrementAsync(RedisKey key, RedisValue field, double value,
            CommandFlags flags = CommandFlags.None);

        Task<long> HashDecrementAsync(RedisKey key, RedisValue field, long value,
            CommandFlags flags = CommandFlags.None);

        Task<double> HashDecrementAsync(RedisKey key, RedisValue field, double value,
            CommandFlags flags = CommandFlags.None);
        ...
        IAsyncEnumerable<HashEntry> HashScanAsync(RedisKey key, RedisValue pattern = default, int pageSize = 250,
            long cursor = 0, int pageOffset = 0, CommandFlags flags = CommandFlags.None);

        IEnumerable<HashEntry> HashScan(RedisKey key, RedisValue pattern = default, int pageSize = 250, long cursor = 0,
        ...
        Task<long> SortedSetRemoveRangeByScoreAsync(RedisKey key, double start = double.NegativeInfinity,
            double stop = double.PositiveInfinity,
            Exclude exclude = Exclude.None, CommandFlags flags = CommandFlags.None);

        Task<bool> SortedSetUpdateAsync(RedisKey key, RedisValue member, double score,
        ...
        Task<SortedSetEntry[]> SortedSetRangeByRankWithScoresAsync(RedisKey key, long start = 0, long stop = -1,

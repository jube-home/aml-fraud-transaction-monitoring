...
{
    public static class RedisSlowOperationEndpoints
    {
        private const string Base = "/api/RedisSlowOperation";

        public static void MapRedisSlowOperationEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("RedisSlowOperation");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<RedisSlowOperationDto>>()
                .WithName("RedisSlowOperationGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await RedisSlowOperationService.CreateAsync(dbContext, user, log,

...
{
    public static class OpenTelemetryMetricEndpoints
    {
        private const string Base = "/api/OpenTelemetryMetric";

        public static void MapOpenTelemetryMetricEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("OpenTelemetryMetric");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<OpenTelemetryMetricDto>>()
                .WithName("OpenTelemetryMetricGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await OpenTelemetryMetricService.CreateAsync(dbContext, user, log,

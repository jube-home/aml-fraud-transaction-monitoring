...
{
    public static class PermissionSpecificationEndpoints
    {
        private const string Base = "/api/PermissionSpecification";

        public static void MapPermissionSpecificationEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("PermissionSpecification");

            group.MapGet("", GetAsync)
                .Produces<List<PermissionSpecificationDto>>()
                .WithName("PermissionSpecificationList");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await PermissionSpecificationService.CreateAsync(dbContext, user, log,

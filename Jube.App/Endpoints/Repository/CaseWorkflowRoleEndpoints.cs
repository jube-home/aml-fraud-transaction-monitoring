...
{
    public static class CaseWorkflowRoleEndpoints
    {
        private const string Base = "/api/CaseWorkflowRole";

        public static void MapCaseWorkflowRoleEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflowRole");

            group.MapGet("ByCaseWorkflowGuid/{guid:guid}", ByCaseWorkflowGuidAsync)
                .Produces<List<CaseWorkflowRoleDto>>()
                .WithName("CaseWorkflowRoleListByCaseWorkflowGuid");

            group.MapPost("", CreateAsync)
                .Produces<CaseWorkflowRoleDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("CaseWorkflowRoleCreate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .WithName("CaseWorkflowRoleDelete");
        }

        private static async Task<IResult> ByCaseWorkflowGuidAsync(Guid guid, HttpContext httpContext,
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseWorkflowRoleService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> CreateAsync([FromBody] CaseWorkflowRoleDto model,

...
using Jube.Service.Repository.ExhaustiveSearchInstancePromotedTrialInstance;
...
{
    public static class ExhaustiveSearchInstancePromotedTrialInstanceEndpoints
    {
        private const string Base = "/api/ExhaustiveSearchInstancePromotedTrialInstance";

        public static void MapExhaustiveSearchInstancePromotedTrialInstanceEndpoints(
            this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("ExhaustiveSearchInstancePromotedTrialInstance");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("ExhaustiveSearchInstancePromotedTrialInstanceUpdate");
        }

        private static async Task<IResult> UpdateAsync(
            [FromBody] ExhaustiveSearchInstancePromotedTrialInstanceDto model, HttpContext httpContext, ILog log,
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await ExhaustiveSearchInstancePromotedTrialInstanceService.CreateAsync(dbContext, user,
                    log, stringLocalizerFactory, serviceChangeBus, token);
                await service.UpdateActiveAsync(model, token);

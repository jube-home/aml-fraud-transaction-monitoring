...
{
    public static class EntityAnalysisModelRoleEndpoints
    {
        private const string Base = "/api/EntityAnalysisModelRole";

        public static void MapEntityAnalysisModelRoleEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelRole");

            group.MapGet("ByEntityAnalysisModelGuid/{guid:guid}", ByEntityAnalysisModelGuidAsync)
                .Produces<List<EntityAnalysisModelRoleDto>>()
                .WithName("EntityAnalysisModelRoleListByEntityAnalysisModelGuid");

            group.MapPost("", CreateAsync)
                .Produces<EntityAnalysisModelRoleDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelRoleCreate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .WithName("EntityAnalysisModelRoleDelete");
        }

        private static async Task<IResult> ByEntityAnalysisModelGuidAsync(Guid guid, HttpContext httpContext,
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelRoleService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByEntityAnalysisModelGuidAsync(guid, token));
            }
        ...
        private static async Task<IResult> CreateAsync([FromBody] EntityAnalysisModelRoleDto model,

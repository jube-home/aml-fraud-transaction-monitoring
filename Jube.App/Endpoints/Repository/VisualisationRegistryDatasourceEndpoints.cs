...
using Jube.Service.Repository.VisualisationRegistryDatasource;
using log4net;
...
{
    public static class VisualisationRegistryDatasourceEndpoints
    {
        private const string Base = "/api/VisualisationRegistryDatasource";

        public static void MapVisualisationRegistryDatasourceEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("VisualisationRegistryDatasource");

            group.MapGet("", GetAsync)
                .Produces<List<VisualisationRegistryDatasourceDto>>()
                .WithName("VisualisationRegistryDatasourceGetAll");

            group.MapGet("ByVisualisationRegistryId/{visualisationRegistryId:int}",
                    GetByVisualisationRegistryIdAsync)
                .Produces<List<VisualisationRegistryDatasourceDto>>()
                .WithName("VisualisationRegistryDatasourceGetByVisualisationRegistryId");

            group.MapGet("ByVisualisationRegistryIdActiveOnly/{visualisationRegistryId:int}",
                    GetByVisualisationRegistryIdActiveOnlyAsync)
                .Produces<List<VisualisationRegistryDatasourceDto>>()
                .WithName("VisualisationRegistryDatasourceGetByVisualisationRegistryIdActiveOnly");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<VisualisationRegistryDatasourceDto>()
                .WithName("VisualisationRegistryDatasourceGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("VisualisationRegistryDatasourceCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("VisualisationRegistryDatasourceUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("VisualisationRegistryDatasourceDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await VisualisationRegistryDatasourceService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> GetByVisualisationRegistryIdAsync(
            int visualisationRegistryId, HttpContext httpContext, ILog log,
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await VisualisationRegistryDatasourceService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, dynamicEnvironment, token);
                var result = await service.GetByVisualisationRegistryIdAsync(visualisationRegistryId, token);
                return TypedResults.Ok(result);
            }
            catch (NotAuthenticatedException)
            {
        ...
        private static async Task<IResult> GetByVisualisationRegistryIdActiveOnlyAsync(
            int visualisationRegistryId, HttpContext httpContext, ILog log,
            DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {
            ...
            {
                log.Error(
                    $"GET {Base}/ByVisualisationRegistryIdActiveOnly/{visualisationRegistryId}: 500 user={user}",
                    e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] VisualisationRegistryDatasourceDto model, HttpContext httpContext, ILog log,
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            catch (SqlValidationFailedException ex)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"POST {Base}: 400 (SQL) user={user} message={ex.Message}");
                }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            catch (SqlValidationFailedException ex)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"PUT {Base}: 400 (SQL) user={user} message={ex.Message}");
                }

...
{
    public static class CaseWorkflowMacroEndpoints
    {
        private const string Base = "/api/CaseWorkflowMacro";

        public static void MapCaseWorkflowMacroEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflowMacro");

            group.MapGet("", GetAsync)
                .Produces<List<CaseWorkflowMacroDto>>()
                .WithName("CaseWorkflowMacroList");

            group.MapGet("ByCasesWorkflowIdActiveOnly/{id:int}", GetByCasesWorkflowIdActiveOnlyAsync)
                .Produces<List<CaseWorkflowMacroDto>>()
                .WithName("CaseWorkflowMacroListByWorkflowIdActiveOnly");

            group.MapGet("ByCasesWorkflowGuidActiveOnly/{guid:guid}", GetByCasesWorkflowGuidActiveOnlyAsync)
                .Produces<List<CaseWorkflowMacroDto>>()
                .WithName("CaseWorkflowMacroListByWorkflowGuidActiveOnly");

            group.MapGet("ByCasesWorkflowId/{casesWorkflowId:int}", GetByCaseWorkflowIdAsync)
                .Produces<List<CaseWorkflowMacroDto>>()
                .WithName("CaseWorkflowMacroListByWorkflow");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<CaseWorkflowMacroDto>()
                .WithName("CaseWorkflowMacroGetById");

            group.MapPost("", InsertAsync)
                .Produces<CaseWorkflowMacroDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("CaseWorkflowMacroCreate");

            group.MapPut("", UpdateAsync)
                .Produces<CaseWorkflowMacroDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("CaseWorkflowMacroUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("CaseWorkflowMacroDelete");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseWorkflowMacroService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> InsertAsync([FromBody] CaseWorkflowMacroDto model,
        ...
        private static async Task<IResult> UpdateAsync([FromBody] CaseWorkflowMacroDto model,
            HttpContext httpContext, ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {

...
{
    public static class CaseWorkflowStatusRoleEndpoints
    {
        private const string Base = "/api/CaseWorkflowStatusRole";

        public static void MapCaseWorkflowStatusRoleEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflowStatusRole");

            group.MapGet("ByCaseWorkflowStatusGuid/{guid:guid}", ByCaseWorkflowStatusGuidAsync)
                .Produces<List<CaseWorkflowStatusRoleDto>>()
                .WithName("CaseWorkflowStatusRoleListByCaseWorkflowStatusGuid");

            group.MapPost("", CreateAsync)
                .Produces<CaseWorkflowStatusRoleDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("CaseWorkflowStatusRoleCreate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .WithName("CaseWorkflowStatusRoleDelete");
        }

        private static async Task<IResult> ByCaseWorkflowStatusGuidAsync(Guid guid, HttpContext httpContext,
            ...
            CancellationToken token)
        {
            ...
            {
                log.Debug($"GET {Base}/ByCaseWorkflowStatusGuid/{guid}: entry user={user}");
            }
            ...
            try
            {
                var service = await CaseWorkflowStatusRoleService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByCaseWorkflowStatusGuidAsync(guid, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowStatusGuid/{guid}: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowStatusGuid/{guid}: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/ByCaseWorkflowStatusGuid/{guid}: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/ByCaseWorkflowStatusGuid/{guid}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }

        private static async Task<IResult> CreateAsync([FromBody] CaseWorkflowStatusRoleDto model,

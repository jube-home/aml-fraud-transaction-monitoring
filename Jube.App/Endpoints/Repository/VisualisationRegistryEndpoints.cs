...
using Jube.Service.Repository.VisualisationRegistry;
using log4net;
...
{
    public static class VisualisationRegistryEndpoints
    {
        private const string Base = "/api/VisualisationRegistry";

        public static void MapVisualisationRegistryEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("VisualisationRegistry");

            group.MapGet("", GetAsync)
                .Produces<List<VisualisationRegistryDto>>()
                .WithName("VisualisationRegistryList");

            group.MapGet("GetByShowInDirectory", GetByShowInDirectoryAsync)
                .Produces<List<VisualisationRegistryDto>>()
                .WithName("VisualisationRegistryListByShowInDirectory");

            group.MapGet("GetByGuidActiveOnly/{guid:guid}", GetByGuidActiveOnlyAsync)
                .Produces<VisualisationRegistryDto>()
                .WithName("VisualisationRegistryGetByGuidActiveOnly");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<VisualisationRegistryDto>()
                .WithName("VisualisationRegistryGetById");

            group.MapPost("", InsertAsync)
                .Produces<VisualisationRegistryDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("VisualisationRegistryCreate");

            group.MapPut("", UpdateAsync)
                .Produces<VisualisationRegistryDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("VisualisationRegistryUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("VisualisationRegistryDelete");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await VisualisationRegistryService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> GetByShowInDirectoryAsync(HttpContext httpContext, ILog log,
            DynamicEnvironment.DynamicEnvironment dynamicEnvironment, IStringLocalizerFactory stringLocalizerFactory,
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled)
            {
                log.Debug($"GET {Base}/GetByShowInDirectory: entry user={user}");
            }
            ...
            try
            {
                var service = await VisualisationRegistryService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByShowInDirectoryAsync(token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/GetByShowInDirectory: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/GetByShowInDirectory: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/GetByShowInDirectory: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/GetByShowInDirectory: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }

        private static async Task<IResult> GetByGuidActiveOnlyAsync(Guid guid, HttpContext httpContext, ILog log,
            DynamicEnvironment.DynamicEnvironment dynamicEnvironment, IStringLocalizerFactory stringLocalizerFactory,
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled)
            {
                log.Debug($"GET {Base}/GetByGuidActiveOnly/{guid}: entry user={user}");
            }
            ...
            try
            {
                var service = await VisualisationRegistryService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByGuidActiveOnlyAsync(guid, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/GetByGuidActiveOnly/{guid}: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/GetByGuidActiveOnly/{guid}: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/GetByGuidActiveOnly/{guid}: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/GetByGuidActiveOnly/{guid}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }
        ...
        private static async Task<IResult> InsertAsync([FromBody] VisualisationRegistryDto model,
        ...
        private static async Task<IResult> UpdateAsync([FromBody] VisualisationRegistryDto model,
            HttpContext httpContext, ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {

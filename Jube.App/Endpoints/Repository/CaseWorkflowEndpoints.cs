...
{
    public static class CaseWorkflowEndpoints
    {
        private const string Base = "/api/CaseWorkflow";

        public static void MapCaseWorkflowEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflow");

            group.MapGet("", GetAsync)
                .Produces<List<CaseWorkflowDto>>()
                .WithName("CaseWorkflowList");

            group.MapGet("ByEntityAnalysisModelId/{entityAnalysisModelId:int}", GetByEntityAnalysisModelIdAsync)
                .Produces<List<CaseWorkflowDto>>()
                .WithName("CaseWorkflowListByModel");

            group.MapGet("ByEntityAnalysisModelIdActiveOnly", GetByEntityAnalysisModelIdActiveOnlyAsync)
                .Produces<List<CaseWorkflowDto>>()
                .WithName("CaseWorkflowListByModelActiveOnly");

            group.MapGet("ByEntityAnalysisModelGuidActiveOnly", GetByEntityAnalysisModelGuidActiveOnlyAsync)
                .Produces<List<CaseWorkflowDto>>()
                .WithName("CaseWorkflowListByModelGuidActiveOnly");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<CaseWorkflowDto>()
                .WithName("CaseWorkflowGetById");

            group.MapPost("", InsertAsync)
                .Produces<CaseWorkflowDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("CaseWorkflowCreate");

            group.MapPut("", UpdateAsync)
                .Produces<CaseWorkflowDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("CaseWorkflowUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("CaseWorkflowDelete");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseWorkflowService.CreateAsync(dbContext, user, log, stringLocalizerFactory,
        ...
        private static async Task<IResult> GetByEntityAnalysisModelIdAsync(int entityAnalysisModelId,
            ...
            CancellationToken token)
        {
            ...
            try
            {
                ...
                return TypedResults.Ok(
                    await service.GetByEntityAnalysisModelIdAsync(entityAnalysisModelId, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByEntityAnalysisModelId/{entityAnalysisModelId}: 403 (not authenticated) " +
                             $"user={user}");
                }
        ...
        private static async Task<IResult> GetByEntityAnalysisModelIdActiveOnlyAsync(int id, HttpContext httpContext,
            ...
            CancellationToken token)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"GET {Base}/ByEntityAnalysisModelIdActiveOnly: entry id={id} user={user}");
            }
            ...
            try
            {
                var service = await CaseWorkflowService.CreateAsync(dbContext, user, log, stringLocalizerFactory,
                    serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByEntityAnalysisModelIdActiveOnlyAsync(id, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByEntityAnalysisModelIdActiveOnly: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByEntityAnalysisModelIdActiveOnly: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/ByEntityAnalysisModelIdActiveOnly: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/ByEntityAnalysisModelIdActiveOnly: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }

        private static async Task<IResult> GetByEntityAnalysisModelGuidActiveOnlyAsync(Guid guid,
            HttpContext httpContext, ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"GET {Base}/ByEntityAnalysisModelGuidActiveOnly: entry guid={guid} user={user}");
            }
            ...
            try
            {
                var service = await CaseWorkflowService.CreateAsync(dbContext, user, log, stringLocalizerFactory,
                    serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByEntityAnalysisModelGuidActiveOnlyAsync(guid, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByEntityAnalysisModelGuidActiveOnly: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByEntityAnalysisModelGuidActiveOnly: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/ByEntityAnalysisModelGuidActiveOnly: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/ByEntityAnalysisModelGuidActiveOnly: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }
        ...
        private static async Task<IResult> InsertAsync([FromBody] CaseWorkflowDto model, HttpContext httpContext,
            ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {
        ...
        private static async Task<IResult> UpdateAsync([FromBody] CaseWorkflowDto model, HttpContext httpContext,
            ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {

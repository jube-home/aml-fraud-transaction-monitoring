...
{
    public static class CaseWorkflowDisplayRoleEndpoints
    {
        private const string Base = "/api/CaseWorkflowDisplayRole";

        public static void MapCaseWorkflowDisplayRoleEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflowDisplayRole");

            group.MapGet("ByCaseWorkflowDisplayGuid/{guid:guid}", ByCaseWorkflowDisplayGuidAsync)
                .Produces<List<CaseWorkflowDisplayRoleDto>>()
                .WithName("CaseWorkflowDisplayRoleListByCaseWorkflowDisplayGuid");

            group.MapPost("", CreateAsync)
                .Produces<CaseWorkflowDisplayRoleDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("CaseWorkflowDisplayRoleCreate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .WithName("CaseWorkflowDisplayRoleDelete");
        }

        private static async Task<IResult> ByCaseWorkflowDisplayGuidAsync(Guid guid, HttpContext httpContext,
            ...
            CancellationToken token)
        {
            ...
            {
                log.Debug($"GET {Base}/ByCaseWorkflowDisplayGuid/{guid}: entry user={user}");
            }
            ...
            try
            {
                var service = await CaseWorkflowDisplayRoleService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByCaseWorkflowDisplayGuidAsync(guid, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowDisplayGuid/{guid}: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowDisplayGuid/{guid}: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/ByCaseWorkflowDisplayGuid/{guid}: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/ByCaseWorkflowDisplayGuid/{guid}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }

        private static async Task<IResult> CreateAsync([FromBody] CaseWorkflowDisplayRoleDto model,

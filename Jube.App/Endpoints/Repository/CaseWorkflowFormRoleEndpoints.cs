...
{
    public static class CaseWorkflowFormRoleEndpoints
    {
        private const string Base = "/api/CaseWorkflowFormRole";

        public static void MapCaseWorkflowFormRoleEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflowFormRole");

            group.MapGet("ByCaseWorkflowFormGuid/{guid:guid}", ByCaseWorkflowFormGuidAsync)
                .Produces<List<CaseWorkflowFormRoleDto>>()
                .WithName("CaseWorkflowFormRoleListByCaseWorkflowFormGuid");

            group.MapPost("", CreateAsync)
                .Produces<CaseWorkflowFormRoleDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("CaseWorkflowFormRoleCreate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .WithName("CaseWorkflowFormRoleDelete");
        }

        private static async Task<IResult> ByCaseWorkflowFormGuidAsync(Guid guid, HttpContext httpContext,
            ...
            CancellationToken token)
        {
            ...
            {
                log.Debug($"GET {Base}/ByCaseWorkflowFormGuid/{guid}: entry user={user}");
            }
            ...
            try
            {
                var service = await CaseWorkflowFormRoleService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByCaseWorkflowFormGuidAsync(guid, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowFormGuid/{guid}: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowFormGuid/{guid}: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/ByCaseWorkflowFormGuid/{guid}: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/ByCaseWorkflowFormGuid/{guid}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }

        private static async Task<IResult> CreateAsync([FromBody] CaseWorkflowFormRoleDto model,

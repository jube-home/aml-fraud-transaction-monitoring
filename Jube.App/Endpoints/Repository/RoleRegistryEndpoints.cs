...
{
    public static class RoleRegistryEndpoints
    {
        private const string Base = "/api/RoleRegistry";

        public static void MapRoleRegistryEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("RoleRegistry");

            group.MapGet("", GetAsync)
                .Produces<List<RoleRegistryDto>>()
                .WithName("RoleRegistryList");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<RoleRegistryDto>()
                .WithName("RoleRegistryGetById");

            group.MapPost("", InsertAsync)
                .Produces<RoleRegistryDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("RoleRegistryCreate");

            group.MapPut("", UpdateAsync)
                .Produces<RoleRegistryDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("RoleRegistryUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("RoleRegistryDelete");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await RoleRegistryService.CreateAsync(dbContext, user, log, stringLocalizerFactory,
        ...
        private static async Task<IResult> InsertAsync([FromBody] RoleRegistryDto model, HttpContext httpContext,
        ...
        private static async Task<IResult> UpdateAsync([FromBody] RoleRegistryDto model, HttpContext httpContext,
            ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {

...
{
    public static class CaseWorkflowStatusEndpoints
    {
        private const string Base = "/api/CaseWorkflowStatus";

        public static void MapCaseWorkflowStatusEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflowStatus");

            group.MapGet("ByCasesWorkflowIdActiveOnly/{id:int}", GetByCasesWorkflowIdActiveOnlyAsync)
                .Produces<List<CaseWorkflowStatusDto>>()
                .WithName("CaseWorkflowStatusListByWorkflowIdActiveOnly");

            group.MapGet("ByCasesWorkflowGuidActiveOnly/{guid:guid}", GetByCasesWorkflowGuidActiveOnlyAsync)
                .Produces<List<CaseWorkflowStatusDto>>()
                .WithName("CaseWorkflowStatusListByWorkflowGuidActiveOnly");

            group.MapGet("", GetAsync)
                .Produces<List<CaseWorkflowStatusDto>>()
                .WithName("CaseWorkflowStatusList");

            group.MapGet("ByCaseWorkflowId/{caseWorkflowId:int}", GetByCaseWorkflowIdAsync)
                .Produces<List<CaseWorkflowStatusDto>>()
                .WithName("CaseWorkflowStatusListByWorkflow");

            group.MapGet("ByCaseWorkflowGuid/{guid:guid}", GetByCaseWorkflowGuidAsync)
                .Produces<List<CaseWorkflowStatusDto>>()
                .WithName("CaseWorkflowStatusListByWorkflowGuid");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<CaseWorkflowStatusDto>()
                .WithName("CaseWorkflowStatusGetById");

            group.MapPost("", InsertAsync)
                .Produces<CaseWorkflowStatusDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("CaseWorkflowStatusCreate");

            group.MapPut("", UpdateAsync)
                .Produces<CaseWorkflowStatusDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("CaseWorkflowStatusUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("CaseWorkflowStatusDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseWorkflowStatusService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> GetByCaseWorkflowIdAsync(int caseWorkflowId, HttpContext httpContext,
            ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"GET {Base}/ByCaseWorkflowId/{caseWorkflowId}: entry user={user}");
            }
            ...
            try
            {
                var service = await CaseWorkflowStatusService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByCaseWorkflowIdAsync(caseWorkflowId, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowId/{caseWorkflowId}: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowId/{caseWorkflowId}: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/ByCaseWorkflowId/{caseWorkflowId}: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/ByCaseWorkflowId/{caseWorkflowId}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"GET {Base}/ByCaseWorkflowGuid/{guid}: entry user={user}");
            }
            ...
            try
            {
                var service = await CaseWorkflowStatusService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetByCaseWorkflowGuidAsync(guid, token));
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowGuid/{guid}: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/ByCaseWorkflowGuid/{guid}: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/ByCaseWorkflowGuid/{guid}: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/ByCaseWorkflowGuid/{guid}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }
        ...
        private static async Task<IResult> InsertAsync([FromBody] CaseWorkflowStatusDto model,
        ...
        private static async Task<IResult> UpdateAsync([FromBody] CaseWorkflowStatusDto model,
            HttpContext httpContext, ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {

...
{
    public static class EtcdMemberStatusEndpoints
    {
        private const string Base = "/api/EtcdMemberStatus";

        public static void MapEtcdMemberStatusEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EtcdMemberStatus");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<EtcdMemberStatusDto>>()
                .WithName("EtcdMemberStatusGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EtcdMemberStatusService.CreateAsync(dbContext, user, log,

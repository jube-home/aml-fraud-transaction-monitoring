...
{
    public static class CaseNoteByCaseKeyValueEndpoints
    {
        private const string Base = "/api/GetCaseNoteByCaseKeyValueQuery";

        public static void MapCaseNoteByCaseKeyValueEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseNoteByCaseKeyValue");

            group.MapGet("", GetAsync)
                .Produces<List<CaseNoteByCaseKeyValueDto>>()
                .WithName("CaseNoteByCaseKeyValueGet");
        }
            ...
            IServiceChangeBus serviceChangeBus, string key, string value, CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseNoteByCaseKeyValueService.CreateAsync(dbContext, user, log,

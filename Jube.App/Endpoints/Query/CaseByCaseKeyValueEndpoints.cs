...
{
    public static class CaseByCaseKeyValueEndpoints
    {
        private const string Base = "/api/GetCaseByCaseKeyValueQuery";

        public static void MapCaseByCaseKeyValueEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseByCaseKeyValue");

            group.MapGet("", GetAsync)
                .Produces<List<CaseByCaseKeyValueDto>>()
                .WithName("CaseByCaseKeyValueGet");
        }
            ...
            IServiceChangeBus serviceChangeBus, string key, string value, CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseByCaseKeyValueService.CreateAsync(dbContext, user, log,

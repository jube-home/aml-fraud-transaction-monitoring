...
{
    public static class CaseByIdEndpoints
    {
        private const string Base = "/api/GetCaseByIdQuery";

        public static void MapCaseByIdEndpoints(this IEndpointRouteBuilder endpoints)
        {
            ...
            group.MapGet("{id:int}", GetAsync)
                .Produces<CaseByIdDto>()
                .WithName("CaseByIdGet");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseByIdService.CreateAsync(dbContext, user, log,
            ...
            catch (NotFoundException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/{id}: 404 (case not found) user={user}");
                }

...
{
    public static class EntityAnalysisModelSuppressionQueryEndpoints
    {
        private const string Base = "/api/GetEntityAnalysisModelSuppressionQuery";

        public static void MapEntityAnalysisModelSuppressionQueryEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelSuppressionQuery");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelSuppressionQueryDto>>()
                .WithName("EntityAnalysisModelSuppressionQueryGet");
        }
        ...
        private static async Task<IResult> GetAsync(HttpContext httpContext, ILog log,
            ...
            IServiceChangeBus serviceChangeBus, string suppressionKey, string suppressionKeyValue,
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelSuppressionQueryService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                return TypedResults.Ok(await service.GetAsync(suppressionKey, suppressionKeyValue, token));
            }

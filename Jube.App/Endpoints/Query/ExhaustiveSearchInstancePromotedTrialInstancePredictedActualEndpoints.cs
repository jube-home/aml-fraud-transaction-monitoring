...
{
    public static class ExhaustiveSearchInstancePromotedTrialInstancePredictedActualEndpoints
    {
        private const string Base = "/api/GetExhaustiveSearchInstancePromotedTrialInstancePredictedActual";

        public static void MapExhaustiveSearchInstancePromotedTrialInstancePredictedActualEndpoints(
            this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("ExhaustiveSearchInstancePromotedTrialInstancePredictedActual");

            group.MapGet("{id:int}", GetAsync)
                .Produces<List<ExhaustiveSearchInstancePromotedTrialInstancePredictedActualDto>>()
                .WithName("ExhaustiveSearchInstancePromotedTrialInstancePredictedActualGet");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await ExhaustiveSearchInstancePromotedTrialInstancePredictedActualService.CreateAsync(
                    dbContext, user, log,

...
{
    public static class CaseWorkflowDisplayExecutionEndpoints
    {
        private const string Base = "/api/CaseWorkflowDisplayExecution";

        public static void MapCaseWorkflowDisplayExecutionEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseWorkflowDisplayExecution");

            group.MapPost("", ExecuteAsync)
                .Produces<string>()
                .WithName("CaseWorkflowDisplayExecutionExecute");
        }

        private static async Task<IResult> ExecuteAsync([FromBody] CaseWorkflowDisplayExecutionDto model,
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseWorkflowDisplayExecutionService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, jsonSerializationHelper, token);
            ...
            catch (NotFoundException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"POST {Base}: 400 (case, display or status not found) user={user}");
                }

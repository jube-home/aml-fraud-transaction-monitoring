...
{
    public static class CaseEventByCaseKeyValueEndpoints
    {
        private const string Base = "/api/GetCaseEventByCaseKeyValueQuery";

        public static void MapCaseEventByCaseKeyValueEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("CaseEventByCaseKeyValue");

            group.MapGet("", GetAsync)
                .Produces<List<CaseEventByCaseKeyValueDto>>()
                .WithName("CaseEventByCaseKeyValueGet");
        }
            ...
            IServiceChangeBus serviceChangeBus, string key, string value, CancellationToken token)
        {
            ...
            try
            {
                var service = await CaseEventByCaseKeyValueService.CreateAsync(dbContext, user, log,

...
{
    public static class PostgresLogEntryEndpoints
    {
        private const string Base = "/api/PostgresLogEntry";

        public static void MapPostgresLogEntryEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("PostgresLogEntry");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<PostgresLogEntryDto>>()
                .WithName("PostgresLogEntryGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await PostgresLogEntryService.CreateAsync(dbContext, user, log,

...
{
    public static class DockerEventEndpoints
    {
        private const string Base = "/api/DockerEvent";

        public static void MapDockerEventEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("DockerEvent");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<DockerEventDto>>()
                .WithName("DockerEventGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await DockerEventService.CreateAsync(dbContext, user, log,

...
{
    public static class EntityAnalysisModelInlineScriptEndpoints
    {
        private const string Base = "/api/EntityAnalysisModelInlineScript";

        public static void MapEntityAnalysisModelInlineScriptEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelInlineScript");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelInlineScriptDto>>()
                .WithName("EntityAnalysisModelInlineScriptGetAll");

            group.MapGet("ByEntityAnalysisModelId/{entityAnalysisModelId:int}", GetByEntityAnalysisModelIdAsync)
                .Produces<List<EntityAnalysisModelInlineScriptDto>>()
                .WithName("EntityAnalysisModelInlineScriptGetByEntityAnalysisModelId");

            group.MapGet("{entityAnalysisModelInlineScriptId:int}", GetByIdAsync)
                .Produces<EntityAnalysisModelInlineScriptDto>()
                .WithName("EntityAnalysisModelInlineScriptGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelInlineScriptCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelInlineScriptUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelInlineScriptDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelInlineScriptService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> GetByIdAsync(
            int entityAnalysisModelInlineScriptId, HttpContext httpContext, ILog log,
            DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {
            ...
            if (log.IsDebugEnabled)
            {
                log.Debug($"GET {Base}/{entityAnalysisModelInlineScriptId}: entry user={user}");
            }
            ...
            try
            {
                var service = await EntityAnalysisModelInlineScriptService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                var result = await service.GetByIdAsync(entityAnalysisModelInlineScriptId, token);
                return TypedResults.Ok(result);
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/{entityAnalysisModelInlineScriptId}: 403 (not authenticated) user={user}");
                }
            ...
            catch (ForbiddenException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"GET {Base}/{entityAnalysisModelInlineScriptId}: 403 user={user}");
                }
            ...
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
                {
                    log.Debug($"GET {Base}/{entityAnalysisModelInlineScriptId}: client cancelled user={user}");
                }
            ...
            {
                log.Error($"GET {Base}/{entityAnalysisModelInlineScriptId}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }

        private static async Task<IResult> CreateAsync(
            [FromBody] EntityAnalysisModelInlineScriptDto model, HttpContext httpContext, ILog log,

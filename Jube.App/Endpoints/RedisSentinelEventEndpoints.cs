...
{
    public static class RedisSentinelEventEndpoints
    {
        private const string Base = "/api/RedisSentinelEvent";

        public static void MapRedisSentinelEventEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("RedisSentinelEvent");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<RedisSentinelEventDto>>()
                .WithName("RedisSentinelEventGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await RedisSentinelEventService.CreateAsync(dbContext, user, log,

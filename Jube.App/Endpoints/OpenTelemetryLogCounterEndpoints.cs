...
{
    public static class OpenTelemetryLogCounterEndpoints
    {
        private const string Base = "/api/OpenTelemetryLogCounter";

        public static void MapOpenTelemetryLogCounterEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("OpenTelemetryLogCounter");

            group.MapGet("", GetAsync)
                .Produces<List<OpenTelemetryLogCounterDto>>()
                .WithName("OpenTelemetryLogCounterGetAll");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<OpenTelemetryLogCounterDto>()
                .WithName("OpenTelemetryLogCounterGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("OpenTelemetryLogCounterCreate");
            ...
            group.MapPut("", UpdateAsync)
                ...
                .WithName("OpenTelemetryLogCounterUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .Produces((int)HttpStatusCode.Conflict)
                .WithName("OpenTelemetryLogCounterDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await OpenTelemetryLogCounterService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] OpenTelemetryLogCounterDto model, HttpContext httpContext, ILog log,
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            catch (LockedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"PUT {Base}: 409 (locked) user={user}");
                }

                return TypedResults.StatusCode((int)HttpStatusCode.Conflict);
            }
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled)
            ...
            CancellationToken token)
        {
            ...
            catch (LockedException)
            {
                if (log.IsWarnEnabled)
                {
                    log.Warn($"DELETE {Base}/{id}: 409 (locked) user={user}");
                }

...
{
    public static class EntityAnalysisModelActivationRuleSuppressionEndpoints
    {
        private const string Base = "/api/EntityAnalysisModelActivationRuleSuppression";

        public static void MapEntityAnalysisModelActivationRuleSuppressionEndpoints(
            this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelActivationRuleSuppression");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelActivationRuleSuppressionDto>>()
                .WithName("EntityAnalysisModelActivationRuleSuppressionGetAll");

            group.MapGet("ByEntityAnalysisModelGuid/{entityAnalysisModelGuid:guid}",
                    GetByEntityAnalysisModelGuidAsync)
                .Produces<List<EntityAnalysisModelActivationRuleSuppressionDto>>()
                .WithName("EntityAnalysisModelActivationRuleSuppressionGetByEntityAnalysisModelGuid");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<EntityAnalysisModelActivationRuleSuppressionDto>()
                .WithName("EntityAnalysisModelActivationRuleSuppressionGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelActivationRuleSuppressionCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelActivationRuleSuppressionUpdate");

            group.MapPut("DeleteExpiryDate", UpdateDeleteExpiryDateAsync)
                .Produces<EntityAnalysisModelActivationRuleSuppressionDto>()
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelActivationRuleSuppressionUpdateDeleteExpiryDate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelActivationRuleSuppressionDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelActivationRuleSuppressionService.CreateAsync(dbContext, user,
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] EntityAnalysisModelActivationRuleSuppressionDto model, HttpContext httpContext, ILog log,
        ...
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled) log.Debug($"PUT {Base}/DeleteExpiryDate: entry user={user}");
            ...
            {
                if (log.IsWarnEnabled) log.Warn($"PUT {Base}/DeleteExpiryDate: 403 user={user}");
            ...
            {
                if (log.IsWarnEnabled) log.Warn($"PUT {Base}/DeleteExpiryDate: 204 (not found) user={user}");

                return TypedResults.StatusCode((int)HttpStatusCode.NoContent);
            }
            catch (OperationCanceledException)
            {
                if (log.IsDebugEnabled) log.Debug($"PUT {Base}/DeleteExpiryDate: client cancelled user={user}");

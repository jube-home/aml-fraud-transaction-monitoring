...
{
    public static class RedisMetricEndpoints
    {
        private const string Base = "/api/RedisMetric";

        public static void MapRedisMetricEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("RedisMetric");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<RedisMetricDto>>()
                .WithName("RedisMetricGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await RedisMetricService.CreateAsync(dbContext, user, log,

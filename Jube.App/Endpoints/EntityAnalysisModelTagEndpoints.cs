...
{
    public static class EntityAnalysisModelTagEndpoints
    {
        private const string Base = "/api/EntityAnalysisModelTag";

        public static void MapEntityAnalysisModelTagEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelTag");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelTagDto>>()
                .WithName("EntityAnalysisModelTagGetAll");

            group.MapGet("ByEntityAnalysisModelId/{entityAnalysisModelId:int}", GetByEntityAnalysisModelIdAsync)
                .Produces<List<EntityAnalysisModelTagDto>>()
                .WithName("EntityAnalysisModelTagGetByEntityAnalysisModelId");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<EntityAnalysisModelTagDto>()
                .WithName("EntityAnalysisModelTagGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelTagCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelTagUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelTagDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelTagService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] EntityAnalysisModelTagDto model, HttpContext httpContext, ILog log,

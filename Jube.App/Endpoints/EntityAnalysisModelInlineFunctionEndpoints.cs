...
{
    public static class EntityAnalysisModelInlineFunctionEndpoints
    {
        private const string Base = "/api/EntityAnalysisModelInlineFunction";

        public static void MapEntityAnalysisModelInlineFunctionEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelInlineFunction");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelInlineFunctionDto>>()
                .WithName("EntityAnalysisModelInlineFunctionGetAll");

            group.MapGet("ByEntityAnalysisModelId/{entityAnalysisModelId:int}", GetByEntityAnalysisModelIdAsync)
                .Produces<List<EntityAnalysisModelInlineFunctionDto>>()
                .WithName("EntityAnalysisModelInlineFunctionGetByEntityAnalysisModelId");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<EntityAnalysisModelInlineFunctionDto>()
                .WithName("EntityAnalysisModelInlineFunctionGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelInlineFunctionCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelInlineFunctionUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelInlineFunctionDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelInlineFunctionService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] EntityAnalysisModelInlineFunctionDto model, HttpContext httpContext, ILog log,

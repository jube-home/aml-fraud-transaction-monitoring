...
{
    public static class PostgresMetricEndpoints
    {
        private const string Base = "/api/PostgresMetric";

        public static void MapPostgresMetricEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("PostgresMetric");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<PostgresMetricDto>>()
                .WithName("PostgresMetricGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await PostgresMetricService.CreateAsync(dbContext, user, log,

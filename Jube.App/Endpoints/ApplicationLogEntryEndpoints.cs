...
{
    public static class ApplicationLogEntryEndpoints
    {
        private const string Base = "/api/ApplicationLogEntry";

        public static void MapApplicationLogEntryEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("ApplicationLogEntry");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<ApplicationLogEntryDto>>()
                .WithName("ApplicationLogEntryGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await ApplicationLogEntryService.CreateAsync(dbContext, user, log,

...
{
    public static class HaProxyReachabilityProbeEndpoints
    {
        private const string Base = "/api/HAProxyReachabilityProbe";

        public static void MapHaProxyReachabilityProbeEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("HAProxyReachabilityProbe");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<HaProxyReachabilityProbeDto>>()
                .WithName("HAProxyReachabilityProbeGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await HaProxyReachabilityProbeService.CreateAsync(dbContext, user, log,

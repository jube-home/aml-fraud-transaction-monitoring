...
{
    public static class EntityAnalysisModelTtlCounterEndpoints
    {
        private const string Base = "/api/EntityAnalysisModelTtlCounter";

        public static void MapEntityAnalysisModelTtlCounterEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelTtlCounter");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelTtlCounterDto>>()
                .WithName("EntityAnalysisModelTtlCounterGetAll");

            group.MapGet("ByEntityAnalysisModelId/{entityAnalysisModelId:int}", GetByEntityAnalysisModelIdAsync)
                .Produces<List<EntityAnalysisModelTtlCounterDto>>()
                .WithName("EntityAnalysisModelTtlCounterGetByEntityAnalysisModelId");

            group.MapGet("ByEntityAnalysisModelGuid/{guid:guid}", GetByEntityAnalysisModelGuidAsync)
                .Produces<List<EntityAnalysisModelTtlCounterDto>>()
                .WithName("EntityAnalysisModelTtlCounterGetByEntityAnalysisModelGuid");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<EntityAnalysisModelTtlCounterDto>()
                .WithName("EntityAnalysisModelTtlCounterGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelTtlCounterCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelTtlCounterUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelTtlCounterDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelTtlCounterService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> GetByEntityAnalysisModelGuidAsync(
            Guid guid, HttpContext httpContext, ILog log, DynamicEnvironment.DynamicEnvironment dynamicEnvironment,
            IStringLocalizerFactory stringLocalizerFactory, IServiceChangeBus serviceChangeBus,
            CancellationToken token)
        {
            var user = httpContext.User.Identity?.Name;
            if (log.IsDebugEnabled)
                log.Debug($"GET {Base}/ByEntityAnalysisModelGuid/{guid}: entry user={user}");
            ...
            try
            {
                var service = await EntityAnalysisModelTtlCounterService.CreateAsync(dbContext, user, log,
                    stringLocalizerFactory, serviceChangeBus, token);
                var result = await service.GetByEntityAnalysisModelGuidAsync(guid, token);
                return TypedResults.Ok(result);
            }
            catch (NotAuthenticatedException)
            {
                if (log.IsWarnEnabled)
                    log.Warn($"GET {Base}/ByEntityAnalysisModelGuid/{guid}: 403 (not authenticated) user={user}");
            ...
            {
                if (log.IsWarnEnabled)
                    log.Warn($"GET {Base}/ByEntityAnalysisModelGuid/{guid}: 403 user={user}");
            ...
            {
                if (log.IsDebugEnabled)
                    log.Debug($"GET {Base}/ByEntityAnalysisModelGuid/{guid}: client cancelled user={user}");

                throw;
            }
            catch (Exception e)
            {
                log.Error($"GET {Base}/ByEntityAnalysisModelGuid/{guid}: 500 user={user}", e);
                return TypedResults.StatusCode((int)HttpStatusCode.InternalServerError);
            }
        }
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] EntityAnalysisModelTtlCounterDto model, HttpContext httpContext, ILog log,

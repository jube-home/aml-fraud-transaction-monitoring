...
{
    public static class PostgresReplicationStatusEndpoints
    {
        private const string Base = "/api/PostgresReplicationStatus";

        public static void MapPostgresReplicationStatusEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("PostgresReplicationStatus");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<PostgresReplicationStatusDto>>()
                .WithName("PostgresReplicationStatusGetAll");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await PostgresReplicationStatusService.CreateAsync(dbContext, user, log,

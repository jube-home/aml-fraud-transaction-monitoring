...
{
    public static class PostgresActivityEndpoints
    {
        private const string Base = "/api/PostgresActivity";

        public static void MapPostgresActivityEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("PostgresActivity");

            group.MapGet("", GetAsync)
                .Produces<PayloadResult<PostgresActivityDto>>()
                .WithName("PostgresActivityGetAll");
        }
            ...
            IServiceChangeBus serviceChangeBus, CancellationToken token)
        {
            ...
            try
            {
                var service = await PostgresActivityService.CreateAsync(dbContext, connectionString, user, log,

...
{
    public static class EntityAnalysisModelReprocessingRuleEndpoints
    {
        private const string Base = "/api/EntityAnalysisModelReprocessingRule";

        public static void MapEntityAnalysisModelReprocessingRuleEndpoints(this IEndpointRouteBuilder endpoints)
        {
            var group = endpoints.MapGroup(Base)
                .RequireAuthorization()
                .WithTags("EntityAnalysisModelReprocessingRule");

            group.MapGet("", GetAsync)
                .Produces<List<EntityAnalysisModelReprocessingRuleDto>>()
                .WithName("EntityAnalysisModelReprocessingRuleGetAll");

            group.MapGet("ByEntityAnalysisModelId/{entityAnalysisModelId:int}", GetByEntityAnalysisModelIdAsync)
                .Produces<List<EntityAnalysisModelReprocessingRuleDto>>()
                .WithName("EntityAnalysisModelReprocessingRuleGetByEntityAnalysisModelId");

            group.MapGet("{id:int}", GetByIdAsync)
                .Produces<EntityAnalysisModelReprocessingRuleDto>()
                .WithName("EntityAnalysisModelReprocessingRuleGetById");

            group.MapPost("", CreateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .WithName("EntityAnalysisModelReprocessingRuleCreate");

            group.MapPut("", UpdateAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces<ValidationResult>((int)HttpStatusCode.BadRequest)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelReprocessingRuleUpdate");

            group.MapDelete("{id:int}", DeleteAsync)
                .Produces((int)HttpStatusCode.OK)
                .Produces((int)HttpStatusCode.NoContent)
                .WithName("EntityAnalysisModelReprocessingRuleDelete");
        }
            ...
            CancellationToken token)
        {
            ...
            try
            {
                var service = await EntityAnalysisModelReprocessingRuleService.CreateAsync(dbContext, user, log,
        ...
        private static async Task<IResult> CreateAsync(
            [FromBody] EntityAnalysisModelReprocessingRuleDto model, HttpContext httpContext, ILog log,

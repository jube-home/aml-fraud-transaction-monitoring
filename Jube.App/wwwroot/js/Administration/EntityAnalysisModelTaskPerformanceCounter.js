...
let $grid, $fromDate, $toDate, $directionFilter, $taskTypeFilter, $applyFilters, $clearFilters, $exportCsv;

const dataSourceEntity = new kendo.data.DataSource({
    transport: {
        read: {
            url: "/api/EntityAnalysisModelTaskPerformanceCounter",
            type: "GET",
            dataType: "json",
            data: function () {
                ...
                return {
                    ...
                    directionId: $directionFilter.val() || null,
                    taskTypeId: $taskTypeFilter.val() || null,
    ...
    schema: {
        ...
        model: {
            ...
            fields: {
                ...
                directionName: {type: "string", editable: false},
                taskName: {type: "string", editable: false},
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Direction <select id="directionFilter">' +
            '<option value="">All</option>' +
            '<option value="1">Read</option>' +
            '<option value="2">Write</option>' +
            '</select></label>' +
            '<label style="display:flex;align-items:center;gap:4px;">Task <select id="taskTypeFilter">' +
            '<option value="">All</option>' +
            '<option value="1">SanctionsAsync</option>' +
            '<option value="2">TtlCountersAsync</option>' +
            '<option value="3">AbstractionRulesWithSearchKeysAsync</option>' +
            '<option value="4">CachePayloadLatestUpsertAsync</option>' +
            '<option value="5">CachePayloadUpsertAsync</option>' +
            '<option value="6">CachePayloadInsertAsync</option>' +
            '<option value="7">CacheTtlCounterEntryUpsertAsync</option>' +
            '<option value="8">CacheTtlCounterEntryIncrementAsync</option>' +
            '<option value="10">CacheSanctionInsertAsync</option>' +
            '<option value="28">UpsertReferenceDateAsync</option>' +
            '</select></label>' +
        ...
        columns: [
            ...
            {field: "directionName", title: "Direction"},
    ...
    $directionFilter.kendoDropDownList();
    $taskTypeFilter.kendoDropDownList();
    ...
    $clearFilters.on("click", function () {
        $fromDate.data("kendoDateTimePicker").value(defaultFromDate());
        $toDate.data("kendoDateTimePicker").value(defaultToDate());
        $directionFilter.data("kendoDropDownList").value("");
        $taskTypeFilter.data("kendoDropDownList").value("");
        dataSourceEntity.read();
    });

    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/EntityAnalysisModelTaskPerformanceCounter", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            directionId: $directionFilter.val() || null,
            taskTypeId: $taskTypeFilter.val() || null
        }, "EntityAnalysisModelTaskPerformanceCounter.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=EntityAnalysisModelTaskPerformanceCounter.js

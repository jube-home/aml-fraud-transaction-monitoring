...
const dataSourceEntity = new kendo.data.DataSource({
    ...
    schema: {
        ...
        model: {
            id: "id",
            fields: {
                pxName: {type: "string", editable: false},
                svName: {type: "string", editable: false},
                status: {type: "string", editable: false},
                addr: {type: "string", editable: false},
                checkStatus: {type: "string", editable: false},
                checkCode: {type: "number", editable: false},
                chkFail: {type: "number", editable: false},
                chkDown: {type: "number", editable: false},
                lastChg: {type: "number", editable: false},
                scur: {type: "number", editable: false},
                qcur: {type: "number", editable: false},
                weight: {type: "number", editable: false},
                act: {type: "number", editable: false},
                bck: {type: "number", editable: false},
                hrsp2Xx: {type: "number", editable: false},
                hrsp5Xx: {type: "number", editable: false},
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Search <input type="text" id="searchBox" placeholder="Proxy, server or status..." /></label>' +
        ...
        columns: [
            ...
            {field: "pxName", title: "Proxy", width: 140},
            {field: "svName", title: "Server", width: 140},
            {field: "status", title: "Status", width: 100},
            {field: "addr", title: "Address", width: 160},
            {field: "checkStatus", title: "Check Status", width: 130},
            {field: "checkCode", title: "Check Code", width: 110},
            {field: "chkFail", title: "Check Failures", width: 130},
            {field: "chkDown", title: "Down Transitions", width: 150},
            {field: "lastChg", title: "Seconds Since Change", width: 170},
            {field: "scur", title: "Current Sessions", width: 150},
            {field: "qcur", title: "Current Queued", width: 140},
            {field: "weight", title: "Weight", width: 90},
            {field: "act", title: "Active", width: 90},
            {field: "bck", title: "Backup", width: 90},
            {field: "hrsp2Xx", title: "HTTP 2xx", width: 110},
            {field: "hrsp5Xx", title: "HTTP 5xx", width: 110},
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/HAProxyServerStatus", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            search: $searchBox.val() || null
        }, "HAProxyServerStatus.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=HaProxyServerStatus.js

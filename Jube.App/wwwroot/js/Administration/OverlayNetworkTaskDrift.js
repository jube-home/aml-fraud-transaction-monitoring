...
const dataSourceEntity = new kendo.data.DataSource({
    transport: {
        read: {
            url: "/api/OverlayNetworkTaskDrift",
            type: "GET",
            dataType: "json",
            data: function () {
    ...
    schema: {
        ...
        model: {
            ...
            fields: {
                ...
                dnsResolvedAddresses: {type: "string", editable: false},
                swarmTaskAddresses: {type: "string", editable: false},
                addressesOnlyInDns: {type: "string", editable: false},
                addressesOnlyInSwarm: {type: "string", editable: false},
                isConsistent: {type: "boolean", editable: false},
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Search <input type="text" id="searchBox" placeholder="Service name..." /></label>' +
        ...
        columns: [
            ...
            {field: "serviceName", title: "Service", width: 140},
            {field: "isConsistent", title: "Consistent", width: 110},
            {field: "dnsResolvedAddresses", title: "DNS Addresses", width: 220},
            {field: "swarmTaskAddresses", title: "Swarm Task Addresses", width: 220},
            {field: "addressesOnlyInDns", title: "Only In DNS", width: 200},
            {field: "addressesOnlyInSwarm", title: "Only In Swarm", width: 200}
        ]
    });
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/OverlayNetworkTaskDrift", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            search: $searchBox.val() || null
        }, "OverlayNetworkTaskDrift.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=OverlayNetworkTaskDrift.js

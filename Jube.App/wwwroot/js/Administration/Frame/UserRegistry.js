...
var wirePasswordHash = $("#WirePasswordHash").kendoSwitch().data("kendoSwitch");
...
function getPasswordAndShow() {
    $.ajax({
        ...
        data: JSON.stringify({id: id, wirePasswordHash: wirePasswordHash.value()}),
...
function getData() {
    return {
        name: $("#Name").val(),
        email: $("#Email").val(),
        wirePasswordHash: wirePasswordHash.value(),
...
function commitAdd() {
    ...
    if (!name) {
        $inpName.css("border-color", "#c0392b").focus();
        return;
    }

    const $inputs = $("#inp-name, #inp-desc");

...
let $grid, $fromDate, $toDate, $applyFilters, $clearFilters, $exportCsv;
    ...
    schema: {
        ...
        model: {
            fields: {
                ...
                model: {type: "number", editable: false},
                asynchronousModel: {type: "number", editable: false},
                tag: {type: "number", editable: false},
                error: {type: "number", editable: false},
                sanction: {type: "number", editable: false},
                exhaustive: {type: "number", editable: false},
                all: {type: "number", editable: false}
            }
        }
    },
    pageSize: 100
...
$(document).ready(function () {
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/HttpProcessingCounter", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null
        }, "HttpProcessingCounter.csv");
    });

    ...
    schema: {
        ...
        model: {
            ...
            fields: {
                ...
                loggerName: {type: "string", editable: false},
                threadContext: {type: "string", editable: false},
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Search <input type="text" id="searchBox" placeholder="Message, class or exception..." /></label>' +
        ...
        columns: [
            ...
            {field: "loggerName", title: "Class", width: 260},
            {field: "threadContext", title: "Thread", width: 90},
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/ApplicationLogEntry", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            search: $searchBox.val() || null
        }, "ApplicationLogEntry.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=ApplicationLogEntry.js

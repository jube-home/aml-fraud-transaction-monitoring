...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<option value="1">BuildArchiveJson</option>' +
            '<option value="2">CaseCreationDispatch</option>' +
            '<option value="3">RdbmsArchiveWrite</option>' +
            '<option value="4">BulkCopyArchiveBuffer</option>' +
            '</select></label>' +
        ...
        columns: [
            ...
            {field: "entityAnalysisModelName", title: "Entity Analysis Model", width: 200},
            {field: "stageName", title: "Stage", width: 180},
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/ArchiverWarning", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            stageId: $stageFilter.val() || null
        }, "ArchiverWarning.csv");

...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Search <input type="text" id="searchBox" placeholder="Message or level..." /></label>' +
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/PostgresLogEntry", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            search: $searchBox.val() || null
        }, "PostgresLogEntry.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=PostgresLogEntry.js

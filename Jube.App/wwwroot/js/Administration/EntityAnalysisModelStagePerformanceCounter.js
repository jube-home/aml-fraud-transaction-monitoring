...
const dataSourceEntity = new kendo.data.DataSource({
    transport: {
        read: {
            url: "/api/EntityAnalysisModelStagePerformanceCounter",
            type: "GET",
            dataType: "json",
            data: function () {
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<option value="7">Sanctions</option>' +
            '<option value="8">TtlCounters</option>' +
            '<option value="9">AbstractionRulesWithSearchKeys</option>' +
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/EntityAnalysisModelStagePerformanceCounter", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            stageId: $stageFilter.val() || null
        }, "EntityAnalysisModelStagePerformanceCounter.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=EntityAnalysisModelStagePerformanceCounter.js

    ...
    schema: {
        ...
        model: {
            ...
            fields: {
                ...
                minResponseTimeMicroseconds: {type: "number", editable: false},
                maxResponseTimeMicroseconds: {type: "number", editable: false},
                archiveWalPendingCount: {type: "number", editable: false}
            }
        }
    },
    pageSize: 100
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Search <input type="text" id="searchBox" placeholder="Model name..." /></label>' +
        ...
        columns: [
            ...
            {field: "minResponseTimeMicroseconds", title: "Min Response Time Microseconds"},
            {field: "maxResponseTimeMicroseconds", title: "Max Response Time Microseconds"},
            {field: "archiveWalPendingCount", title: "Archive WAL Pending Count"}
        ]
    });
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/EntityAnalysisModelProcessingCounter", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            search: $searchBox.val() || null
        }, "EntityAnalysisModelProcessingCounter.csv");
    });

...
let $grid, $fromDate, $toDate, $streamTypeFilter, $searchBox, $applyFilters, $clearFilters, $exportCsv;

const dataSourceEntity = new kendo.data.DataSource({
    transport: {
        read: {
            url: "/api/ContainerLogEntry",
            type: "GET",
            dataType: "json",
            data: function () {
                ...
                return {
                    ...
                    streamTypeId: $streamTypeFilter.val() || null,
    ...
    schema: {
        ...
        model: {
            ...
            fields: {
                ...
                containerName: {type: "string", editable: false},
                streamTypeName: {type: "string", editable: false},
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Stream <select id="streamTypeFilter">' +
            '<option value="">All</option>' +
            '<option value="1">Stdout</option>' +
            '<option value="2">Stderr</option>' +
            '</select></label>' +
            '<label style="display:flex;align-items:center;gap:4px;">Search <input type="text" id="searchBox" placeholder="Container or message..." /></label>' +
        ...
        columns: [
            ...
            {field: "containerName", title: "Container", width: 200},
            {field: "streamTypeName", title: "Stream", width: 90},
            {field: "message", title: "Message"},
            {field: "instance", title: "Host Instance", width: 160},
    ...
    $streamTypeFilter.kendoDropDownList();
    ...
    $clearFilters.on("click", function () {
        $fromDate.data("kendoDateTimePicker").value(defaultFromDate());
        $toDate.data("kendoDateTimePicker").value(defaultToDate());
        $streamTypeFilter.data("kendoDropDownList").value("");
        $searchBox.val("");
        dataSourceEntity.read();
    });
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/ContainerLogEntry", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            streamTypeId: $streamTypeFilter.val() || null,
            search: $searchBox.val() || null
        }, "ContainerLogEntry.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=ContainerLogEntry.js

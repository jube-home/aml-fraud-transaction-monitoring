...
const dataSourceEntity = new kendo.data.DataSource({
    transport: {
        read: {
            url: "/api/HAProxyReachabilityProbe",
            type: "GET",
            dataType: "json",
            data: function () {
    ...
    schema: {
        ...
        model: {
            ...
            fields: {
                ...
                haProxyAddress: {type: "string", editable: false},
                success: {type: "boolean", editable: false},
                connectMicroseconds: {type: "number", editable: false},
                httpStatusCode: {type: "number", editable: false},
...
$(document).ready(function () {
    ...
    $grid.kendoGrid({
        ...
        toolbar: kendo.template(
            ...
            '<label style="display:flex;align-items:center;gap:4px;">Search <input type="text" id="searchBox" placeholder="Target or HAProxy address..." /></label>' +
        ...
        columns: [
            ...
            {field: "haProxyAddress", title: "HAProxy Address", width: 180},
            {field: "success", title: "Success", width: 90},
            {field: "connectMicroseconds", title: "Connect Microseconds", width: 170},
            {field: "httpStatusCode", title: "HTTP Status", width: 110},
            {field: "errorMessage", title: "Error Message", width: 260}
        ]
    });
    ...
    $exportCsv.on("click", function () {
        exportGridToCsv($grid.data("kendoGrid"), "/api/HAProxyReachabilityProbe", {
            from: $fromDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            to: $toDate.data("kendoDateTimePicker").value()?.toISOString() ?? null,
            search: $searchBox.val() || null
        }, "HAProxyReachabilityProbe.csv");
    });

    dataSourceEntity.read();
});

//# sourceURL=HaProxyReachabilityProbe.js

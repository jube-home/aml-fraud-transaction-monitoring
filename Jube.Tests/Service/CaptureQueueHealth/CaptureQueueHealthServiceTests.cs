...
using Jube.Service.CaptureQueueHealth;
...
namespace Jube.Test.Service.CaptureQueueHealth
{
    using CaptureQueueHealthService = CaptureQueueHealthService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaptureQueueHealthServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.CaptureQueueHealth.Where(w => w.Id == id).DeleteAsync();
            }
        }

        private static Task<CaptureQueueHealthService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaptureQueueHealthService.CreateAsync(
                dbContext, userName, log ?? TestLog.NoOp, localizers, serviceChangeBus ?? new NullServiceChangeBus(),
                auditLog ?? TestLog.NoOp);
        }

        private async Task<int> CreateRowAsync(DbContext dbContext, CaptureQueueType queue, int queueDepth = 0,
            long droppedCount = 0, DateTime? createdDate = null)
        {
            var id = await dbContext.InsertWithInt32IdentityAsync(new Data.Poco.CaptureQueueHealth
            {
                QueueId = (int)queue,
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var firstId = await CreateRowAsync(dbContext, CaptureQueueType.ModelInvokeWarning, 1, 0,
            ...
            mine[0].QueueName.Should().Be("Archiver Warning");
            mine[0].QueueDepth.Should().Be(2);
            mine[0].DroppedCount.Should().Be(5);
        ...
        public async Task ListDefaultsToLastHourWhenFromAndToOmittedAsync()
        {
            ...
            var oldId = await CreateRowAsync(dbContext, CaptureQueueType.ModelInvokeWarning,
        ...
        [Fact]
        public async Task ListFiltersByQueueIdAsync()
        {
            await using var dbContext = fx.GetDbContext();
            ...
            mine.Should().ContainSingle();
            mine[0].Id.Should().Be(modelInvokeId);
        }
        ...
        [Fact]
        public async Task ListSortsByQueueDepthAscendingAndDescendingAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var id1 = await CreateRowAsync(dbContext, CaptureQueueType.ModelInvokeWarning, 30);
            var id2 = await CreateRowAsync(dbContext, CaptureQueueType.ModelInvokeWarning, 10);
            ...
            ascending.Rows.Where(r => ids.Contains(r.Id)).Select(r => r.QueueDepth).Should().Equal(10, 20, 30);

            var descending = await service.ListAsync(queueId: (int)CaptureQueueType.ModelInvokeWarning,
                sortField: "queueDepth", sortDirection: "desc");
            descending.Rows.Where(r => ids.Contains(r.Id)).Select(r => r.QueueDepth).Should().Equal(30, 20, 10);
        }
        ...
        public async Task ListSortDirectionIsCaseInsensitiveForAscendingAsync(string ascendingKeyword)
        {
            ...
            result.Rows.Where(r => ids.Contains(r.Id)).Select(r => r.QueueDepth).Should().Equal(10, 20);
        }
        ...
        public async Task ListWithGarbageSortDirectionFallsBackToDescendingAsync()
        {
            ...
            result.Rows.Where(r => ids.Contains(r.Id)).Select(r => r.QueueDepth).Should().Equal(20, 10);
        }
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var firstId = await CreateRowAsync(dbContext, CaptureQueueType.ModelInvokeWarning,

...
using Jube.Service.EntityAnalysisModelTtlCounter;
...
namespace Jube.Test.Service.EntityAnalysisModelTtlCounter
{
    using EntityAnalysisModelTtlCounterService = EntityAnalysisModelTtlCounterService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelTtlCounterServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            foreach (var id in createdIds)
            {
                await dbContext.GetTable<EntityAnalysisModelTtlCounterVersion>()
                    .Where(w => w.EntityAnalysisModelTtlCounterId == id).DeleteAsync();
                await dbContext.GetTable<Data.Poco.EntityAnalysisModelTtlCounter>().Where(w => w.Id == id)
                    .DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelTtlCounterService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelTtlCounterService.CreateAsync(
        ...
        private static EntityAnalysisModelTtlCounterDto NewDto(int entityAnalysisModelId, string name)
        {
            return new EntityAnalysisModelTtlCounterDto
            {
        ...
        public async Task GetByEntityAnalysisModelGuidReturnsOnlyRowsForThatModelAsync()
        {
            ...
            var modelAId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var modelBId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var modelAGuid = await dbContext.EntityAnalysisModel.Where(w => w.Id == modelAId)
                .Select(s => s.Guid).SingleAsync();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var savedA = await service.InsertAsync(NewDto(modelAId, UniqueName("ByGuidA")));
            createdIds.Add(savedA.Id);
            var savedB = await service.InsertAsync(NewDto(modelBId, UniqueName("ByGuidB")));
            createdIds.Add(savedB.Id);
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            dto.Id = saved.Id;
            dto.TtlCounterValue = 5;

            var updated = await service.UpdateAsync(dto);

            updated.Version.Should().Be(2);
            updated.TtlCounterValue.Should().Be(5);
            updated.Guid.Should().Be(saved.Guid);

            var auditRows = await dbContext.GetTable<EntityAnalysisModelTtlCounterVersion>()
                .Where(w => w.EntityAnalysisModelTtlCounterId == saved.Id).CountAsync();
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelTtlCounter>()
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelTtlCounter>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelTtlCounterService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelTtlCounterDto.Name) &&
        ...
        [Fact]
        public async Task TtlCounterDataNameRequiredRejectsEmptyAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoDataName"));
            dto.TtlCounterDataName = "";

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "TtlCounterDataNameNotEmpty");
        }

        [Fact]
        public async Task InvalidTtlCounterIntervalIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadInterval"));
            dto.TtlCounterInterval = "x";

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "TtlCounterIntervalInvalid");
        }

        [Fact]
        public async Task TtlCounterValueNegativeIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadValue"));
            dto.TtlCounterValue = -1;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "TtlCounterValueRange");
        }

        [Fact]
        public async Task InvalidResolutionIntervalIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadResolution"));
            dto.ResolutionInterval = "x";

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "ResolutionIntervalInvalid");
        }

        [Fact]
        public async Task TtlCounterDataValueRequiredWhenSumEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoSumValue"));
            dto.EnableSum = true;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "TtlCounterDataValueNotEmpty");
        }

        [Fact]
        public async Task TtlCounterDataValueNotRequiredWhenSumDisabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("SumOff"));
            dto.EnableSum = false;
        ...
        [Fact]
        public async Task SumEnabledWithDataValueSucceedsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("SumOn"));
            dto.EnableSum = true;
            dto.TtlCounterDataValue = "Amount";

            var saved = await service.InsertAsync(dto);
            createdIds.Add(saved.Id);

            var fetched = await service.GetByIdAsync(saved.Id);
            fetched!.EnableSum.Should().BeTrue();
            fetched.TtlCounterDataValue.Should().Be("Amount");
        }
        ...
        [Fact]
        public async Task UpdatedUserAndUpdatedDateAreAlwaysNullNoBackingColumnAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var saved = await service.InsertAsync(NewDto(modelId, UniqueName("NoUpdatedColumn")));
            createdIds.Add(saved.Id);
            ...
            var fetched = await service.GetByIdAsync(saved.Id);
            fetched!.UpdatedUser.Should().BeNull(
                "EntityAnalysisModelTtlCounter has no UpdatedUser/UpdatedDate columns -- pre-existing quirk carried " +
                "through verbatim");
            fetched.UpdatedDate.Should().BeNull();
        }
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelTtlCounter.Create").Subject;
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelTtlCounterList", "EntityAnalysisModelTtlCounterGet",
                "EntityAnalysisModelTtlCounterGetByEntityAnalysisModelId",
                "EntityAnalysisModelTtlCounterGetByEntityAnalysisModelGuid",
                "EntityAnalysisModelTtlCounterCreate", "EntityAnalysisModelTtlCounterUpdate",
                "EntityAnalysisModelTtlCounterDelete"
            ]);
        }

...
using Jube.Service.RedisConnectionMultiplexerMetric;
...
namespace Jube.Test.Service.RedisConnectionMultiplexerMetric
{
    using RedisConnectionMultiplexerMetricService =
        RedisConnectionMultiplexerMetricService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class RedisConnectionMultiplexerMetricServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.RedisConnectionMultiplexerMetric.Where(w => w.Id == id).DeleteAsync();
            }
        }

        private static Task<RedisConnectionMultiplexerMetricService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return RedisConnectionMultiplexerMetricService.CreateAsync(
                dbContext, userName, log ?? TestLog.NoOp, localizers, serviceChangeBus ?? new NullServiceChangeBus(),
                auditLog ?? TestLog.NoOp);
        }

        private async Task CreateSampleAsync(DbContext dbContext, string instance, int totalOutstanding,
            DateTime? createdDate = null)
        {
            var id = await dbContext.InsertWithInt32IdentityAsync(new Data.Poco.RedisConnectionMultiplexerMetric
            {
                CreatedDate = createdDate ?? DateTime.UtcNow,
                Instance = instance,
                ClientName = $"{DatabaseFixture.Prefix}Client",
        ...
        public async Task ListReturnsMostRecentFirstAndProjectsFieldsAsync()
        {
            ...
            await CreateSampleAsync(dbContext, instance, 2);
            ...
            mine[0].TotalOutstanding.Should().Be(2);
            mine[1].TotalOutstanding.Should().Be(1);
        }
        ...
        public async Task ListFiltersByDateRangeAsync()
        {
            ...
            await CreateSampleAsync(dbContext, instance, 1, oldDate);
            await CreateSampleAsync(dbContext, instance, 2, newDate);
        ...
        public async Task ListFiltersBySearchAsync()
        {
            ...
            await CreateSampleAsync(dbContext, uniqueInstance, 1);
            await CreateSampleAsync(dbContext, otherInstance, 2);
        ...
        public async Task ListDefaultsToLastHourWhenFromAndToOmittedAsync()
        {
            ...
            await CreateSampleAsync(dbContext, $"{instance}Old", 1, twoHoursAgo);
            await CreateSampleAsync(dbContext, $"{instance}New", 2, justNow);
        ...
        [Fact]
        public async Task ListSortsByTotalOutstandingAscendingAndDescendingAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";
            ...
            var ascending = await service.ListAsync(search: instance, sortField: "totalOutstanding",
                sortDirection: "asc");
            ascending.Rows.Select(r => r.TotalOutstanding).Should().Equal(1000, 2000, 3000);

            var descending = await service.ListAsync(search: instance, sortField: "totalOutstanding",
                sortDirection: "desc");
            descending.Rows.Select(r => r.TotalOutstanding).Should().Equal(3000, 2000, 1000);
        }
        ...
        public async Task ListSortDirectionIsCaseInsensitiveForAscendingAsync(string ascendingKeyword)
        {
            ...
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var result = await service.ListAsync(search: instance, sortField: "totalOutstanding",
                sortDirection: ascendingKeyword);

            result.Rows.Select(r => r.TotalOutstanding).Should().Equal(1000, 2000);
        }
        ...
        public async Task ListWithGarbageSortDirectionFallsBackToDescendingAsync()
        {
            ...
            result.Rows.Select(r => r.TotalOutstanding).Should().Equal(2000, 1000);
        }
        ...
        public async Task ListWithUnrecognisedSortFieldFallsBackToMostRecentFirstAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";
            await CreateSampleAsync(dbContext, instance, 1, DateTime.UtcNow.AddMinutes(-30));
            await CreateSampleAsync(dbContext, instance, 2, DateTime.UtcNow);
        ...
        [Fact]
        public async Task ListStatisticsComputesExactValuesForTotalOutstandingAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";
            ...
            var stats = result.Statistics.Columns["totalOutstanding"];
        ...
        public async Task ListStatisticsIncludesEveryContinuousMeasuredColumnAsync()
        {
            ...
            result.Statistics.Columns.Keys.Should().BeEquivalentTo(
                ...
                "totalOutstanding", "interactivePendingUnsentItems", "interactiveSentItemsAwaitingResponse",
                "interactiveResponsesAwaitingAsyncCompletion", "interactiveTotalOutstanding",
                "interactiveCompletedAsynchronously", "interactiveCompletedSynchronously",
                "interactiveFailedAsynchronously", "interactiveNonPreferredEndpointCount",
                "interactiveSocketCount", "interactiveWriterCount", "subscriptionPendingUnsentItems",
                "subscriptionSentItemsAwaitingResponse", "subscriptionResponsesAwaitingAsyncCompletion",
                "subscriptionTotalOutstanding", "subscriptionCompletedAsynchronously",
                "subscriptionCompletedSynchronously", "subscriptionFailedAsynchronously",
                "subscriptionSocketCount", "subscriptionCount", "connectionFailedCount",
                "connectionRestoredCount", "errorMessageCount", "internalErrorCount",
                "configurationChangedCount", "configurationChangedBroadcastCount");
        }
    }
}
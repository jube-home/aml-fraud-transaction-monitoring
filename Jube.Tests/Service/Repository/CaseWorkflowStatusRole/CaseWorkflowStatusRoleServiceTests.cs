...
namespace Jube.Test.Service.Repository.CaseWorkflowStatusRole
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaseWorkflowStatusRoleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        private static Task<CaseWorkflowStatusRoleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaseWorkflowStatusRoleService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
        ...
        private async Task<Guid> CreateStatusAsync(DbContext dbContext, int caseWorkflowId, string createdUser)
        {
            ...
            createdStatusIds.Add(statusId);
            return statusGuid;
        }

        private async Task<(int modelId, int workflowId, Guid statusGuid)> CreateModelWorkflowStatusAsync(
            DbContext dbContext, string userName)
        {
            ...
            var statusGuid = await CreateStatusAsync(dbContext, workflowId, userName);
            return (modelId, workflowId, statusGuid);
        }
        ...
        public async Task ListWithoutPermissionThrowsForbiddenAsync()
        {
            ...
            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.GetByCaseWorkflowStatusGuidAsync(Guid.NewGuid()));
        }
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, statusGuid) = await CreateModelWorkflowStatusAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowStatusRole>()
                .AnyAsync(w => w.CaseWorkflowStatusGuid == statusGuid)).Should().BeFalse();
        }
        ...
        public async Task InsertPersistsAndReturnsGrantAsync()
        {
            ...
            var saved = await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            saved.CaseWorkflowStatusGuid.Should().Be(statusGuid);
            saved.RoleRegistryGuid.Should().Be(roleRegistryGuid);
        }

        [Fact]
        public async Task InsertWithEmptyCaseWorkflowStatusGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
        ...
        [Fact]
        public async Task InsertWithUnknownCaseWorkflowStatusGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
        ...
        [Fact]
        public async Task InsertWithCrossTenantCaseWorkflowStatusGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, statusGuidB) = await CreateModelWorkflowStatusAsync(dbContext, fx.Seed.UserTenantB);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuidB,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task InsertPublishesCreatedChangeEventAsync()
        {
            ...
            bus.Published[0].Area.Should().Be("CaseWorkflowStatusRole");
        ...
        [Fact]
        public async Task ListByCaseWorkflowStatusGuidReturnsGrantsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, statusGuid) = await CreateModelWorkflowStatusAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });

            var grants = await service.GetByCaseWorkflowStatusGuidAsync(statusGuid);
        ...
        [Fact]
        public async Task ListForUnknownStatusGuidReturnsEmptyAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var grants = await service.GetByCaseWorkflowStatusGuidAsync(Guid.NewGuid());
        ...
        public async Task ListIsScopedToCallersTenantAsync()
        {
            ...
            await serviceB.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
            ...
            var grantsForA = await serviceA.GetByCaseWorkflowStatusGuidAsync(statusGuidB);
        ...
        public async Task DeleteSoftDeletesAndPublishesDeletedChangeEventAsync()
        {
            ...
            var row = await dbContext.GetTable<Data.Poco.CaseWorkflowStatusRole>()
        ...
        public async Task DeleteOfCrossTenantRowThrowsKeyNotFoundExceptionAsync()
        {
            ...
            var roleRegistryGuidB = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserTenantB);
            var serviceB = await BuildServiceAsync(dbContext, fx.Seed.UserTenantB);
            var savedB = await serviceB.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
        ...
        public async Task ListHonoursCancellationAsync()
        {
            ...
            var act = () => service.GetByCaseWorkflowStatusGuidAsync(Guid.NewGuid(), cancelledToken);
        ...
        public async Task ListLogsOneAuditRecordAsync()
        {
            ...
            await service.GetByCaseWorkflowStatusGuidAsync(Guid.NewGuid());

            capturingAuditLog.Entries.Should().ContainSingle();
        }

        [Fact]
        public void CatalogueListsUniqueCaseWorkflowStatusRoleToolNames()
        {
            var tools = ServiceToolCatalogue.All;
            var names = tools.Where(t => t.Name.StartsWith("CaseWorkflowStatusRole"))
                .Select(t => t.Name).ToList();

            names.Should().BeEquivalentTo([
                "CaseWorkflowStatusRoleListByCaseWorkflowStatusGuid",
                "CaseWorkflowStatusRoleCreate",
                "CaseWorkflowStatusRoleDelete",
            ]);
        ...
        public async Task RepeatedGrantReturnsTheExistingGrantAndWritesNoSecondRowAsync()
        {
            ...
            var first = await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var second = await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowStatusRole>().CountAsync(w =>
                w.CaseWorkflowStatusGuid == statusGuid
        ...
        public async Task RepeatedGrantAfterRevokeCreatesANewLiveGrantAsync()
        {
            ...
            var again = await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task RepeatedGrantAndRevokeLeavesNoAccessBehindAsync()
        {
            ...
            var repeated = await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task TheSameRoleOnTwoParentsIsTwoIndependentGrantsAsync()
        {
            ...
            var (_, _, statusGuidTwo) = await CreateModelWorkflowStatusAsync(dbContext, fx.Seed.UserWithPermission);

            var one = await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var two = await service.InsertAsync(new CaseWorkflowStatusRoleDto
            {
                CaseWorkflowStatusGuid = statusGuidTwo,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowStatusRole>().CountAsync(w =>
                ...
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
            (await dbContext.GetTable<Data.Poco.CaseWorkflowStatusRole>().CountAsync(w =>
                w.CaseWorkflowStatusGuid == statusGuidTwo
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
        }
        ...
        public async Task ConcurrentIdenticalGrantsProduceExactlyOneLiveRowAsync()
        {
            ...
            await Task.WhenAll(Enumerable.Range(0, 8).Select(async _ =>
            {
                ...
                var saved = await raceService.InsertAsync(new CaseWorkflowStatusRoleDto
                {
                    CaseWorkflowStatusGuid = statusGuid,
                    RoleRegistryGuid = roleRegistryGuid,
                });

...
namespace Jube.Test.Service.Repository.CaseWorkflowXPathRole
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaseWorkflowXPathRoleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            var xpathGuids = await dbContext.CaseWorkflowXPath
                .Where(w => createdXPathIds.Contains(w.Id)).Select(w => w.Guid).ToListAsync();

            await dbContext.GetTable<Data.Poco.CaseWorkflowXPathRole>()
                .Where(w => xpathGuids.Contains(w.CaseWorkflowXPathGuid)).DeleteAsync();
        ...
        private static Task<CaseWorkflowXPathRoleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaseWorkflowXPathRoleService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
        ...
        private async Task<Guid> CreateXPathAsync(DbContext dbContext, int caseWorkflowId, string createdUser)
        {
            var xpathGuid = Guid.NewGuid();
            var xpathId = await dbContext.InsertWithInt32IdentityAsync(new Data.Poco.CaseWorkflowXPath
            {
                Name = $"{DatabaseFixture.Prefix}XPath{Guid.NewGuid():N}"[..40],
                Guid = xpathGuid,
        ...
        private async Task<(int modelId, int workflowId, Guid xpathGuid)> CreateModelWorkflowXPathAsync(
            DbContext dbContext, string userName)
        {
            ...
            var xpathGuid = await CreateXPathAsync(dbContext, workflowId, userName);
            return (modelId, workflowId, xpathGuid);
        }
        ...
        public async Task ListWithoutPermissionThrowsForbiddenAsync()
        {
            ...
            await Assert.ThrowsAsync<ForbiddenException>(() => service.GetByCaseWorkflowXPathGuidAsync(Guid.NewGuid()));
        }
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, xpathGuid) = await CreateModelWorkflowXPathAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowXPathRole>()
                .AnyAsync(w => w.CaseWorkflowXPathGuid == xpathGuid)).Should().BeFalse();
        }
        ...
        public async Task InsertPersistsAndReturnsGrantAsync()
        {
            ...
            var saved = await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            saved.CaseWorkflowXPathGuid.Should().Be(xpathGuid);
            saved.RoleRegistryGuid.Should().Be(roleRegistryGuid);
        }

        [Fact]
        public async Task InsertWithEmptyCaseWorkflowXPathGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = Guid.Empty,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithUnknownCaseWorkflowXPathGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            ...
            var act = () => service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = Guid.NewGuid(),
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithCrossTenantCaseWorkflowXPathGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, xpathGuidB) = await CreateModelWorkflowXPathAsync(dbContext, fx.Seed.UserTenantB);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuidB,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task InsertPublishesCreatedChangeEventAsync()
        {
            ...
            bus.Published[0].Area.Should().Be("CaseWorkflowXPathRole");
        ...
        [Fact]
        public async Task ListByCaseWorkflowXPathGuidReturnsGrantsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, xpathGuid) = await CreateModelWorkflowXPathAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });

            var grants = await service.GetByCaseWorkflowXPathGuidAsync(xpathGuid);
        ...
        [Fact]
        public async Task ListForUnknownXPathGuidReturnsEmptyAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var grants = await service.GetByCaseWorkflowXPathGuidAsync(Guid.NewGuid());
        ...
        public async Task ListIsScopedToCallersTenantAsync()
        {
            ...
            await serviceB.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
            ...
            var grantsForA = await serviceA.GetByCaseWorkflowXPathGuidAsync(xpathGuidB);
        ...
        public async Task DeleteSoftDeletesAndPublishesDeletedChangeEventAsync()
        {
            ...
            var row = await dbContext.GetTable<Data.Poco.CaseWorkflowXPathRole>()
        ...
        public async Task DeleteOfCrossTenantRowThrowsKeyNotFoundExceptionAsync()
        {
            ...
            var roleRegistryGuidB = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserTenantB);
            var serviceB = await BuildServiceAsync(dbContext, fx.Seed.UserTenantB);
            var savedB = await serviceB.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
        ...
        public async Task ListHonoursCancellationAsync()
        {
            ...
            var act = () => service.GetByCaseWorkflowXPathGuidAsync(Guid.NewGuid(), cancelledToken);
        ...
        public async Task ListLogsOneAuditRecordAsync()
        {
            ...
            await service.GetByCaseWorkflowXPathGuidAsync(Guid.NewGuid());

            capturingAuditLog.Entries.Should().ContainSingle();
        }

        [Fact]
        public void CatalogueListsUniqueCaseWorkflowXPathRoleToolNames()
        {
            var tools = ServiceToolCatalogue.All;
            var names = tools.Where(t => t.Name.StartsWith("CaseWorkflowXPathRole"))
                .Select(t => t.Name).ToList();

            names.Should().BeEquivalentTo([
                "CaseWorkflowXPathRoleListByCaseWorkflowXPathGuid",
                "CaseWorkflowXPathRoleCreate",
                "CaseWorkflowXPathRoleDelete",
            ]);
        ...
        public async Task RepeatedGrantReturnsTheExistingGrantAndWritesNoSecondRowAsync()
        {
            ...
            var first = await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var second = await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowXPathRole>().CountAsync(w =>
                w.CaseWorkflowXPathGuid == xpathGuid
        ...
        public async Task RepeatedGrantAfterRevokeCreatesANewLiveGrantAsync()
        {
            ...
            var again = await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task RepeatedGrantAndRevokeLeavesNoAccessBehindAsync()
        {
            ...
            var repeated = await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task TheSameRoleOnTwoParentsIsTwoIndependentGrantsAsync()
        {
            ...
            var (_, _, xpathGuidTwo) = await CreateModelWorkflowXPathAsync(dbContext, fx.Seed.UserWithPermission);

            var one = await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var two = await service.InsertAsync(new CaseWorkflowXPathRoleDto
            {
                CaseWorkflowXPathGuid = xpathGuidTwo,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowXPathRole>().CountAsync(w =>
                ...
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
            (await dbContext.GetTable<Data.Poco.CaseWorkflowXPathRole>().CountAsync(w =>
                w.CaseWorkflowXPathGuid == xpathGuidTwo
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
        }
        ...
        public async Task ConcurrentIdenticalGrantsProduceExactlyOneLiveRowAsync()
        {
            ...
            await Task.WhenAll(Enumerable.Range(0, 8).Select(async _ =>
            {
                ...
                var saved = await raceService.InsertAsync(new CaseWorkflowXPathRoleDto
                {
                    CaseWorkflowXPathGuid = xpathGuid,
                    RoleRegistryGuid = roleRegistryGuid,
                });

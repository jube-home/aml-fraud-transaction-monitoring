...
namespace Jube.Test.Service.Repository.CaseWorkflowDisplay
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaseWorkflowDisplayServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        private static Task<CaseWorkflowDisplayService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaseWorkflowDisplayService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
        ...
        private async Task GrantDisplayRoleAsync(DbContext dbContext, Guid caseWorkflowDisplayGuid, string userName)
        {
            var roleRegistryGuid = await dbContext.UserRegistry
                .Where(u => u.Name == userName).Select(u => u.RoleRegistryGuid).FirstAsync();
            ...
            {
                CaseWorkflowDisplayGuid = caseWorkflowDisplayGuid,
                Guid = Guid.NewGuid(),
                RoleRegistryGuid = roleRegistryGuid,
                CreatedUser = userName,
                CreatedDate = DateTime.UtcNow,
        ...
        private static CaseWorkflowDisplayDto ValidDto(int caseWorkflowId, string? name = null) => new()
        {
            Name = name ?? $"{DatabaseFixture.Prefix}Display{Guid.NewGuid():N}"[..40],
        ...
        public async Task InsertWithoutPermissionThrowsForbiddenAndWritesNoRowAsync()
        {
            ...
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);
            var name = $"{DatabaseFixture.Prefix}Display{Guid.NewGuid():N}"[..40];

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(ValidDto(workflowId, name)));

            (await dbContext.CaseWorkflowDisplay.AnyAsync(w => w.Name == name)).Should().BeFalse();
        }
        ...
        public async Task UpdateWithoutPermissionThrowsForbiddenAsync()
        {
            ...
            createdDisplayIds.Add(created.Id);
        ...
        public async Task InsertPersistsAndDropsIdentityFieldsAsync()
        {
            ...
            createdDisplayIds.Add(saved.Id);
        ...
        public async Task InsertWithDuplicateNameInSameWorkflowThrowsValidationFailedAsync()
        {
            ...
            createdDisplayIds.Add(first.Id);
        ...
        [Fact]
        public async Task InsertWithBlankHtmlThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateModelAsync(dbContext, fx.Seed.UserWithPermission);
            var workflowId = await CreateWorkflowAsync(dbContext, modelId, fx.Seed.UserWithPermission);
        ...
        public async Task InsertIgnoresDescriptionAndHttpNotificationFieldsAsync()
        {
            ...
            dto.HttpEndpoint = "https://example.invalid/webhook";
            dto.HttpEndpointTypeId = 1;
            dto.EnableNotification = true;
            dto.NotificationDestination = "someone@example.invalid";
            ...
            saved.HttpEndpointTypeId.Should().Be(0);
        ...
        [Fact]
        public async Task ListByWorkflowIdActiveOnlyRequiresBothWorkflowAndDisplayRoleGrantsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateModelAsync(dbContext, fx.Seed.UserWithPermission);
            ...
            await GrantDisplayRoleAsync(dbContext, created.Guid, fx.Seed.UserWithPermissionNoApproveByReview);
            (await service.GetByCasesWorkflowIdActiveOnlyAsync(workflowId)).Should()
        ...
        public async Task InsertPublishesCreatedChangeEventAsync()
        {
            ...
            change.Area.Should().Be("CaseWorkflowDisplay");
        ...
        [Fact]
        public void CatalogueRegistersUniqueCaseWorkflowDisplayToolNames()
        {
            var tools = ServiceToolCatalogue.All;

            var names = new[]
            {
                "CaseWorkflowDisplayList", "CaseWorkflowDisplayListByWorkflowIdActiveOnly",
                "CaseWorkflowDisplayListByWorkflowGuidActiveOnly", "CaseWorkflowDisplayGet",
                "CaseWorkflowDisplayCreate", "CaseWorkflowDisplayUpdate", "CaseWorkflowDisplayDelete",
            };

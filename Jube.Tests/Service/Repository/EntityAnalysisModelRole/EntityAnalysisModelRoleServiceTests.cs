...
using Jube.Service.Repository.EntityAnalysisModelRole;
...
namespace Jube.Test.Service.Repository.EntityAnalysisModelRole
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelRoleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            var modelGuids = await dbContext.EntityAnalysisModel
                .Where(w => createdModelIds.Contains(w.Id)).Select(w => w.Guid).ToListAsync();

            await dbContext.GetTable<Data.Poco.EntityAnalysisModelRole>()
        ...
        private static Task<EntityAnalysisModelRoleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelRoleService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
                serviceChangeBus ?? new NullServiceChangeBus(), auditLog ?? TestLog.NoOp);
        }

        private async Task<Guid> CreateModelAsync(DbContext dbContext, string createdUser)
        {
        ...
        public async Task ListWithoutPermissionThrowsForbiddenAsync()
        {
            ...
            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.GetByEntityAnalysisModelGuidAsync(Guid.NewGuid()));
        }
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var modelGuid = await CreateModelAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(new EntityAnalysisModelRoleDto
            {
            ...
            (await dbContext.GetTable<Data.Poco.EntityAnalysisModelRole>()
                .AnyAsync(w => w.EntityAnalysisModelGuid == modelGuid)).Should().BeFalse();
        }
        ...
        public async Task InsertPersistsAndReturnsGrantAsync()
        {
            ...
            var saved = await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            saved.EntityAnalysisModelGuid.Should().Be(modelGuid);
            saved.RoleRegistryGuid.Should().Be(roleRegistryGuid);
        }

        [Fact]
        public async Task InsertWithEmptyEntityAnalysisModelGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = Guid.Empty,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithUnknownEntityAnalysisModelGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            ...
            var act = () => service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = Guid.NewGuid(),
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithCrossTenantEntityAnalysisModelGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelGuidB = await CreateModelAsync(dbContext, fx.Seed.UserTenantB);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuidB,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task InsertPublishesCreatedChangeEventAsync()
        {
            ...
            bus.Published[0].Area.Should().Be("EntityAnalysisModelRole");
        ...
        [Fact]
        public async Task ListByEntityAnalysisModelGuidReturnsGrantsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelGuid = await CreateModelAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });

            var grants = await service.GetByEntityAnalysisModelGuidAsync(modelGuid);
        ...
        [Fact]
        public async Task ListForUnknownModelGuidReturnsEmptyAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var grants = await service.GetByEntityAnalysisModelGuidAsync(Guid.NewGuid());
        ...
        public async Task ListIsScopedToCallersTenantAsync()
        {
            ...
            await serviceB.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
            ...
            var grantsForA = await serviceA.GetByEntityAnalysisModelGuidAsync(modelGuidB);
        ...
        public async Task DeleteSoftDeletesAndPublishesDeletedChangeEventAsync()
        {
            ...
            var row = await dbContext.GetTable<Data.Poco.EntityAnalysisModelRole>()
        ...
        public async Task DeleteOfCrossTenantRowThrowsKeyNotFoundExceptionAsync()
        {
            ...
            var roleRegistryGuidB = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserTenantB);
            var serviceB = await BuildServiceAsync(dbContext, fx.Seed.UserTenantB);
            var savedB = await serviceB.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
        ...
        public async Task ListHonoursCancellationAsync()
        {
            ...
            var act = () => service.GetByEntityAnalysisModelGuidAsync(Guid.NewGuid(), cancelledToken);
        ...
        public async Task ListLogsOneAuditRecordAsync()
        {
            ...
            await service.GetByEntityAnalysisModelGuidAsync(Guid.NewGuid());

            capturingAuditLog.Entries.Should().ContainSingle();
        }

        [Fact]
        public void CatalogueListsUniqueEntityAnalysisModelRoleToolNames()
        {
            var tools = ServiceToolCatalogue.All;
            var names = tools.Where(t => t.Name.StartsWith("EntityAnalysisModelRole"))
                .Select(t => t.Name).ToList();

            names.Should().BeEquivalentTo([
                "EntityAnalysisModelRoleListByEntityAnalysisModelGuid",
                "EntityAnalysisModelRoleCreate",
                "EntityAnalysisModelRoleDelete",
            ]);
        ...
        public async Task RepeatedGrantReturnsTheExistingGrantAndWritesNoSecondRowAsync()
        {
            ...
            var first = await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var second = await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.EntityAnalysisModelRole>().CountAsync(w =>
                w.EntityAnalysisModelGuid == modelGuid
        ...
        public async Task RepeatedGrantAfterRevokeCreatesANewLiveGrantAsync()
        {
            ...
            var again = await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task RepeatedGrantAndRevokeLeavesNoAccessBehindAsync()
        {
            ...
            var repeated = await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task TheSameRoleOnTwoParentsIsTwoIndependentGrantsAsync()
        {
            ...
            var modelGuidTwo = await CreateModelAsync(dbContext, fx.Seed.UserWithPermission);

            var one = await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var two = await service.InsertAsync(new EntityAnalysisModelRoleDto
            {
                EntityAnalysisModelGuid = modelGuidTwo,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.EntityAnalysisModelRole>().CountAsync(w =>
                ...
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
            (await dbContext.GetTable<Data.Poco.EntityAnalysisModelRole>().CountAsync(w =>
                w.EntityAnalysisModelGuid == modelGuidTwo
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
        }
        ...
        public async Task ConcurrentIdenticalGrantsProduceExactlyOneLiveRowAsync()
        {
            ...
            await Task.WhenAll(Enumerable.Range(0, 8).Select(async _ =>
            {
                ...
                var saved = await raceService.InsertAsync(new EntityAnalysisModelRoleDto
                {
                    EntityAnalysisModelGuid = modelGuid,
                    RoleRegistryGuid = roleRegistryGuid,
                });

...
using Jube.Service.Repository.CaseWorkflowForm;
...
namespace Jube.Test.Service.Repository.CaseWorkflowForm
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaseWorkflowFormServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            await dbContext.GetTable<Data.Poco.CaseWorkflowFormVersion>()
                .Where(w => w.CaseWorkflowFormId != null && createdFormIds.Contains(w.CaseWorkflowFormId.Value))
                .DeleteAsync();
        ...
        private static Task<CaseWorkflowFormService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaseWorkflowFormService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
        ...
        private async Task GrantFormRoleAsync(DbContext dbContext, Guid caseWorkflowFormGuid, string userName)
        {
            var roleRegistryGuid = await dbContext.UserRegistry
                .Where(u => u.Name == userName).Select(u => u.RoleRegistryGuid).FirstAsync();
            ...
            {
                CaseWorkflowFormGuid = caseWorkflowFormGuid,
                Guid = Guid.NewGuid(),
                RoleRegistryGuid = roleRegistryGuid,
                CreatedUser = userName,
                CreatedDate = DateTime.UtcNow,
        ...
        private static CaseWorkflowFormDto ValidDto(int caseWorkflowId, string? name = null) => new()
        {
            Name = name ?? $"{DatabaseFixture.Prefix}Form{Guid.NewGuid():N}"[..40],
            CaseWorkflowId = caseWorkflowId,
            Active = true,
            Locked = false,
            Html = "<div>Case Workflow Form</div>",
        ...
        public async Task InsertWithoutPermissionThrowsForbiddenAndWritesNoRowAsync()
        {
            ...
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);
            var name = $"{DatabaseFixture.Prefix}Form{Guid.NewGuid():N}"[..40];

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(ValidDto(workflowId, name)));

            var stillExists = await dbContext.CaseWorkflowForm.AnyAsync(w => w.Name == name);
        ...
        public async Task UpdatePreservesImportIdAsync()
        {
            ...
            await dbContext.CaseWorkflowForm.Where(w => w.Id == created.Id)
            ...
            try
            {
                ...
                var persistedImportId = await dbContext.CaseWorkflowForm.Where(w => w.Id == created.Id)
        ...
        public async Task ListByWorkflowIdActiveOnlyRequiresBothWorkflowAndFormRoleGrantsAsync()
        {
            ...
            await GrantFormRoleAsync(dbContext, created.Guid, fx.Seed.UserWithPermission);
        ...
        public void CatalogueRegistersExpectedToolNamesForThisArea()
        {
            ...
            names.Should().Contain("CaseWorkflowFormList");
            names.Should().Contain("CaseWorkflowFormListByWorkflowIdActiveOnly");
            names.Should().Contain("CaseWorkflowFormListByWorkflowGuidActiveOnly");
            names.Should().Contain("CaseWorkflowFormGet");
            names.Should().Contain("CaseWorkflowFormCreate");
            names.Should().Contain("CaseWorkflowFormUpdate");
            names.Should().Contain("CaseWorkflowFormDelete");
        }
    }
}
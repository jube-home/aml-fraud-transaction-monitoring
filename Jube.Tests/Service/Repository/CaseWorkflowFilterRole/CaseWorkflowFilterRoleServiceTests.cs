...
namespace Jube.Test.Service.Repository.CaseWorkflowFilterRole
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaseWorkflowFilterRoleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        private static Task<CaseWorkflowFilterRoleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaseWorkflowFilterRoleService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
        ...
        private async Task<Guid> CreateFilterAsync(DbContext dbContext, int caseWorkflowId, string createdUser)
        {
            var filterGuid = Guid.NewGuid();
            var filterId = await dbContext.InsertWithInt32IdentityAsync(new Data.Poco.CaseWorkflowFilter
            {
                Name = $"{DatabaseFixture.Prefix}Filter{Guid.NewGuid():N}"[..40],
                Guid = filterGuid,
        ...
        private async Task<(int modelId, int workflowId, Guid filterGuid)> CreateModelWorkflowFilterAsync(
            DbContext dbContext, string userName)
        {
            ...
            var filterGuid = await CreateFilterAsync(dbContext, workflowId, userName);
            return (modelId, workflowId, filterGuid);
        }
        ...
        public async Task ListWithoutPermissionThrowsForbiddenAsync()
        {
            ...
            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.GetByCaseWorkflowFilterGuidAsync(Guid.NewGuid()));
        }
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, filterGuid) = await CreateModelWorkflowFilterAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowFilterRole>()
                .AnyAsync(w => w.CaseWorkflowFilterGuid == filterGuid)).Should().BeFalse();
        }
        ...
        public async Task InsertPersistsAndReturnsGrantAsync()
        {
            ...
            var saved = await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            saved.CaseWorkflowFilterGuid.Should().Be(filterGuid);
            saved.RoleRegistryGuid.Should().Be(roleRegistryGuid);
        }

        [Fact]
        public async Task InsertWithEmptyCaseWorkflowFilterGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = Guid.Empty,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithUnknownCaseWorkflowFilterGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            ...
            var act = () => service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = Guid.NewGuid(),
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithCrossTenantCaseWorkflowFilterGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, filterGuidB) = await CreateModelWorkflowFilterAsync(dbContext, fx.Seed.UserTenantB);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuidB,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task InsertPublishesCreatedChangeEventAsync()
        {
            ...
            bus.Published[0].Area.Should().Be("CaseWorkflowFilterRole");
        ...
        [Fact]
        public async Task ListByCaseWorkflowFilterGuidReturnsGrantsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, filterGuid) = await CreateModelWorkflowFilterAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });

            var grants = await service.GetByCaseWorkflowFilterGuidAsync(filterGuid);
        ...
        [Fact]
        public async Task ListForUnknownFilterGuidReturnsEmptyAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var grants = await service.GetByCaseWorkflowFilterGuidAsync(Guid.NewGuid());
        ...
        public async Task ListIsScopedToCallersTenantAsync()
        {
            ...
            await serviceB.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
            ...
            var grantsForA = await serviceA.GetByCaseWorkflowFilterGuidAsync(filterGuidB);
        ...
        public async Task DeleteSoftDeletesAndPublishesDeletedChangeEventAsync()
        {
            ...
            var row = await dbContext.GetTable<Data.Poco.CaseWorkflowFilterRole>()
        ...
        public async Task DeleteOfCrossTenantRowThrowsKeyNotFoundExceptionAsync()
        {
            ...
            var roleRegistryGuidB = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserTenantB);
            var serviceB = await BuildServiceAsync(dbContext, fx.Seed.UserTenantB);
            var savedB = await serviceB.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
        ...
        public async Task ListHonoursCancellationAsync()
        {
            ...
            var token = cts.Token;
            var act = () => service.GetByCaseWorkflowFilterGuidAsync(Guid.NewGuid(), token);
        ...
        public async Task ListLogsOneAuditRecordAsync()
        {
            ...
            await service.GetByCaseWorkflowFilterGuidAsync(Guid.NewGuid());

            capturingAuditLog.Entries.Should().ContainSingle();
        }

        [Fact]
        public void CatalogueListsUniqueCaseWorkflowFilterRoleToolNames()
        {
            var tools = ServiceToolCatalogue.All;
            var names = tools.Where(t => t.Name.StartsWith("CaseWorkflowFilterRole"))
                .Select(t => t.Name).ToList();

            names.Should().BeEquivalentTo([
                "CaseWorkflowFilterRoleListByCaseWorkflowFilterGuid",
                "CaseWorkflowFilterRoleCreate",
                "CaseWorkflowFilterRoleDelete",
            ]);
        ...
        public async Task RepeatedGrantReturnsTheExistingGrantAndWritesNoSecondRowAsync()
        {
            ...
            var first = await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var second = await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowFilterRole>().CountAsync(w =>
                w.CaseWorkflowFilterGuid == filterGuid
        ...
        public async Task RepeatedGrantAfterRevokeCreatesANewLiveGrantAsync()
        {
            ...
            var again = await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task RepeatedGrantAndRevokeLeavesNoAccessBehindAsync()
        {
            ...
            var repeated = await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task TheSameRoleOnTwoParentsIsTwoIndependentGrantsAsync()
        {
            ...
            var (_, _, filterGuidTwo) = await CreateModelWorkflowFilterAsync(dbContext, fx.Seed.UserWithPermission);

            var one = await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var two = await service.InsertAsync(new CaseWorkflowFilterRoleDto
            {
                CaseWorkflowFilterGuid = filterGuidTwo,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowFilterRole>().CountAsync(w =>
                ...
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
            (await dbContext.GetTable<Data.Poco.CaseWorkflowFilterRole>().CountAsync(w =>
                w.CaseWorkflowFilterGuid == filterGuidTwo
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
        }
        ...
        public async Task ConcurrentIdenticalGrantsProduceExactlyOneLiveRowAsync()
        {
            ...
            await Task.WhenAll(Enumerable.Range(0, 8).Select(async _ =>
            {
                ...
                var saved = await raceService.InsertAsync(new CaseWorkflowFilterRoleDto
                {
                    CaseWorkflowFilterGuid = filterGuid,
                    RoleRegistryGuid = roleRegistryGuid,
                });

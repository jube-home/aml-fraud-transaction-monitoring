...
using Jube.Service.Repository.CaseWorkflowDisplayRole;
...
namespace Jube.Test.Service.Repository.CaseWorkflowDisplayRole
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaseWorkflowDisplayRoleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            var displayGuids = await dbContext.CaseWorkflowDisplay
                .Where(w => createdDisplayIds.Contains(w.Id)).Select(w => w.Guid).ToListAsync();
            ...
            await dbContext.GetTable<Data.Poco.CaseWorkflowDisplayVersion>()
                .Where(w => w.CaseWorkflowDisplayId != null &&
                            createdDisplayIds.Contains(w.CaseWorkflowDisplayId.Value))
                .DeleteAsync();
        ...
        private static Task<CaseWorkflowDisplayRoleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaseWorkflowDisplayRoleService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
        ...
        private async Task<Guid> CreateDisplayAsync(DbContext dbContext, int caseWorkflowId, string createdUser)
        {
        ...
        private async Task<(int modelId, int workflowId, Guid displayGuid)> CreateModelWorkflowDisplayAsync(
            DbContext dbContext, string userName)
        {
            ...
            var displayGuid = await CreateDisplayAsync(dbContext, workflowId, userName);
            return (modelId, workflowId, displayGuid);
        }
        ...
        public async Task ListWithoutPermissionThrowsForbiddenAsync()
        {
            ...
            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.GetByCaseWorkflowDisplayGuidAsync(Guid.NewGuid()));
        }
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, displayGuid) = await CreateModelWorkflowDisplayAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowDisplayRole>()
                .AnyAsync(w => w.CaseWorkflowDisplayGuid == displayGuid)).Should().BeFalse();
        }
        ...
        public async Task InsertPersistsAndReturnsGrantAsync()
        {
            ...
            var saved = await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            saved.CaseWorkflowDisplayGuid.Should().Be(displayGuid);
            saved.RoleRegistryGuid.Should().Be(roleRegistryGuid);
        }

        [Fact]
        public async Task InsertWithEmptyCaseWorkflowDisplayGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = Guid.Empty,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithUnknownCaseWorkflowDisplayGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            ...
            var act = () => service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = Guid.NewGuid(),
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithCrossTenantCaseWorkflowDisplayGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, displayGuidB) = await CreateModelWorkflowDisplayAsync(dbContext, fx.Seed.UserTenantB);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuidB,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task InsertPublishesCreatedChangeEventAsync()
        {
            ...
            bus.Published[0].Area.Should().Be("CaseWorkflowDisplayRole");
        ...
        [Fact]
        public async Task ListByCaseWorkflowDisplayGuidReturnsGrantsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, displayGuid) = await CreateModelWorkflowDisplayAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });

            var grants = await service.GetByCaseWorkflowDisplayGuidAsync(displayGuid);
        ...
        [Fact]
        public async Task ListForUnknownDisplayGuidReturnsEmptyAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var grants = await service.GetByCaseWorkflowDisplayGuidAsync(Guid.NewGuid());
        ...
        public async Task ListIsScopedToCallersTenantAsync()
        {
            ...
            await serviceB.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
            ...
            var grantsForA = await serviceA.GetByCaseWorkflowDisplayGuidAsync(displayGuidB);
        ...
        public async Task DeleteSoftDeletesAndPublishesDeletedChangeEventAsync()
        {
            ...
            var row = await dbContext.GetTable<Data.Poco.CaseWorkflowDisplayRole>()
        ...
        public async Task DeleteOfCrossTenantRowThrowsKeyNotFoundExceptionAsync()
        {
            ...
            var roleRegistryGuidB = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserTenantB);
            var serviceB = await BuildServiceAsync(dbContext, fx.Seed.UserTenantB);
            var savedB = await serviceB.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
        ...
        public async Task ListHonoursCancellationAsync()
        {
            ...
            var token = cts.Token;
            var act = () => service.GetByCaseWorkflowDisplayGuidAsync(Guid.NewGuid(), token);
        ...
        public async Task ListLogsOneAuditRecordAsync()
        {
            ...
            await service.GetByCaseWorkflowDisplayGuidAsync(Guid.NewGuid());

            capturingAuditLog.Entries.Should().ContainSingle();
        }

        [Fact]
        public void CatalogueListsUniqueCaseWorkflowDisplayRoleToolNames()
        {
            var tools = ServiceToolCatalogue.All;
            var names = tools.Where(t => t.Name.StartsWith("CaseWorkflowDisplayRole"))
                .Select(t => t.Name).ToList();

            names.Should().BeEquivalentTo([
                "CaseWorkflowDisplayRoleListByCaseWorkflowDisplayGuid",
                "CaseWorkflowDisplayRoleCreate",
                "CaseWorkflowDisplayRoleDelete",
            ]);
        ...
        public async Task RepeatedGrantReturnsTheExistingGrantAndWritesNoSecondRowAsync()
        {
            ...
            var first = await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var second = await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowDisplayRole>().CountAsync(w =>
                w.CaseWorkflowDisplayGuid == displayGuid
        ...
        public async Task RepeatedGrantAfterRevokeCreatesANewLiveGrantAsync()
        {
            ...
            var again = await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task RepeatedGrantAndRevokeLeavesNoAccessBehindAsync()
        {
            ...
            var repeated = await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task TheSameRoleOnTwoParentsIsTwoIndependentGrantsAsync()
        {
            ...
            var (_, _, displayGuidTwo) = await CreateModelWorkflowDisplayAsync(dbContext, fx.Seed.UserWithPermission);

            var one = await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var two = await service.InsertAsync(new CaseWorkflowDisplayRoleDto
            {
                CaseWorkflowDisplayGuid = displayGuidTwo,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowDisplayRole>().CountAsync(w =>
                ...
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
            (await dbContext.GetTable<Data.Poco.CaseWorkflowDisplayRole>().CountAsync(w =>
                w.CaseWorkflowDisplayGuid == displayGuidTwo
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
        }
        ...
        public async Task ConcurrentIdenticalGrantsProduceExactlyOneLiveRowAsync()
        {
            ...
            await Task.WhenAll(Enumerable.Range(0, 8).Select(async _ =>
            {
                ...
                var saved = await raceService.InsertAsync(new CaseWorkflowDisplayRoleDto
                {
                    CaseWorkflowDisplayGuid = displayGuid,
                    RoleRegistryGuid = roleRegistryGuid,
                });

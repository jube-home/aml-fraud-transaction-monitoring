...
using Jube.Service.Repository.CaseWorkflowActionRole;
...
namespace Jube.Test.Service.Repository.CaseWorkflowActionRole
{
    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class CaseWorkflowActionRoleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        private static Task<CaseWorkflowActionRoleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return CaseWorkflowActionRoleService.CreateAsync(dbContext, userName, log ?? TestLog.NoOp, localizers,
        ...
        private async Task<Guid> CreateActionAsync(DbContext dbContext, int caseWorkflowId, string createdUser)
        {
        ...
        private async Task<(int modelId, int workflowId, Guid actionGuid)> CreateModelWorkflowActionAsync(
            DbContext dbContext, string userName)
        {
            ...
            var actionGuid = await CreateActionAsync(dbContext, workflowId, userName);
            return (modelId, workflowId, actionGuid);
        }
        ...
        public async Task ListWithoutPermissionThrowsForbiddenAsync()
        {
            ...
            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.GetByCaseWorkflowActionGuidAsync(Guid.NewGuid()));
        }
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, actionGuid) = await CreateModelWorkflowActionAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            await Assert.ThrowsAsync<ForbiddenException>(() => service.InsertAsync(new CaseWorkflowActionRoleDto
            {
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowActionRole>()
                .AnyAsync(w => w.CaseWorkflowActionGuid == actionGuid)).Should().BeFalse();
        }
        ...
        public async Task InsertPersistsAndReturnsGrantAsync()
        {
            ...
            var saved = await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            saved.CaseWorkflowActionGuid.Should().Be(actionGuid);
            saved.RoleRegistryGuid.Should().Be(roleRegistryGuid);
        }

        [Fact]
        public async Task InsertWithEmptyCaseWorkflowActionGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = Guid.Empty,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithUnknownCaseWorkflowActionGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            ...
            var act = () => service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = Guid.NewGuid(),
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        [Fact]
        public async Task InsertWithCrossTenantCaseWorkflowActionGuidThrowsValidationFailedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, actionGuidB) = await CreateModelWorkflowActionAsync(dbContext, fx.Seed.UserTenantB);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var act = () => service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuidB,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task InsertPublishesCreatedChangeEventAsync()
        {
            ...
            bus.Published[0].Area.Should().Be("CaseWorkflowActionRole");
        ...
        [Fact]
        public async Task ListByCaseWorkflowActionGuidReturnsGrantsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, _, actionGuid) = await CreateModelWorkflowActionAsync(dbContext, fx.Seed.UserWithPermission);
            var roleRegistryGuid = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });

            var grants = await service.GetByCaseWorkflowActionGuidAsync(actionGuid);
        ...
        [Fact]
        public async Task ListForUnknownActionGuidReturnsEmptyAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var grants = await service.GetByCaseWorkflowActionGuidAsync(Guid.NewGuid());
        ...
        public async Task ListIsScopedToCallersTenantAsync()
        {
            ...
            await serviceB.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
            ...
            var grantsForA = await serviceA.GetByCaseWorkflowActionGuidAsync(actionGuidB);
        ...
        public async Task DeleteSoftDeletesAndPublishesDeletedChangeEventAsync()
        {
            ...
            var row = await dbContext.GetTable<Data.Poco.CaseWorkflowActionRole>()
        ...
        public async Task DeleteOfCrossTenantRowThrowsKeyNotFoundExceptionAsync()
        {
            ...
            var roleRegistryGuidB = await GetRoleRegistryGuidAsync(dbContext, fx.Seed.UserTenantB);
            var serviceB = await BuildServiceAsync(dbContext, fx.Seed.UserTenantB);
            var savedB = await serviceB.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuidB,
                RoleRegistryGuid = roleRegistryGuidB,
            });
        ...
        public async Task ListHonoursCancellationAsync()
        {
            ...
            var act = () => service.GetByCaseWorkflowActionGuidAsync(Guid.NewGuid(), cancelledToken);
        ...
        public async Task ListLogsOneAuditRecordAsync()
        {
            ...
            await service.GetByCaseWorkflowActionGuidAsync(Guid.NewGuid());

            capturingAuditLog.Entries.Should().ContainSingle();
        }

        [Fact]
        public void CatalogueListsUniqueCaseWorkflowActionRoleToolNames()
        {
            var tools = ServiceToolCatalogue.All;
            var names = tools.Where(t => t.Name.StartsWith("CaseWorkflowActionRole"))
                .Select(t => t.Name).ToList();

            names.Should().BeEquivalentTo([
                "CaseWorkflowActionRoleListByCaseWorkflowActionGuid",
                "CaseWorkflowActionRoleCreate",
                "CaseWorkflowActionRoleDelete",
            ]);
        ...
        public async Task RepeatedGrantReturnsTheExistingGrantAndWritesNoSecondRowAsync()
        {
            ...
            var first = await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var second = await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowActionRole>().CountAsync(w =>
                w.CaseWorkflowActionGuid == actionGuid
        ...
        public async Task RepeatedGrantAfterRevokeCreatesANewLiveGrantAsync()
        {
            ...
            var again = await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task RepeatedGrantAndRevokeLeavesNoAccessBehindAsync()
        {
            ...
            var repeated = await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
        ...
        public async Task TheSameRoleOnTwoParentsIsTwoIndependentGrantsAsync()
        {
            ...
            var (_, _, actionGuidTwo) = await CreateModelWorkflowActionAsync(dbContext, fx.Seed.UserWithPermission);

            var one = await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuid,
                RoleRegistryGuid = roleRegistryGuid,
            });
            var two = await service.InsertAsync(new CaseWorkflowActionRoleDto
            {
                CaseWorkflowActionGuid = actionGuidTwo,
                RoleRegistryGuid = roleRegistryGuid,
            });
            ...
            (await dbContext.GetTable<Data.Poco.CaseWorkflowActionRole>().CountAsync(w =>
                ...
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
            (await dbContext.GetTable<Data.Poco.CaseWorkflowActionRole>().CountAsync(w =>
                w.CaseWorkflowActionGuid == actionGuidTwo
                && w.RoleRegistryGuid == roleRegistryGuid && (w.Deleted == 0 || w.Deleted == null))).Should().Be(1);
        }
        ...
        public async Task ConcurrentIdenticalGrantsProduceExactlyOneLiveRowAsync()
        {
            ...
            await Task.WhenAll(Enumerable.Range(0, 8).Select(async _ =>
            {
                ...
                var saved = await raceService.InsertAsync(new CaseWorkflowActionRoleDto
                {
                    CaseWorkflowActionGuid = actionGuid,
                    RoleRegistryGuid = roleRegistryGuid,
                });

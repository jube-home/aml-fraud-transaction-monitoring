...
namespace Jube.Test.Service.EntityAnalysisModelInlineFunction
{
    using EntityAnalysisModelInlineFunctionService = EntityAnalysisModelInlineFunctionService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelInlineFunctionServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.GetTable<EntityAnalysisModelInlineFunctionVersion>()
                    .Where(w => w.EntityAnalysisModelInlineFunctionId == id).DeleteAsync();
                await dbContext.GetTable<Data.Poco.EntityAnalysisModelInlineFunction>().Where(w => w.Id == id)
                    .DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelInlineFunctionService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelInlineFunctionService.CreateAsync(
        ...
        private static EntityAnalysisModelInlineFunctionDto NewDto(int entityAnalysisModelId, string name)
        {
            return new EntityAnalysisModelInlineFunctionDto
            {
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            dto.Id = saved.Id;
            dto.ReturnDataTypeId = 2;

            var updated = await service.UpdateAsync(dto);

            updated.Version.Should().Be(2);
            updated.ReturnDataTypeId.Should().Be(2);
            updated.Guid.Should().Be(saved.Guid);

            var auditRows = await dbContext.GetTable<EntityAnalysisModelInlineFunctionVersion>()
                .Where(w => w.EntityAnalysisModelInlineFunctionId == saved.Id).CountAsync();
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelInlineFunction>()
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelInlineFunction>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelInlineFunctionService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelInlineFunctionDto.Name) &&
        ...
        [Fact]
        public async Task InvalidReturnDataTypeIdIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadDataType"));
            dto.ReturnDataTypeId = 999;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "ReturnDataTypeIdInvalid");
        }
        ...
        [InlineData("")]
        [InlineData("   ")]
        public async Task FunctionScriptRequiredRejectsNullEmptyOrWhitespaceAsync(string? functionScript)
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoScript"));
            dto.FunctionScript = functionScript;
        ...
        [Fact]
        public async Task FunctionScriptOverMaximumLengthIsRejectedWithErrorCodeAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BigScript"));
            dto.FunctionScript = new string('x', 65537);
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelInlineFunction.Create").Subject;
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelInlineFunctionList", "EntityAnalysisModelInlineFunctionGet",
                "EntityAnalysisModelInlineFunctionGetByEntityAnalysisModelId",
                "EntityAnalysisModelInlineFunctionCreate", "EntityAnalysisModelInlineFunctionUpdate",
                "EntityAnalysisModelInlineFunctionDelete"
            ]);
        }

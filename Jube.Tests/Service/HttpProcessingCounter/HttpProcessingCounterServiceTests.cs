...
using Jube.Service.HttpProcessingCounter;
...
namespace Jube.Test.Service.HttpProcessingCounter
{
    using HttpProcessingCounterService = HttpProcessingCounterService;
    using HttpProcessingCounterPoco = Data.Poco.HttpProcessingCounter;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class HttpProcessingCounterServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.HttpProcessingCounter.Where(w => w.Id == id).DeleteAsync();
            }
        }

        private static Task<HttpProcessingCounterService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return HttpProcessingCounterService.CreateAsync(
                dbContext, userName, log ?? TestLog.NoOp, localizers, serviceChangeBus ?? new NullServiceChangeBus(),
                auditLog ?? TestLog.NoOp);
        }

        private async Task<string> CreateCounterAsync(DbContext dbContext, int all, int model = 0,
            int asynchronousModel = 0, int tag = 0, int error = 0, int sanction = 0, int exhaustive = 0,
            DateTime? createdDate = null, string? instance = null)
        {
            instance ??= $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";

            var id = await dbContext.InsertWithInt32IdentityAsync(
                new HttpProcessingCounterPoco
                {
                    Instance = instance,
        ...
        [Fact]
        public async Task ListSortsByAllAscendingAndDescendingAsync()
        {
            await using var dbContext = fx.GetDbContext();
            ...
            var ascendingMine = ascending.Rows
                ...
                .ToList();
            ascendingMine.Select(r => r.All).Should().Equal(10, 20, 30);

            var descending = await service.ListAsync(sortField: "all", sortDirection: "desc");
            var descendingMine = descending.Rows
                .Where(r => r.Instance == instanceLow || r.Instance == instanceMid || r.Instance == instanceHigh)
                .ToList();
            descendingMine.Select(r => r.All).Should().Equal(30, 20, 10);
        }
        ...
        public async Task ListSortDirectionIsCaseInsensitiveForAscendingAsync(string ascendingKeyword)
        {
            ...
            mine.Select(r => r.All).Should().Equal(10, 20);
        }
        ...
        public async Task ListWithGarbageSortDirectionFallsBackToDescendingAsync()
        {
            ...
            mine.Select(r => r.All).Should().Equal(20, 10);
        }
        ...
        public async Task ListStatisticsIncludesEveryContinuousMeasuredColumnAsync()
        {
            ...
            result.Statistics.Columns["all"].Histogram.Should().NotBeNull();
        }
    }
}
...
using Jube.Service.EntityAnalysisModelDictionary;
using Jube.Service.Exceptions.EntityAnalysisModelDictionary;
...
namespace Jube.Test.Service.EntityAnalysisModelDictionary
{
    using EntityAnalysisModelDictionaryService = EntityAnalysisModelDictionaryService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelDictionaryServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            foreach (var id in createdIds)
            {
                await dbContext.GetTable<EntityAnalysisModelDictionaryVersion>()
                    .Where(w => w.EntityAnalysisModelDictionaryId == id).DeleteAsync();
                await dbContext.GetTable<Data.Poco.EntityAnalysisModelDictionary>().Where(w => w.Id == id)
                    .DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelDictionaryService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelDictionaryService.CreateAsync(
        ...
        private static EntityAnalysisModelDictionaryDto NewDto(Guid entityAnalysisModelGuid, string name)
        {
            return new EntityAnalysisModelDictionaryDto
            {
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Insert")));
        ...
        public async Task GetAllReturnsCreatedRowAndGetByIdReturnsItAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(model.Guid, name));
            createdIds.Add(saved.Id);
            ...
            byId.DataName.Should().Be("AccountId");
        }

        [Fact]
        public async Task GetByEntityAnalysisModelIdReturnsActiveAndInactiveRowsForThatModelOrderedByIdAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelA = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var modelB = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var activeDto = NewDto(modelA.Guid, UniqueName("ByModelAActive"));
            activeDto.Active = true;
            var savedActive = await service.InsertAsync(activeDto);
            createdIds.Add(savedActive.Id);

            var inactiveDto = NewDto(modelA.Guid, UniqueName("ByModelAInactive"));
            inactiveDto.Active = false;
            var savedInactive = await service.InsertAsync(inactiveDto);
            createdIds.Add(savedInactive.Id);

            var otherModelDto = NewDto(modelB.Guid, UniqueName("ByModelB"));
            otherModelDto.Active = true;
            var savedB = await service.InsertAsync(otherModelDto);
            createdIds.Add(savedB.Id);

            var forModelA = await service.GetByEntityAnalysisModelIdAsync(modelA.Id);
            forModelA.Should().Contain(d => d.Id == savedActive.Id);
            forModelA.Should().Contain(d => d.Id == savedInactive.Id);
            forModelA.Should().NotContain(d => d.Id == savedB.Id);
            forModelA.Select(d => d.Id).Should().BeInAscendingOrder();
        }
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Update")));
            createdIds.Add(saved.Id);

            var dto = NewDto(model.Guid, saved.Name);
            dto.Id = saved.Id;
            dto.Active = true;
            ...
            var auditRows = await dbContext.GetTable<EntityAnalysisModelDictionaryVersion>()
                .Where(w => w.EntityAnalysisModelDictionaryId == saved.Id).CountAsync();
            auditRows.Should().Be(1);
        }

        [Fact]
        public async Task DeleteSoftDeletesAndRowDisappearsFromGetByIdAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Delete")));
            createdIds.Add(saved.Id);
            ...
            var byModel = await service.GetByEntityAnalysisModelIdAsync(model.Id);
            byModel.Should().NotContain(d => d.Id == saved.Id);
        }

        [Fact]
        public async Task DeletedRowStillAppearsInUnboundedGetAsyncPreExistingRepositoryQuirkAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("DeleteAllQuirk")));
            createdIds.Add(saved.Id);
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var model = await CreateParentModelAsync(writerDb, fx.Seed.UserWithPermission);
            var writer = await BuildServiceAsync(writerDb, fx.Seed.UserWithPermission);
            var seedRow = await writer.InsertAsync(NewDto(model.Guid, UniqueName("Forbidden")));
            createdIds.Add(seedRow.Id);

            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelDictionary>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));

            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.InsertAsync(NewDto(model.Guid, UniqueName("Denied"))));
            await Assert.ThrowsAsync<ForbiddenException>(() => service.GetByIdAsync(seedRow.Id));

            var updateDto = NewDto(model.Guid, seedRow.Name);
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelDictionary>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelDictionaryService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task UserInTenantBCannotGetUpdateOrDeleteTenantARowAsync()
        {
            ...
            var model = await CreateParentModelAsync(ownerDb, fx.Seed.UserWithPermission);
            var owner = await BuildServiceAsync(ownerDb, fx.Seed.UserWithPermission);
            var saved = await owner.InsertAsync(NewDto(model.Guid, UniqueName("Isolation")));
            createdIds.Add(saved.Id);
            ...
            var updateDto = NewDto(model.Guid, saved.Name);
        ...
        public async Task GetAllAsTenantBNeverContainsTenantARowsAsync()
        {
            ...
            var model = await CreateParentModelAsync(ownerDb, fx.Seed.UserWithPermission);
            var owner = await BuildServiceAsync(ownerDb, fx.Seed.UserWithPermission);
            var saved = await owner.InsertAsync(NewDto(model.Guid, UniqueName("TenantAOnly")));
            createdIds.Add(saved.Id);
        ...
        public async Task NameOverMaximumLengthIsRejectedWithErrorCodeAsync()
        {
            ...
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(model.Guid, new string('n', 257));
        ...
        public async Task DuplicateNameWithinSameModelFailsButDifferentModelSucceedsAsync()
        {
            ...
            var first = await service.InsertAsync(NewDto(modelA.Guid, name));
            createdIds.Add(first.Id);

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() =>
                service.InsertAsync(NewDto(modelA.Guid, name)));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "NameDuplicate");

            var second = await service.InsertAsync(NewDto(modelB.Guid, name));
        ...
        public async Task DuplicateNameDifferingOnlyByCaseWithinSameModelFailsAsync()
        {
            ...
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var first = await service.InsertAsync(NewDto(model.Guid, name));
            createdIds.Add(first.Id);

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() =>
                service.InsertAsync(NewDto(model.Guid, name.ToUpperInvariant())));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "NameDuplicate");
        }

        [Fact]
        public async Task InvalidEntityAnalysisModelGuidIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(Guid.Empty, UniqueName("BadModelGuid"));

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "EntityAnalysisModelGuidInvalid");
        }
        ...
        [InlineData("")]
        [InlineData("   ")]
        public async Task DataNameRequiredRejectsNullEmptyOrWhitespaceAsync(string? dataName)
        {
            await using var dbContext = fx.GetDbContext();
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(model.Guid, UniqueName("BadDataName"));
            dto.DataName = dataName;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelDictionaryDto.DataName) &&
                e.ErrorCode == "DataNameNotEmpty");
        }

        [Fact]
        public async Task DataNameOverMaximumLengthIsRejectedWithErrorCodeAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(model.Guid, UniqueName("LongDataName"));
            dto.DataName = new string('d', 257);

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "DataNameMaximumLength");
        }
        ...
        public async Task TamperedIdentityAndAuditFieldsOnInsertHaveNoEffectAsync()
        {
            ...
            var dto = NewDto(model.Guid, UniqueName("Tamper"));
        ...
        public async Task TamperedIdentityAndAuditFieldsOnUpdateHaveNoEffectAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("TamperUpdate")));
            createdIds.Add(saved.Id);
        ...
        public async Task UpdateOfLockedRowThrowsNotFoundAsync()
        {
            ...
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Locked")));
            createdIds.Add(saved.Id);
        ...
        public async Task UpdateOfSoftDeletedRowThrowsNotFoundAsync()
        {
            ...
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("PreDeleted")));
            createdIds.Add(saved.Id);
        ...
        public async Task UpdateOfNeverExistedIdThrowsNotFoundAsync()
        {
            ...
            var dto = NewDto(model.Guid, UniqueName("Missing"));
        ...
        public async Task DeleteOfMissingOrAlreadyDeletedRowThrowsNotFoundAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("DoubleDelete")));
            createdIds.Add(saved.Id);
            await service.DeleteAsync(saved.Id);
        ...
        [Fact]
        public async Task ActiveLockedAndResponsePayloadRoundTripBoolToByteAndBackAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(model.Guid, UniqueName("ActiveLocked"));
            dto.Active = true;
        ...
        public async Task SuccessfulInsertLogsExactlyOneInfoAndReadsLogNoInfoAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("LogInsert")));
            createdIds.Add(saved.Id);
        ...
        public async Task WithGatesDisabledHappyPathRecordsNoDebugInfoOrWarnAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Gated")));
            createdIds.Add(saved.Id);
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Span")));
            createdIds.Add(saved.Id);

            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelDictionary.Create").Subject;
        ...
        public async Task EachCallRecordsOneDurationMeasurementAsync()
        {
            ...
            var model = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Metric")));
            createdIds.Add(saved.Id);
        ...
        public async Task InsertPublishesExactlyOneCreatedEventAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName("Reactive")));
            createdIds.Add(saved.Id);
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelDictionaryList", "EntityAnalysisModelDictionaryGet",
                "EntityAnalysisModelDictionaryGetByEntityAnalysisModelId",
                "EntityAnalysisModelDictionaryCreate", "EntityAnalysisModelDictionaryUpdate",
                "EntityAnalysisModelDictionaryDelete"
            ]);
        }
        ...
        public async Task ListAsyncClampsTakeAndPaginatesDeterministicallyAsync()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                var saved = await service.InsertAsync(NewDto(model.Guid, UniqueName($"Page{i}")));
                createdIds.Add(saved.Id);
            }

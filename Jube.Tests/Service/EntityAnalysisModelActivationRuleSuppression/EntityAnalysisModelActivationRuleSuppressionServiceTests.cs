...
using Jube.Service.EntityAnalysisModelActivationRuleSuppression;
...
namespace Jube.Test.Service.EntityAnalysisModelActivationRuleSuppression
{
    using EntityAnalysisModelActivationRuleSuppressionService = EntityAnalysisModelActivationRuleSuppressionService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelActivationRuleSuppressionServiceTests(DatabaseFixture fx)
        : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.GetTable<EntityAnalysisModelActivationRuleSuppressionVersion>()
                    .Where(w => w.EntityAnalysisModelActivationRuleSuppressionId == id).DeleteAsync();
        ...
        private static Task<EntityAnalysisModelActivationRuleSuppressionService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelActivationRuleSuppressionService.CreateAsync(
        ...
        private async Task<string> CreateActivationRuleAsync(DbContext dbContext, string createdUser,
            int entityAnalysisModelId, string? name = null)
        {
            var repository = new EntityAnalysisModelActivationRuleRepository(dbContext, createdUser);
            var ruleName = name ?? $"{DatabaseFixture.Prefix}Rule{Guid.NewGuid():N}"[..40];
            var saved = await repository.InsertAsync(new Data.Poco.EntityAnalysisModelActivationRule
            {
        ...
        private static EntityAnalysisModelActivationRuleSuppressionDto NewDto(Guid entityAnalysisModelGuid,
            string key, string keyValue, string activationRuleName)
        {
            return new EntityAnalysisModelActivationRuleSuppressionDto
            {
        ...
        {
            await using var dbContext = fx.GetDbContext();
            var (modelId, modelGuid) = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Insert"), ruleName));
        ...
        public async Task GetAllReturnsCreatedRowAndGetByIdReturnsItAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", keyValue, ruleName));
            createdIds.Add(saved.Id);
            ...
            byId.EntityAnalysisModelActivationRuleName.Should().Be(ruleName);
        }

        [Fact]
        public async Task GetByEntityAnalysisModelGuidReturnsOnlyRowsForThatModelAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (modelAId, modelAGuid) = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var ruleAName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelAId);
            var (modelBId, modelBGuid) = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var ruleBName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelBId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var savedA = await service.InsertAsync(NewDto(modelAGuid, "IP", UniqueValue("ByModelA"), ruleAName));
            createdIds.Add(savedA.Id);
            var savedB = await service.InsertAsync(NewDto(modelBGuid, "IP", UniqueValue("ByModelB"), ruleBName));
            createdIds.Add(savedB.Id);

            var forModelA = await service.GetByEntityAnalysisModelGuidAsync(modelAGuid);
        ...
        public async Task UpdateTogglesOffExistingRowAndOnANewRowAsync()
        {
            ...
            var toggleOn = NewDto(modelGuid, key, keyValue, ruleName);
            var firstResult = await service.UpdateAsync(toggleOn);
            firstResult.Id.Should().BeGreaterThan(0);
            createdIds.Add(firstResult.Id);

            var stillOn = await service.GetByEntityAnalysisModelGuidAsync(modelGuid);
            stillOn.Should().ContainSingle(d => d.SuppressionKeyValue == keyValue);

            var toggleOff = NewDto(modelGuid, key, keyValue, ruleName);
            await service.UpdateAsync(toggleOff);

            var nowOff = await service.GetByEntityAnalysisModelGuidAsync(modelGuid);
        ...
        public async Task UpdateDeleteExpiryDateUpdatesActiveRowAsync()
        {
            ...
            var saved = await service.UpdateAsync(NewDto(modelGuid, key, keyValue, ruleName));
            createdIds.Add(saved.Id);

            var expiry = DateTimeOffset.UtcNow.AddDays(1);
            var updateDto = NewDto(modelGuid, key, keyValue, ruleName);
        ...
        public async Task UpdateDeleteExpiryDateForMissingRowThrowsNotFoundAsync()
        {
            ...
            var dto = NewDto(modelGuid, "IP", UniqueValue("NoExpiry"), ruleName);
        ...
        public async Task DeleteSoftDeletesAndRowDisappearsFromReadsAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Delete"), ruleName));
            createdIds.Add(saved.Id);
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(writerDb, fx.Seed.UserWithPermission, modelId);
            var writer = await BuildServiceAsync(writerDb, fx.Seed.UserWithPermission);
            var seedRow = await writer.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Forbidden"), ruleName));
            createdIds.Add(seedRow.Id);

            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithoutPermission);

            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelActivationRuleSuppression>()
                .CountAsync(w => w.SuppressionKeyValue!.StartsWith(DatabaseFixture.Prefix));

            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Denied"), ruleName)));
            ...
            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.GetByEntityAnalysisModelGuidAsync(modelGuid));
            await Assert.ThrowsAsync<ForbiddenException>(() =>
                service.UpdateAsync(NewDto(modelGuid, "IP", UniqueValue("DeniedUpdate"), ruleName)));
            await Assert.ThrowsAsync<ForbiddenException>(() => service.DeleteAsync(seedRow.Id));

            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelActivationRuleSuppression>()
                .CountAsync(w => w.SuppressionKeyValue!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelActivationRuleSuppressionService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task UserInTenantBCannotGetOrDeleteTenantARowAsync()
        {
            ...
            var (modelId, modelGuid) = await CreateParentModelAsync(ownerDb, fx.Seed.UserWithPermission);
            var ruleName = await CreateActivationRuleAsync(ownerDb, fx.Seed.UserWithPermission, modelId);
            var owner = await BuildServiceAsync(ownerDb, fx.Seed.UserWithPermission);
            var saved = await owner.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Isolation"), ruleName));
            createdIds.Add(saved.Id);
        ...
        public async Task GetAllAsTenantBNeverContainsTenantARowsAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(ownerDb, fx.Seed.UserWithPermission, modelId);
            var owner = await BuildServiceAsync(ownerDb, fx.Seed.UserWithPermission);
            var saved = await owner.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("TenantAOnly"), ruleName));
            createdIds.Add(saved.Id);
        ...
        public async Task SuppressionKeyRequiredRejectsNullEmptyOrWhitespaceAsync(string? key)
        {
            ...
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelGuid, key!, UniqueValue("BadKey"), ruleName);

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelActivationRuleSuppressionDto.SuppressionKey) &&
        ...
        public async Task SuppressionKeyOverMaximumLengthIsRejectedWithErrorCodeAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelGuid, new string('k', 257), UniqueValue("BadKeyLen"), ruleName);
        ...
        public async Task SuppressionKeyValueRequiredRejectsEmptyAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelGuid, "IP", "", ruleName);
        ...
        public async Task SuppressionKeyValueOverMaximumLengthIsRejectedWithErrorCodeAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelGuid, "IP", new string('v', 257), ruleName);
        ...
        public async Task EmptyEntityAnalysisModelGuidIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(Guid.Empty, "IP", UniqueValue("BadGuid"), "SomeRule");
        ...
        public async Task NonExistentEntityAnalysisModelGuidIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(Guid.NewGuid(), "IP", UniqueValue("MissingModel"), "SomeRule");
        ...
        [InlineData("")]
        [InlineData("   ")]
        public async Task EntityAnalysisModelActivationRuleNameRequiredRejectsNullEmptyOrWhitespaceAsync(
            string? name)
        {
            await using var dbContext = fx.GetDbContext();
            var (_, modelGuid) = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelGuid, "IP", UniqueValue("BadRuleName"), name!);

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "EntityAnalysisModelActivationRuleNameNotEmpty");
        }

        [Fact]
        public async Task NonExistentEntityAnalysisModelActivationRuleNameIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var (_, modelGuid) = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelGuid, "IP", UniqueValue("MissingRule"), UniqueValue("NoSuchRule"));

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "EntityAnalysisModelActivationRuleNameNotFound");
        }
        ...
        public async Task EntityAnalysisModelActivationRuleNameFromAnotherModelIsRejectedAsync()
        {
            ...
            var dto = NewDto(modelAGuid, "IP", UniqueValue("CrossModelRule"), ruleOnModelBName);
        ...
        public async Task DeleteExpiryDateInThePastIsRejectedAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelGuid, "IP", UniqueValue("PastExpiry"), ruleName);
            dto.DeleteExpiryDate = DateTimeOffset.UtcNow.AddDays(-1);
        ...
        public async Task TamperedIdentityAndAuditFieldsOnInsertHaveNoEffectAsync()
        {
            ...
            var dto = NewDto(modelGuid, "IP", UniqueValue("Tamper"), ruleName);
        ...
        public async Task SuccessfulInsertLogsExactlyOneInfoAndReadsLogNoInfoAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("LogInsert"), ruleName));
            createdIds.Add(saved.Id);
        ...
        public async Task WithGatesDisabledHappyPathRecordsNoDebugInfoOrWarnAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Gated"), ruleName));
            createdIds.Add(saved.Id);
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Span"), ruleName));
            createdIds.Add(saved.Id);

            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelActivationRuleSuppression.Create")
                .Subject;
        ...
        public async Task EachCallRecordsOneDurationMeasurementAsync()
        {
            ...
            var ruleName = await CreateActivationRuleAsync(dbContext, fx.Seed.UserWithPermission, modelId);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Metric"), ruleName));
            createdIds.Add(saved.Id);
        ...
        public async Task InsertPublishesExactlyOneCreatedEventAsync()
        {
            ...
            var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue("Reactive"), ruleName));
            createdIds.Add(saved.Id);
        ...
        public async Task ReadsAndFailuresPublishNothingAsync()
        {
            ...
            await Assert.ThrowsAsync<DtoValidationException>(() =>
                service.InsertAsync(NewDto(modelGuid, "IP", "", ruleName)));
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelActivationRuleSuppressionList",
                "EntityAnalysisModelActivationRuleSuppressionGet",
                "EntityAnalysisModelActivationRuleSuppressionGetByEntityAnalysisModelGuid",
                "EntityAnalysisModelActivationRuleSuppressionCreate",
                "EntityAnalysisModelActivationRuleSuppressionUpdate",
                "EntityAnalysisModelActivationRuleSuppressionUpdateDeleteExpiryDate",
                "EntityAnalysisModelActivationRuleSuppressionDelete"
            ]);
        }
        ...
        public async Task ListAsyncClampsTakeAndPaginatesDeterministicallyAsync()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                var saved = await service.InsertAsync(NewDto(modelGuid, "IP", UniqueValue($"Page{i}"), ruleName));
                createdIds.Add(saved.Id);
            }

...
using Jube.Service.PostgresIndexStatistics;
...
namespace Jube.Test.Service.PostgresIndexStatistics
{
    using PostgresIndexStatisticsService = PostgresIndexStatisticsService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class PostgresIndexStatisticsServiceTests(DatabaseFixture fx)
    {
        private static readonly IStringLocalizerFactory localizers =
            new ResourceManagerStringLocalizerFactory(Options.Create(new LocalizationOptions()),
                NullLoggerFactory.Instance);

        private static Task<PostgresIndexStatisticsService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return PostgresIndexStatisticsService.CreateAsync(
                dbContext, userName, log ?? TestLog.NoOp, localizers, serviceChangeBus ?? new NullServiceChangeBus(),
                auditLog ?? TestLog.NoOp);
        }

        [Fact]
        public async Task ListReturnsRealIndexesFromTheSchemaAsync()
        {
            ...
            result.Rows.Should().Contain(r => r.TableName == "EntityAnalysisModel" && r.IsPrimary);
        }
        ...
        [Fact]
        public async Task ListSortsByIndexSizeBytesAscendingAndDescendingAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var ascending = await service.ListAsync("indexSizeBytes", "asc");
            ascending.Rows.Select(r => r.IndexSizeBytes).Should().BeInAscendingOrder();

            var descending = await service.ListAsync("indexSizeBytes", "desc");
            descending.Rows.Select(r => r.IndexSizeBytes).Should().BeInDescendingOrder();
        }
        ...
        public async Task ListWithUnrecognisedSortFieldPreservesQueryOrderAsync()
        {
            ...
            withGarbageSort.Rows.Select(r => r.IndexName).Should().Equal(unsorted.Rows.Select(r => r.IndexName));
        }
        ...
        public async Task ListStatisticsIncludesEveryContinuousMeasuredColumnAsync()
        {
            ...
            result.Statistics.Columns.Keys.Should().BeEquivalentTo(
                "indexScans", "indexTuplesRead", "indexTuplesFetched", "indexSizeBytes");
        }
    }
}
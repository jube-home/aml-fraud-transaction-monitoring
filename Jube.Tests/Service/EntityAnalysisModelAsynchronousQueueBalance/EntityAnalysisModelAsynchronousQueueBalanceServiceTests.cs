...
using Jube.Service.EntityAnalysisModelAsynchronousQueueBalance;
...
namespace Jube.Test.Service.EntityAnalysisModelAsynchronousQueueBalance
{
    using EntityAnalysisModelAsynchronousQueueBalanceService =
        EntityAnalysisModelAsynchronousQueueBalanceService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelAsynchronousQueueBalanceServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.EntityAnalysisModelAsynchronousQueueBalance.Where(w => w.Id == id).DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelAsynchronousQueueBalanceService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelAsynchronousQueueBalanceService.CreateAsync(
        ...
        private async Task CreateRowAsync(DbContext dbContext, Guid entityAnalysisModelGuid, int archive = 0,
            int activationWatcher = 0, DateTime? createdDate = null, string? instance = null)
        {
            var id = await dbContext.InsertWithInt32IdentityAsync(
                new Data.Poco.EntityAnalysisModelAsynchronousQueueBalance
                {
        ...
        public async Task ListReturnsMostRecentFirstAndProjectsModelNameAsync()
        {
            ...
            mine[0].Archive.Should().Be(2);
            mine[1].Archive.Should().Be(1);
        ...
        public async Task ListFiltersBySearchAgainstModelNameCaseInsensitivelyAsync()
        {
            ...
            var mine = result.Rows.Where(r =>
                r.EntityAnalysisModelGuid == matchingGuid || r.EntityAnalysisModelGuid == otherGuid).ToList();
            mine.Should().ContainSingle();
        ...
        public async Task ListSortsByArchiveAscendingAndDescendingAsync()
        {
            ...
            ascendingMine.Select(r => r.Archive).Should().Equal(10, 20, 30);

            var descending = await service.ListAsync(sortField: "archive", sortDirection: "desc");
            var descendingMine = descending.Rows.Where(r => r.EntityAnalysisModelGuid == modelGuid).ToList();
            descendingMine.Select(r => r.Archive).Should().Equal(30, 20, 10);
        }
        ...
        public async Task ListSortDirectionIsCaseInsensitiveForAscendingAsync(string ascendingKeyword)
        {
            ...
            var mine = result.Rows.Where(r => r.EntityAnalysisModelGuid == modelGuid).ToList();
            mine.Select(r => r.Archive).Should().Equal(10, 20);
        }
        ...
        public async Task ListWithGarbageSortDirectionFallsBackToDescendingAsync()
        {
            ...
            var mine = result.Rows.Where(r => r.EntityAnalysisModelGuid == modelGuid).ToList();
            mine.Select(r => r.Archive).Should().Equal(20, 10);
        }

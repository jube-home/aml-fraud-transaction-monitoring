...
{
    using EntityAnalysisModelAbstractionRuleService = EntityAnalysisModelAbstractionRuleService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelAbstractionRuleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        private static Task<EntityAnalysisModelAbstractionRuleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelAbstractionRuleService.CreateAsync(
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            updated.SearchValue.Should().Be(5);
            updated.Guid.Should().Be(saved.Guid);

            var auditRows = await dbContext.GetTable<EntityAnalysisModelAbstractionRuleVersion>()
                .Where(w => w.EntityAnalysisModelAbstractionRuleId == saved.Id).CountAsync();
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelAbstractionRule>()
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelAbstractionRule>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelAbstractionRuleService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelAbstractionRuleDto.Name) &&
        ...
        [Fact]
        public async Task SearchKeyRequiredWhenSearchEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoSearchKey"));
            dto.SearchKey = "";

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "SearchKeyNotEmpty");
        }

        [Fact]
        public async Task SearchKeyNotRequiredWhenSearchDisabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoSearch"));
            dto.Search = false;
            dto.SearchKey = null;
        ...
        [Fact]
        public async Task SearchValueNegativeIsRejectedWhenSearchEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadSearchValue"));
            dto.SearchValue = -1;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "SearchValueRange");
        }

        [Theory]
        [InlineData("x")]
        [InlineData("")]
        public async Task InvalidSearchIntervalIsRejectedWhenSearchEnabledAsync(string searchInterval)
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadInterval"));
            dto.SearchInterval = searchInterval;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "SearchIntervalInvalid");
        }

        [Fact]
        public async Task InvalidSearchFunctionTypeIdIsRejectedWhenSearchEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadFunctionType"));
            dto.SearchFunctionTypeId = 999;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "SearchFunctionTypeIdInvalid");
        }

        [Fact]
        public async Task SearchFunctionKeyRequiredWhenFunctionTypeIsNotCountAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoFunctionKey"));
            dto.SearchFunctionTypeId = 3;
            dto.SearchFunctionKey = null;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "SearchFunctionKeyNotEmpty");
        }

        [Fact]
        public async Task SearchFunctionKeyNotRequiredWhenFunctionTypeIsCountAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("CountFunction"));
            dto.SearchFunctionTypeId = 1;
            dto.SearchFunctionKey = null;
        ...
        [Fact]
        public async Task InvalidOffsetTypeIdIsRejectedWhenOffsetEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadOffsetType"));
            dto.Offset = true;
            dto.OffsetTypeId = 99;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "OffsetTypeIdInvalid");
        }

        [Fact]
        public async Task OffsetValueNegativeIsRejectedWhenOffsetEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadOffsetValue"));
            dto.Offset = true;
            dto.OffsetValue = -1;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "OffsetValueRange");
        }

        [Fact]
        public async Task OffsetNotValidatedWhenOffsetDisabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("OffsetOff"));
            dto.Offset = false;
            dto.OffsetTypeId = 99;
            dto.OffsetValue = -1;
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelAbstractionRule.Create").Subject;
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelAbstractionRuleList", "EntityAnalysisModelAbstractionRuleGet",
                "EntityAnalysisModelAbstractionRuleGetByEntityAnalysisModelId",
                "EntityAnalysisModelAbstractionRuleCreate", "EntityAnalysisModelAbstractionRuleUpdate",
                "EntityAnalysisModelAbstractionRuleDelete"
            ]);
        }

...
using Jube.Service.EntityAnalysisModelReprocessingRule;
...
namespace Jube.Test.Service.EntityAnalysisModelReprocessingRule
{
    using EntityAnalysisModelReprocessingRuleService = EntityAnalysisModelReprocessingRuleService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelReprocessingRuleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        private static Task<EntityAnalysisModelReprocessingRuleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelReprocessingRuleService.CreateAsync(
        ...
        private static EntityAnalysisModelReprocessingRuleDto NewDto(int entityAnalysisModelId, string name)
        {
            return new EntityAnalysisModelReprocessingRuleDto
            {
        ...
        public async Task UpdateSupersedesRowWithNewIdAndIncrementsVersionAsync()
        {
            ...
            var oldRow = await dbContext.GetTable<Data.Poco.EntityAnalysisModelReprocessingRule>()
        ...
        public async Task DeleteSoftDeletesAndRowDisappearsFromReadsAsync()
        {
            ...
            var forModel = await service.GetByEntityAnalysisModelIdAsync(modelId);
            forModel.Should().NotContain(d => d.Id == saved.Id);
        }
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelReprocessingRule>()
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelReprocessingRule>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelReprocessingRuleService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelReprocessingRuleDto.Name) &&
        ...
        [Fact]
        public async Task NegativePriorityIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
        ...
        [InlineData(-1)]
        [InlineData(101)]
        public async Task ReprocessingSampleOutOfRangeIsRejectedAsync(double sample)
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadSample"));
            dto.ReprocessingSample = sample;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "ReprocessingSampleRange");
        }

        [Fact]
        public async Task NegativeReprocessingValueIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadReprocessingValue"));
            dto.ReprocessingValue = -1;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "ReprocessingValueRange");
        }
        ...
        [InlineData("")]
        [InlineData("s")]
        public async Task InvalidReprocessingIntervalIsRejectedAsync(string interval)
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadInterval"));
            dto.ReprocessingInterval = interval;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "ReprocessingIntervalInvalid");
        }
        ...
        [InlineData("d")]
        [InlineData("m")]
        public async Task EachAllowedReprocessingIntervalIsAcceptedAsync(string interval)
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName($"Interval{interval}"));
            dto.ReprocessingInterval = interval;
        ...
        public async Task UpdateOfLockedRowThrowsNotFoundAsync()
        {
            ...
            await dbContext.GetTable<Data.Poco.EntityAnalysisModelReprocessingRule>()
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelReprocessingRule.Create").Subject;
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelReprocessingRuleList", "EntityAnalysisModelReprocessingRuleGet",
                "EntityAnalysisModelReprocessingRuleGetByEntityAnalysisModelId",
                "EntityAnalysisModelReprocessingRuleCreate", "EntityAnalysisModelReprocessingRuleUpdate",
                "EntityAnalysisModelReprocessingRuleDelete"
            ]);
        }

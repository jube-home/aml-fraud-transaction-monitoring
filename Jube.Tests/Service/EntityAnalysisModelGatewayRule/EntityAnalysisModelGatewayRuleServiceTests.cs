...
namespace Jube.Test.Service.EntityAnalysisModelGatewayRule
{
    using EntityAnalysisModelGatewayRuleService = EntityAnalysisModelGatewayRuleService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelGatewayRuleServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            foreach (var id in createdIds)
            {
                await dbContext.GetTable<EntityAnalysisModelGatewayRuleVersion>()
                    .Where(w => w.EntityAnalysisModelGatewayRuleId == id).DeleteAsync();
                await dbContext.GetTable<Data.Poco.EntityAnalysisModelGatewayRule>().Where(w => w.Id == id)
                    .DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelGatewayRuleService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelGatewayRuleService.CreateAsync(
        ...
        private static EntityAnalysisModelGatewayRuleDto NewDto(int entityAnalysisModelId, string name)
        {
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            var auditRows = await dbContext.GetTable<EntityAnalysisModelGatewayRuleVersion>()
                .Where(w => w.EntityAnalysisModelGatewayRuleId == saved.Id).CountAsync();
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelGatewayRule>()
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelGatewayRule>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelGatewayRuleService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task UserInTenantBCannotGetUpdateOrDeleteTenantARowAsync()
        {
            ...
            await Assert.ThrowsAsync<NotFoundException>(() => otherTenant.ResetCounterAsync(saved.Id));
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelGatewayRuleDto.Name) &&
        ...
        [InlineData(-0.1)]
        [InlineData(1.1)]
        public async Task GatewaySampleOutOfRangeIsRejectedAsync(double gatewaySample)
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadSample"));
            dto.GatewaySample = gatewaySample;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "GatewaySampleRange");
        }

        [Fact]
        public async Task MaxResponseElevationNegativeIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadElevation"));
            dto.MaxResponseElevation = -1;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "MaxResponseElevationRange");
        }

        [Fact]
        public async Task PriorityNegativeIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
        ...
        [Fact]
        public async Task ResponsePayloadIsNeverPersistedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("ResponsePayload"));
            ...
            var fetched = await service.GetByIdAsync(saved.Id);
            fetched!.ResponsePayload.Should().BeFalse(
                "ResponsePayload has no backing column on the Poco -- pre-existing quirk carried through verbatim");
        }

        [Fact]
        public async Task UpdateOverwritesCounterFieldsWithSuppliedValuesAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var insertDto = NewDto(modelId, UniqueName("CounterZero"));
            insertDto.ActivationCounter = 7;
            insertDto.EvaluationCounter = 42;
            var saved = await service.InsertAsync(insertDto);
            createdIds.Add(saved.Id);
            ...
            updated.ActivationCounter.Should().Be(0,
                "the legacy client never sends counter fields on update, so they are zeroed -- pre-existing quirk " +
                "carried through verbatim");
            updated.EvaluationCounter.Should().Be(0);
        }
        ...
        public async Task ResetCounterZeroesActivationAndEvaluationCountersAsync()
        {
            ...
            var insertDto = NewDto(modelId, UniqueName("Reset"));
            insertDto.ActivationCounter = 3;
            insertDto.EvaluationCounter = 9;
            var saved = await service.InsertAsync(insertDto);
            createdIds.Add(saved.Id);
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelGatewayRule.Create").Subject;
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelGatewayRuleList", "EntityAnalysisModelGatewayRuleGet",
                "EntityAnalysisModelGatewayRuleGetByEntityAnalysisModelId",
                "EntityAnalysisModelGatewayRuleCreate", "EntityAnalysisModelGatewayRuleUpdate",
                "EntityAnalysisModelGatewayRuleDelete", "EntityAnalysisModelGatewayRuleResetCounter"
            ]);
        }

...
using Jube.Service.EntityAnalysisModelTag;
...
namespace Jube.Test.Service.EntityAnalysisModelTag
{
    using EntityAnalysisModelTagService = EntityAnalysisModelTagService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelTagServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.GetTable<EntityAnalysisModelTagVersion>()
                    .Where(w => w.EntityAnalysisModelTagId == id).DeleteAsync();
                await dbContext.GetTable<Data.Poco.EntityAnalysisModelTag>().Where(w => w.Id == id).DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelTagService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelTagService.CreateAsync(
        ...
        private static EntityAnalysisModelTagDto NewDto(int entityAnalysisModelId, string name)
        {
            return new EntityAnalysisModelTagDto
            {
        ...
        [Fact]
        public async Task GetByEntityAnalysisModelIdReturnsOnlyActiveRowsForThatModelOrderedByNameAsync()
        {
            await using var dbContext = fx.GetDbContext();
            ...
            forModelA.Should().NotContain(d => d.Id == savedInactive.Id);
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            var auditRows = await dbContext.GetTable<EntityAnalysisModelTagVersion>()
                .Where(w => w.EntityAnalysisModelTagId == saved.Id).CountAsync();
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelTag>()
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelTag>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelTagService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelTagDto.Name) &&
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelTag.Create").Subject;
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelTagList", "EntityAnalysisModelTagGet",
                "EntityAnalysisModelTagGetByEntityAnalysisModelId",
                "EntityAnalysisModelTagCreate", "EntityAnalysisModelTagUpdate",
                "EntityAnalysisModelTagDelete"
            ]);
        }

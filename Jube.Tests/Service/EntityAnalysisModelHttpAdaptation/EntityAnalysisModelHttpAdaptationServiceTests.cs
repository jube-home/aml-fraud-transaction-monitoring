...
using Jube.Service.EntityAnalysisModelHttpAdaptation;
...
namespace Jube.Test.Service.EntityAnalysisModelHttpAdaptation
{
    using EntityAnalysisModelHttpAdaptationService = EntityAnalysisModelHttpAdaptationService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelHttpAdaptationServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.GetTable<EntityAnalysisModelHttpAdaptationVersion>()
                    .Where(w => w.EntityAnalysisModelHttpAdaptationId == id).DeleteAsync();
                await dbContext.GetTable<Data.Poco.EntityAnalysisModelHttpAdaptation>().Where(w => w.Id == id)
                    .DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelHttpAdaptationService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelHttpAdaptationService.CreateAsync(
        ...
        private static EntityAnalysisModelHttpAdaptationDto NewDto(int entityAnalysisModelId, string name)
        {
            return new EntityAnalysisModelHttpAdaptationDto
            {
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            dto.Id = saved.Id;
            dto.HttpEndpoint = "/api/invoke/ChangedEndpoint";

            var updated = await service.UpdateAsync(dto);

            updated.Version.Should().Be(2);
            updated.HttpEndpoint.Should().Be("/api/invoke/ChangedEndpoint");
            updated.Guid.Should().Be(saved.Guid);

            var auditRows = await dbContext.GetTable<EntityAnalysisModelHttpAdaptationVersion>()
                .Where(w => w.EntityAnalysisModelHttpAdaptationId == saved.Id).CountAsync();
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelHttpAdaptation>()
            ...
            var afterCount = await dbContext.GetTable<Data.Poco.EntityAnalysisModelHttpAdaptation>()
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelHttpAdaptationService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelHttpAdaptationDto.Name) &&
        ...
        [InlineData("")]
        [InlineData("   ")]
        public async Task HttpEndpointRequiredRejectsNullEmptyOrWhitespaceAsync(string? httpEndpoint)
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NoEndpoint"));
            dto.HttpEndpoint = httpEndpoint;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "HttpEndpointNotEmpty");
        }

        [Fact]
        public async Task HttpEndpointOverMaximumLengthIsRejectedWithErrorCodeAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("HugeEndpoint"));
            dto.HttpEndpoint = "/" + new string('a', 2048);

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "HttpEndpointMaximumLength");
        }

        [Fact]
        public async Task HttpEndpointWithoutLeadingSlashIsRejectedWithErrorCodeAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("RelativeEndpoint"));
            dto.HttpEndpoint = "api/invoke/ExampleFraudScoreLocalEndpoint";

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "HttpEndpointMustBeAbsolutePath");
        }

        [Fact]
        public async Task NegativePriorityIsRejectedWithErrorCodeAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("NegativePriority"));
            dto.Priority = -0.1;
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelHttpAdaptation.Create").Subject;
        ...
        public void CatalogueRegistersUniquePascalCaseNoUnderscoreNames()
        {
            ...
            [
                "EntityAnalysisModelHttpAdaptationList", "EntityAnalysisModelHttpAdaptationGet",
                "EntityAnalysisModelHttpAdaptationGetByEntityAnalysisModelId",
                "EntityAnalysisModelHttpAdaptationCreate", "EntityAnalysisModelHttpAdaptationUpdate",
                "EntityAnalysisModelHttpAdaptationDelete"
            ]);
        }

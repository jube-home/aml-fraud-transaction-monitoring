...
using Jube.Service.RedisMetric;
...
namespace Jube.Test.Service.RedisMetric
{
    using RedisMetricService = RedisMetricService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class RedisMetricServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            {
                await dbContext.RedisMetric.Where(w => w.Id == id).DeleteAsync();
            }
        }

        private static Task<RedisMetricService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return RedisMetricService.CreateAsync(
                dbContext, userName, log ?? TestLog.NoOp, localizers, serviceChangeBus ?? new NullServiceChangeBus(),
                auditLog ?? TestLog.NoOp);
        }

        private async Task CreateSampleAsync(DbContext dbContext, string instance, int connectedClients,
            DateTime? createdDate = null, DateTime? lastAofRewriteDate = null, DateTime? lastBgSaveDate = null)
        {
            var id = await dbContext.InsertWithInt32IdentityAsync(new Data.Poco.RedisMetric
            {
                CreatedDate = createdDate ?? DateTime.UtcNow,
                Instance = instance,
                ConnectedClients = connectedClients,
                LastAofRewriteDate = lastAofRewriteDate,
        ...
        public async Task ListReturnsMostRecentFirstAndProjectsFieldsAsync()
        {
            ...
            mine[0].ConnectedClients.Should().Be(10);
            mine[1].ConnectedClients.Should().Be(5);
        }
        ...
        [Fact]
        public async Task ListSortsByConnectedClientsAscendingAndDescendingAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";
            ...
            var ascending = await service.ListAsync(search: instance, sortField: "connectedClients",
                sortDirection: "asc");
            ascending.Rows.Select(r => r.ConnectedClients).Should().Equal(1000, 2000, 3000);

            var descending = await service.ListAsync(search: instance, sortField: "connectedClients",
                sortDirection: "desc");
            descending.Rows.Select(r => r.ConnectedClients).Should().Equal(3000, 2000, 1000);
        }
        ...
        public async Task ListSortDirectionIsCaseInsensitiveForAscendingAsync(string ascendingKeyword)
        {
            ...
            result.Rows.Select(r => r.ConnectedClients).Should().Equal(1000, 2000);
        }
        ...
        public async Task ListWithGarbageSortDirectionFallsBackToDescendingAsync()
        {
            ...
            result.Rows.Select(r => r.ConnectedClients).Should().Equal(2000, 1000);
        }
        ...
        [Fact]
        public async Task ListStatisticsComputesExactValuesForConnectedClientsAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";
        ...
        [Fact]
        public async Task ListProjectsLastAofRewriteAndLastBgSaveDatesAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";

            var lastAofRewriteDate = new DateTime(2026, 1, 2, 3, 4, 5, DateTimeKind.Utc);
            var lastBgSaveDate = new DateTime(2026, 1, 2, 3, 0, 0, DateTimeKind.Utc);
            ...
            var mine = result.Rows.Where(r => r.Instance == instance).ToList();
            mine.Should().ContainSingle();
            mine[0].LastAofRewriteDate.Should().Be(lastAofRewriteDate);
            mine[0].LastBgSaveDate.Should().Be(lastBgSaveDate);
        }

        [Fact]
        public async Task ListProjectsNullLastAofRewriteAndLastBgSaveDatesWhenNeverObservedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";
            ...
            var mine = result.Rows.Where(r => r.Instance == instance).ToList();
            mine.Should().ContainSingle();
            mine[0].LastAofRewriteDate.Should().BeNull();
            mine[0].LastBgSaveDate.Should().BeNull();
        }

        [Fact]
        public async Task ListSortsByLastBgSaveDateAscendingAndDescendingAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var instance = $"{DatabaseFixture.Prefix}Instance{Guid.NewGuid():N}";
            ...
            var ascending = await service.ListAsync(search: instance, sortField: "lastBgSaveDate",
                sortDirection: "asc");
            ascending.Rows.Select(r => r.LastBgSaveDate).Should().Equal(earlier, later);

            var descending = await service.ListAsync(search: instance, sortField: "lastBgSaveDate",
                sortDirection: "desc");
            descending.Rows.Select(r => r.LastBgSaveDate).Should().Equal(later, earlier);
        }
        ...
        public async Task ListStatisticsIncludesEveryContinuousMeasuredColumnAsync()
        {
            ...
            result.Statistics.Columns.Keys.Should().BeEquivalentTo(
                "connectedClients", "blockedClients", "usedMemoryBytes", "usedMemoryRssBytes", "maxMemoryBytes",
                "instantaneousOpsPerSecond", "totalCommandsProcessed", "totalConnectionsReceived",
                "keyspaceHits", "keyspaceMisses", "hitRatePercent", "evictedKeys", "expiredKeys",
                "connectedReplicas", "uptimeSeconds", "totalKeys", "memoryFragmentationRatio",
                "rdbLastSaveAgeSeconds");
        }
    }
}
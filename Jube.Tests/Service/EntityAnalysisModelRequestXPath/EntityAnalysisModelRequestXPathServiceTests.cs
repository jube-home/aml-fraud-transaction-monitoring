...
{
    using EntityAnalysisModelRequestXPathService = EntityAnalysisModelRequestXPathService;

    [Trait("Category", "Service")]
    [Collection("Database")]
    public sealed class EntityAnalysisModelRequestXPathServiceTests(DatabaseFixture fx) : IAsyncLifetime
    {
        ...
        public async Task DisposeAsync()
        {
            ...
            foreach (var id in createdIds)
            {
                await dbContext.GetTable<EntityAnalysisModelRequestXpathVersion>()
                    .Where(w => w.EntityAnalysisModelRequestXpathId == id).DeleteAsync();
                await dbContext.EntityAnalysisModelRequestXpath.Where(w => w.Id == id).DeleteAsync();
            }
        ...
        private static Task<EntityAnalysisModelRequestXPathService> BuildServiceAsync(
            DbContext dbContext, string? userName, ILog? log = null, ILog? auditLog = null,
            IServiceChangeBus? serviceChangeBus = null)
        {
            return EntityAnalysisModelRequestXPathService.CreateAsync(
        ...
        private static EntityAnalysisModelRequestXPathDto NewDto(int entityAnalysisModelId, string name)
        {
            return new EntityAnalysisModelRequestXPathDto
            {
                ...
                SearchKeyCacheFetchLimit = 100000,
        ...
        [Fact]
        public async Task
            GetByEntityAnalysisModelIdByDataTypeIgnoresDataTypeIdParameterAndFiltersToStringIntegerFloatAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var stringDto = NewDto(modelId, UniqueName("StringType"));
            stringDto.DataTypeId = 1;
            var savedString = await service.InsertAsync(stringDto);
            createdIds.Add(savedString.Id);

            var dateDto = NewDto(modelId, UniqueName("DateType"));
            dateDto.DataTypeId = 4;
            dateDto.DefaultValue = "0";
            var savedDate = await service.InsertAsync(dateDto);
            createdIds.Add(savedDate.Id);

            var result = await service.GetByEntityAnalysisModelIdByDataTypeAsync(modelId, 999);
            result.Should().Contain(d => d.Id == savedString.Id);
            result.Should().NotContain(d => d.Id == savedDate.Id);
        }
        ...
        public async Task UpdateIncrementsVersionAndWritesAuditRowAsync()
        {
            ...
            dto.Id = saved.Id;
            dto.SearchKeyFetchLimit = 250;

            var updated = await service.UpdateAsync(dto);

            updated.Version.Should().Be(2);
            updated.SearchKeyFetchLimit.Should().Be(250);
            updated.Guid.Should().Be(saved.Guid);

            var auditRows = await dbContext.GetTable<EntityAnalysisModelRequestXpathVersion>()
                .Where(w => w.EntityAnalysisModelRequestXpathId == saved.Id).CountAsync();
        ...
        public async Task EveryMethodThrowsForbiddenWhenPermissionMissingAndWritesNoRowAsync()
        {
            ...
            var beforeCount = await dbContext.EntityAnalysisModelRequestXpath
            ...
            var afterCount = await dbContext.EntityAnalysisModelRequestXpath
                .CountAsync(w => w.Name!.StartsWith(DatabaseFixture.Prefix));
        ...
        public async Task NullOrBlankUserNameThrowsNotAuthenticatedBeforeAnyQueryAsync(string? userName)
        {
            ...
            var ex = await Assert.ThrowsAsync<NotAuthenticatedException>(() =>
                EntityAnalysisModelRequestXPathService.CreateAsync(dbContext, userName, log, localizers,
        ...
        public async Task NameRequiredRejectsNullEmptyOrWhitespaceAsync(string? name)
        {
            ...
            ex.Result.Errors.Should().Contain(e =>
                e.PropertyName == nameof(EntityAnalysisModelRequestXPathDto.Name) && e.ErrorCode == "NameNotEmpty");
        }
        ...
        [Fact]
        public async Task InvalidDataTypeIdIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadDataType"));
            dto.DataTypeId = 999;
        ...
        [Fact]
        public async Task InvalidEncryptionIdIsRejectedAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("BadEncryption"));
            dto.EncryptionId = 999;

            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dto));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "EncryptionIdInvalid");
        }

        [Fact]
        public async Task DateDefaultValueMustBeIntegerOffsetOnlyWhenDataTypeIsDateAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var nonDate = NewDto(modelId, UniqueName("NotDate"));
            nonDate.DataTypeId = 1;
            nonDate.DefaultValue = "not-a-number";
            var saved = await service.InsertAsync(nonDate);
            createdIds.Add(saved.Id);

            var dateInvalid = NewDto(modelId, UniqueName("BadDateOffset"));
            dateInvalid.DataTypeId = 4;
            dateInvalid.DefaultValue = "not-a-number";
            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(dateInvalid));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "DefaultValueDateOffsetInvalid");

            var dateValid = NewDto(modelId, UniqueName("GoodDateOffset"));
            dateValid.DataTypeId = 4;
        ...
        [Fact]
        public async Task SearchKeySubFieldsOnlyValidatedWhenSearchKeyEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var disabled = NewDto(modelId, UniqueName("SearchKeyOff"));
            disabled.SearchKey = false;
            disabled.SearchKeyTtlInterval = "not-a-valid-interval";
            var saved = await service.InsertAsync(disabled);
            createdIds.Add(saved.Id);

            var enabled = NewDto(modelId, UniqueName("SearchKeyOn"));
            enabled.SearchKey = true;
            enabled.SearchKeyTtlInterval = "not-a-valid-interval";
            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(enabled));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "SearchKeyTtlIntervalInvalid");
        }

        [Fact]
        public async Task SearchKeyCacheSubFieldsOnlyValidatedWhenBothSearchKeyAndSearchKeyCacheEnabledAsync()
        {
            await using var dbContext = fx.GetDbContext();
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);

            var searchKeyOnCacheOff = NewDto(modelId, UniqueName("CacheOff"));
            searchKeyOnCacheOff.SearchKey = true;
            searchKeyOnCacheOff.SearchKeyCache = false;
            searchKeyOnCacheOff.SearchKeyCacheInterval = "not-a-valid-interval";
            var saved = await service.InsertAsync(searchKeyOnCacheOff);
            createdIds.Add(saved.Id);

            var both = NewDto(modelId, UniqueName("CacheOn"));
            both.SearchKey = true;
            both.SearchKeyCache = true;
            both.SearchKeyCacheInterval = "not-a-valid-interval";
            var ex = await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(both));
            ex.Result.Errors.Should().Contain(e => e.ErrorCode == "SearchKeyCacheIntervalInvalid");
        }
        ...
        public async Task UpdateOfLockedRowThrowsNotFoundAsync()
        {
            ...
            await dbContext.EntityAnalysisModelRequestXpath.Where(w => w.Id == saved.Id)
                .Set(s => s.Locked, (byte)1).UpdateAsync();
        ...
        public async Task FieldsWithNoBackingPocoColumnAreAlwaysDefaultOnReadAsync()
        {
            ...
            var modelId = await CreateParentModelAsync(dbContext, fx.Seed.UserWithPermission);
            var service = await BuildServiceAsync(dbContext, fx.Seed.UserWithPermission);
            var dto = NewDto(modelId, UniqueName("Orphan"));
            dto.EncryptOffDeploymentRegion = true;
            dto.XPathExpression = true;
            dto.HashEntityKeyComposite = true;
            dto.HashEntryKeyComposite = true;

            var saved = await service.InsertAsync(dto);
            createdIds.Add(saved.Id);

            var fetched = await service.GetByIdAsync(saved.Id);
            fetched!.EncryptOffDeploymentRegion.Should().BeFalse();
            fetched.XPathExpression.Should().BeFalse();
            fetched.HashEntityKeyComposite.Should().BeFalse();
            fetched.HashEntryKeyComposite.Should().BeFalse();
        }
        ...
        public async Task EachCallEmitsOneSpanWithOutcomeTagAsync()
        {
            ...
            var createSpan = activities.Should()
                .ContainSingle(a => a.OperationName == "EntityAnalysisModelRequestXPath.Create").Subject;
        ...
        public async Task ReadsAndFailuresPublishNothingAsync()
        {
            ...
            await service.GetAsync();
            await Assert.ThrowsAsync<DtoValidationException>(() => service.InsertAsync(NewDto(modelId, "")));

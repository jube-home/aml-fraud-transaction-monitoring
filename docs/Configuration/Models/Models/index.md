...
| Enable Implicit Async                      | A flag indicating that a plain synchronous invocation (no Async switch on the URL) should still wait inline for the Model to finish processing, up to the Timeout Milliseconds value below, before falling back to a same-shape response populated with only the fields known before processing began. See [HTTP API Asynchronous Model Invocation](../../../Concepts/API/HTTPAsynchronousModelInvocation/index.html) for the full behaviour and how to monitor it. | True |
| Enable Implicit Async: Timeout Milliseconds | Given Enable Implicit Async, the number of milliseconds a synchronous invocation waits inline before falling back to the same-shape, not-yet-complete response described above. | 5000 |
| Enable Response Trace | A flag indicating that invocation responses for this Model should echo the full per-stage and per-rule performance breakdown (Stages) -- already captured to the Archive regardless of this switch -- into the direct response too, in addition to the always-present task wrapper stats. See [HTTP API Performance Trace](../../../Concepts/API/PerformanceTrace/index.html) for the full shape and how it differs from what always reaches the Archive. | True |
| Enable Logs: Info | Captures the same step-by-step trace normally only visible via log4net INFO logging into a `logs` array on the invocation payload instead, for a sampled subset of invocations (see Logs: Enable Sampling below), so it can be pulled from the Archive (and, optionally, the direct response) without needing backend log access. Independent of Enable Logs: Warn Threshold below -- the two are separate monitors, not mutually exclusive levels of one switch, and can be on together, individually, or not at all. Does not affect log4net's own INFO logging, which remains controlled separately by the logging configuration. See [HTTP API Invocation Trace Log](../../../Concepts/API/InvocationTraceLog/index.html). | False |
| Logs: Enable Sampling | Given Enable Logs: Info is on, only capture a random sample of invocations (see Sample Percentage below) rather than every one, so Info-level detail can run continuously in production at a small fraction of its normal cost. Has no effect on Enable Logs: Warn Threshold, which always evaluates every invocation regardless of this switch. Also gates the always-on Response Time Pipeline and Task Performance Counter capture for this Model, so a report can correlate a sampled invocation's log with its own timing/memory data. See [HTTP API Invocation Trace Log](../../../Concepts/API/InvocationTraceLog/index.html). | False |
| Logs: Sample Percentage | Given Logs: Enable Sampling is on, the percentage (0-100) of invocations captured at Info level, e.g. `0.02` for roughly 1 in 5000 invocations. | 100 |
| Enable Logs: Warn Threshold | Captures a trace point into the same `logs` array as Enable Logs: Info, but only when the gap since the previous captured entry exceeds Warn Threshold Milliseconds below -- an always-on anomaly watch (every invocation, never sampled) rather than a representative sample. Independent of Enable Logs: Info above. See [HTTP API Invocation Trace Log](../../../Concepts/API/InvocationTraceLog/index.html). | False |
| Logs: Warn Threshold Milliseconds | Given Enable Logs: Warn Threshold is on, the number of milliseconds a trace point's gap since the last captured entry must exceed to be captured. | 250 |
| Enable Logs In Response | A flag indicating that invocation responses for this Model should include the captured `logs` array, in addition to it always being included in the Archive payload whenever either Enable Logs: Info or Enable Logs: Warn Threshold is on. | True |
...
- Implicit Async Timeout Milliseconds must be greater than zero, and is only validated when Enable Implicit Async is on.
- Logs: Sample Percentage must be greater than 0 and no more than 100, and is only validated when Enable Logs: Info and Logs: Enable Sampling are both on.
- Logs: Warn Threshold Milliseconds must be greater than zero, and is only validated when Enable Logs: Warn Threshold is on.

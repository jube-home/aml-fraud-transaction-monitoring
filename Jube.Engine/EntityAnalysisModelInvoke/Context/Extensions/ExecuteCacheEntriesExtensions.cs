...
namespace Jube.Engine.EntityAnalysisModelInvoke.Context.Extensions
{
    public static class ExecuteCacheEntriesExtensions
    {
            ...
            Dictionary<string, DistinctSearchKey> distinctSearchKeys)
        {
            ...
            stages.CacheDbStorage = new StageDuration
            {
        ...
        internal static void UpsertCachePayloadLatest(Context context, CacheService cacheService,
            Dictionary<string, DistinctSearchKey> distinctSearchKeys)
        {
            foreach (var (key, _) in distinctSearchKeys)
            {
                ...
                context.PendingWriteTasks.Add(TaskHelper.MeasureTaskTimeAndMemoryAllocatedAsync(
                    TaskType.CachePayloadLatestUpsertAsync, async () => await cacheService.CachePayloadLatestRepository
                        .UpsertAsync(
                            ...
                            key, searchKeyValue.AsString()).ConfigureAwait(false), context.Log));
            }
        }

        internal static void InsertOrReplaceCacheEntries(Context context, CacheService cacheService)
        {
            ...
            {
                context.TraceLog($"does not allow entity storage in the cache.");
            }
        }

        internal static DictionaryNoBoxing<int> ParseToReducedInternedPayload(Context context)
        {
                             ...
                             entityAnalysisModelInlineScript.EntityAnalysisModelInlineScriptPropertyAttributes))
            {
                ...
                if (context.EntityAnalysisModelInstanceEntryPayload.Payload.TryGetValue(
                        entityAnalysisModelInlineScriptPropertyAttribute.Key, out var value))
                {
            ...
            {
                if (context.EntityAnalysisModelInstanceEntryPayload.Payload.TryGetValue(
                        entityAnalysisModelRequestXPath.Name, out var value))
                {

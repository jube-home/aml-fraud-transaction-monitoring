...
namespace Jube.Data.Repository
{
    ...
    public class CaseWorkflowDisplayRoleRepository
    {
        ...
        public Task<bool> ExistsCaseWorkflowDisplayAsync(Guid caseWorkflowDisplayGuid,
            CancellationToken token = default)
        {
            return dbContext.CaseWorkflowDisplay.AnyAsync(w =>
                w.Guid == caseWorkflowDisplayGuid
        ...
        public Task<List<CaseWorkflowDisplayRole>> GetByCaseWorkflowDisplayGuidAsync(Guid caseWorkflowDisplayGuid,
            CancellationToken token = default)
        {
        ...
        public async Task<CaseWorkflowDisplayRole> InsertAsync(CaseWorkflowDisplayRole model,
            CancellationToken token = default)
        {
            var duplicate = await dbContext.CaseWorkflowDisplayRole.FirstOrDefaultAsync(w =>
                    w.CaseWorkflowDisplayGuid == model.CaseWorkflowDisplayGuid && w.RoleRegistryGuid ==
            ...
            {
                var raced = await dbContext.CaseWorkflowDisplayRole.FirstOrDefaultAsync(w =>
                        w.CaseWorkflowDisplayGuid == model.CaseWorkflowDisplayGuid && w.RoleRegistryGuid ==
                        model.RoleRegistryGuid
                        && (w.Deleted == 0 || w.Deleted == null),
                    token);
            ...
            CancellationToken token = default)
        {
            return dbContext.CaseWorkflowDisplayRole
                .Where(d =>
                    d.CaseWorkflowDisplay.CaseWorkflow.EntityAnalysisModel.TenantRegistryId ==

...
{
    public class PostgresLogEntryRepository(DbContext dbContext)
    {
        public Task BulkCopyAsync(List<PostgresLogEntry> models, CancellationToken token = default)
        {
            return dbContext.BulkCopyAsync(models, token);
        }

        public async Task<IEnumerable<PostgresLogEntry>> GetLastAsync(int take, DateTime? from, DateTime? to,
        ...
        private IQueryable<PostgresLogEntry> BuildFilteredQuery(DateTime? from, DateTime? to, string search,
            double? samplePercentage)
        {
            var query = dbContext.PostgresLogEntry.AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                query = query.Where(w =>
                    ...
                    (w.Level != null && w.Level.ToLower().Contains(lowerSearch)));
            }
        ...
        private static IOrderedQueryable<PostgresLogEntry> ApplySort(IQueryable<PostgresLogEntry> query,
            string sortField, string sortDirection)
        {
            ...
            return sortField switch
            {
                ...
                "pid" => query.OrderByField(o => o.Pid, descending),

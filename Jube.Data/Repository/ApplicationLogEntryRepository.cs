...
{
    public class ApplicationLogEntryRepository(DbContext dbContext)
    {
        public Task BulkCopyAsync(List<ApplicationLogEntry> models, CancellationToken token = default)
        {
            return dbContext.BulkCopyAsync(models, token);
        }

        public async Task<IEnumerable<ApplicationLogEntry>> GetLastAsync(int take, DateTime? from, DateTime? to,
        ...
        private IQueryable<ApplicationLogEntry> BuildFilteredQuery(DateTime? from, DateTime? to, string search,
            double? samplePercentage)
        {
            var query = dbContext.ApplicationLogEntry.AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                query = query.Where(w =>
                    w.Message.ToLower().Contains(lowerSearch) || w.LoggerName.ToLower().Contains(lowerSearch) ||
        ...
        private static IOrderedQueryable<ApplicationLogEntry> ApplySort(IQueryable<ApplicationLogEntry> query,
            string sortField, string sortDirection)
        {
            var descending = !string.Equals(sortDirection, "asc", StringComparison.OrdinalIgnoreCase);
            return sortField switch
            {
                "level" => query.OrderByField(o => o.Level, descending),
                "loggerName" => query.OrderByField(o => o.LoggerName, descending),
                "threadContext" => query.OrderByField(o => o.ThreadContext, descending),

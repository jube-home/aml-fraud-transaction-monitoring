...
{
    public class OverlayNetworkTaskDriftRepository(DbContext dbContext)
    {
        public Task BulkCopyAsync(List<OverlayNetworkTaskDrift> models, CancellationToken token = default)
        {
            return dbContext.BulkCopyAsync(models, token);
        }

        public async Task<IEnumerable<OverlayNetworkTaskDrift>> GetLastAsync(int take, DateTime? from, DateTime? to,
        ...
        private IQueryable<OverlayNetworkTaskDrift> BuildFilteredQuery(DateTime? from, DateTime? to, string search,
            double? samplePercentage)
        {
            var query = dbContext.OverlayNetworkTaskDrift.AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                query = query.Where(w => w.ServiceName != null && w.ServiceName.ToLower().Contains(lowerSearch));
            }
        ...
        private static IOrderedQueryable<OverlayNetworkTaskDrift> ApplySort(
            IQueryable<OverlayNetworkTaskDrift> query, string sortField, string sortDirection)
        {
            var descending = !string.Equals(sortDirection, "asc", StringComparison.OrdinalIgnoreCase);
            return sortField switch
            {
                "serviceName" => query.OrderByField(o => o.ServiceName, descending),
                "isConsistent" => query.OrderByField(o => o.IsConsistent, descending),

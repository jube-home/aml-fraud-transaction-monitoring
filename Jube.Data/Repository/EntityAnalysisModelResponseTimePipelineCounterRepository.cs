...
{
    public class EntityAnalysisModelResponseTimePipelineCounterRepository
    {
        private readonly DbContext dbContext;
        private readonly int? tenantRegistryId;

        public EntityAnalysisModelResponseTimePipelineCounterRepository(DbContext dbContext, string userName)
        {
        ...
        public EntityAnalysisModelResponseTimePipelineCounterRepository(DbContext dbContext)
        {
            this.dbContext = dbContext;
        }

        public async Task<EntityAnalysisModelResponseTimePipelineCounter> InsertAsync(
            EntityAnalysisModelResponseTimePipelineCounter model, CancellationToken token = default)
        {
            model.Id = await dbContext.InsertWithInt32IdentityAsync(model, token: token).ConfigureAwait(false);
            return model;
        }

        private IQueryable<EntityAnalysisModelResponseTimePipelineCounter> BuildFilteredQuery(
            bool includeAllTenants, DateTime? from, DateTime? to, int? stageId, double? samplePercentage)
        {
            var query = dbContext.EntityAnalysisModelResponseTimePipelineCounter
        ...
        private static IOrderedQueryable<EntityAnalysisModelResponseTimePipelineCounter> ApplySort(
            IQueryable<EntityAnalysisModelResponseTimePipelineCounter> query, string sortField,
            string sortDirection)
        {
            ...
            return sortField switch
            {
                ...
                "sequenceNumber" => query.OrderByField(o => o.SequenceNumber, descending),
        ...
        public async Task<IEnumerable<EntityAnalysisModelResponseTimePipelineCounterRow>> GetLastAsync(
            ...
            double? samplePercentage, string sortField, string sortDirection, CancellationToken token = default)
        {
            ...
            return await ApplySort(query, sortField, sortDirection)
                .Take(take)
                .Select(s => new EntityAnalysisModelResponseTimePipelineCounterRow(
                    s.Id, s.EntityAnalysisModelGuid, s.EntityAnalysisModel.Name, s.StageId, s.SequenceNumber,

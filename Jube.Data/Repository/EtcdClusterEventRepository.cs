...
{
    public class EtcdClusterEventRepository(DbContext dbContext)
    {
        public async Task<EtcdClusterEvent> InsertAsync(EtcdClusterEvent model, CancellationToken token = default)
        {
            model.Id = await dbContext.InsertWithInt32IdentityAsync(model, token: token).ConfigureAwait(false);
            return model;
        }

        public async Task<IEnumerable<EtcdClusterEvent>> GetLastAsync(int take, DateTime? from, DateTime? to,
        ...
        private IQueryable<EtcdClusterEvent> BuildFilteredQuery(DateTime? from, DateTime? to, int? eventTypeId,
            string search, double? samplePercentage)
        {
            var query = dbContext.EtcdClusterEvent.AsQueryable();
        ...
        private static IOrderedQueryable<EtcdClusterEvent> ApplySort(IQueryable<EtcdClusterEvent> query,

...
{
    public class RedisSlowOperationRepository(DbContext dbContext)
    {
        public async Task<RedisSlowOperation> InsertAsync(RedisSlowOperation model, CancellationToken token = default)
        {
            model.Id = await dbContext.InsertWithInt32IdentityAsync(model, token: token).ConfigureAwait(false);
            return model;
        }

        public async Task<IEnumerable<RedisSlowOperation>> GetLastAsync(int take, DateTime? from, DateTime? to,
            ...
            double? samplePercentage, int statisticsCap, CancellationToken token = default)
        {
            ...
            {
                ["durationMicroseconds"] = rows.Where(w => w.DurationMicroseconds.HasValue)
                    .Select(s => (double)s.DurationMicroseconds!.Value).ToArray()
            });
        }

        private IQueryable<RedisSlowOperation> BuildFilteredQuery(DateTime? from, DateTime? to, string search,
            double? samplePercentage)
        {
            var query = dbContext.RedisSlowOperation.AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                query = query.Where(w =>
                    w.Command.ToLower().Contains(lowerSearch) ||
                    (w.CommandName != null && w.CommandName.ToLower().Contains(lowerSearch)) ||
                    (w.KeyName != null && w.KeyName.ToLower().Contains(lowerSearch)) ||
        ...
        private static IOrderedQueryable<RedisSlowOperation> ApplySort(IQueryable<RedisSlowOperation> query,
            string sortField, string sortDirection)
        {
            var descending = !string.Equals(sortDirection, "asc", StringComparison.OrdinalIgnoreCase);
            return sortField switch
            {
                "redisSlowLogId" => query.OrderByField(o => o.RedisSlowLogId, descending),
                "durationMicroseconds" => query.OrderByField(o => o.DurationMicroseconds, descending),
                "command" => query.OrderByField(o => o.Command, descending),
                "commandName" => query.OrderByField(o => o.CommandName, descending),
                "keyName" => query.OrderByField(o => o.KeyName, descending),
